    ...
    for step in range(num_steps):
        ...
        if verbose:
            for t in batch[:2]:
                ...
                print(f"  correct={t.is_correct} reward={t.reward:.2f} format={t.has_answer_tag}")
                print()
        ...
        loss_str = f" loss={metrics.loss:.4f} reward={metrics.avg_reward:.2f}" if batch else ""
...
def main():
    ...
    stats = run_sync_loop(actors, args.num_steps, args.num_generators, verbose=args.verbose)
