...
if [ -n "$COMM_MODE" ]; then
    ...
    NGPU="${NGPU}" LOCAL_RANK=0 python3 -m torchtitan.train --model ${MODEL} --config ${CONFIG} "$@" --comm.mode=${COMM_MODE} --training.steps 1
else
    ...
    -m torchtitan.train --model ${MODEL} --config ${CONFIG} "$@"
fi
