...
4. **Automation mode** -- Claude Code can make mistakes. Make sure the user
   ...
   - "Semi-auto" -- Claude does triage and prepares the fix, but step 6
     ...
     submit`.
5. **Max fix attempts** -- ask how many fix-and-push cycles the skill is
   allowed before bailing out. Offer:
   - "20 (default)"
   - "10"
   - "50"
   The chosen number caps step 6: once that many commits have been pushed
   for this invocation, stop and jump to step 7 with the remaining failures
   listed as deferred.
...
Maintain a counter `fix_attempts` (starts at 0) across the loop; increment
it each time step 6 actually pushes or `ghstack submit`s. When
`fix_attempts >= max_fix_attempts`, jump to step 7.
...
3. **Branch on the categorization:**
   ...
   - Only `pending` remain -> `sleep 540` (9 min) in a single Bash call
     with `timeout: 600000` (the Bash tool caps at 10 min in ms; 540s
     leaves headroom for the call itself), then go back to step 2. Tell
     the user once before the first sleep so they know the skill is
     babysitting; don't reprint the same status on every tick.
...
4. **Triage failures.** For each failed job, decide whether this PR is the
   ...
   - If **none** are `caused-by-this-PR` and there are still `pending` jobs,
     `sleep 540` (with `timeout: 600000`) and go back to step 2.
...
5. **Fix the PR-caused failures.** Investigate root cause per the project
   ...
   - For linter / type errors (e.g. pyre, pyrefly, mypy): fix the **root
     cause**. Import the correct type, add an `assert isinstance(...)` to
     narrow the type, restructure the code so the type checker can prove
     the invariant, or fix the actual bug the checker found. Only use
     `# pyrefly: ignore` (or the equivalent suppression comment) when the
     error genuinely cannot be fixed.
...
6. **Commit and update the PR using the workflow chosen in the prerequisite.**
   Before doing anything in this step, check `fix_attempts` against
   `max_fix_attempts`; if the cap is hit, skip directly to step 7.
   ...
   Increment `fix_attempts`. Tell the user what you pushed (commit SHA +
   one-line summary), then `sleep 60` to give CI a moment to register the
   new commit, and go back to step 2.

7. **Write a summary.** Include: PR link, final triage table, what was
   fixed and why, what was deferred (with reasons -- e.g.
   `needs-pytorch-fix`, flaky, infra, pre-existing on main, or
   `max-fix-attempts reached`), and the new commit / ghstack revision
   link(s). Stop the loop here.
