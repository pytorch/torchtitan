# this version adds activation_manager context for offloading activations.
...
class manage_activations(saved_tensors_hooks):
    ...
    def __init__(self, pin_memory: bool = True, device_type: str = "cuda") -> None:
        ...
        self.caching: bool = True  # now we're managing cpu cached memory blocks
        ...
        self.mem_offload_cache = {}  # cache of available memory blocks for tensors, keyed by (shape, dtype)
        ...
        def get_cache_key(x: torch.Tensor)-> tuple[tuple, torch.dtype]:
            # get the tensor shape and dtype as a tuple for cached memory re-use
        ...
        def pack_tensor(activation: torch.Tensor) -> str:
            ...
            if self.is_first_forward:
                ...
                print(f"cache size: {sum(len(v) for v in self.mem_offload_cache.values())} tensors")

                # Clear tracker but keep the memory cache
                self.tracker.clear()
                self.active_tensors = 0
                self.cache_hits = 0
                self.cache_misses = 0
            ...
            cache_key = get_cache_key(activation)
            ...
            if num_bytes < self.min_tensor_size_bytes or (activation_dtype in self.ignore_types):
                ...
                # Try to reuse cached memory
                cpu_tensor = None
                if self.caching and cache_key in self.mem_offload_cache and len(self.mem_offload_cache[cache_key]) > 0:
                    # Reuse existing tensor from cache
                    cpu_tensor = self.mem_offload_cache[cache_key].pop()
                    cpu_tensor.copy_(activation)
                    self.cache_hits += 1
        ...
        def unpack_tensor(unpack_tensor_id: str) -> torch.Tensor:
            ...
            # Add the CPU tensor back to the cache for reuse
            if self.caching:
                cache_key = get_cache_key(cpu_tensor)  # Use the same function as in pack_tensor
                if cache_key not in self.mem_offload_cache:
                    self.mem_offload_cache[cache_key] = []
                self.mem_offload_cache[cache_key].append(cpu_tensor)
                print(f"Added tensor to cache with key {cache_key}")
            else:
                # If not caching, we don't need the CPU tensor anymore
                del cpu_tensor
...
class manage_activations(saved_tensors_hooks):
    ...
    def __init__(self, pin_memory: bool = True, device_type: str = "cuda", offload_ratio: float = 1.0) -> None:
        device_module = getattr(torch, device_type, torch.cuda)

        self.caching: bool = False # are we managing cpu cached memory blocks
        self.min_tensor_size_bytes = 4096 # we don't want to bother with small tensors
        self.tracker = {} # tensor_id = (new_tensor, dtype, if_modified, device)  ---> track what saved/offloaded/compressed tensors, are where
        ...
        self.active_tensors = 0
        self.pin_memory = pin_memory
        # offload ratio (0.0 = keep all on GPU, 1.0 = offload all to CPU)
        self.offload_ratio = max(0.0, min(1.0, offload_ratio))
        self.offloaded_count = 0
        self.kept_on_gpu_count = 0
        ...
        def pack_tensor(activation: torch.Tensor) -> str:
            ...
            if self.is_first_forward:
                ...
                print(f"total managed activations  {len(self.tracker)=}")
                print(f"offload ratio: {self.offload_ratio}, offloaded: {self.offloaded_count}, kept on GPU: {self.kept_on_gpu_count}")
                #if not self.caching:
                self.tracker.clear()
                self.active_tensors = 0
                self.offloaded_count = 0
            ...
            if num_bytes < self.min_tensor_size_bytes or (activation_dtype in self.ignore_types):
                ...
                # Decide whether to offload this tensor based on the ratio
                should_offload = (self.offloaded_count / (self.offloaded_count + self.kept_on_gpu_count + 1) < self.offload_ratio)
                ...
                if should_offload:
                    ...
                    self.tracker[tensor_id] = (cpu_tensor, activation_dtype, True, original_device)
                    self.offloaded_count += 1
                    print(f"Offloaded tensor {tensor_id} to CPU")
                else:
                    # Keep on GPU
                    gpu_tensor = activation.clone().detach()
                    self.tracker[tensor_id] = (gpu_tensor, activation_dtype, True, original_device)
                    self.kept_on_gpu_count += 1
                    print(f"Kept tensor {tensor_id} on GPU")
        ...
        def unpack_tensor(unpack_tensor_id: str) -> torch.Tensor:
            ...
            if self.is_first_backward:
                ...
                if self.timing:
                    ...
                    print(f"***** forward took {(end_forward_time - self.forward_start_time):.3f} seconds")
                    print(f"***** first backward, managing {len(self.tracker)} tensors")
                    print(f"offloaded: {self.offloaded_count}, kept on GPU: {self.kept_on_gpu_count}")
                    self.backward_start_time = time.perf_counter()

            # retrieve the saved/offloaded/compressed tensor
            assert unpack_tensor_id in self.tracker, f"untracked tensor, {unpack_tensor_id}"
            tensor, dtype, modified, original_device = self.tracker[unpack_tensor_id]
            print(f"Unpacking {unpack_tensor_id}, {tensor.size()}, {tensor.dtype=}, {modified=}")

            # If tensor is on CPU, move it back to original device
            if tensor.device.type != original_device.type:
                gpu_tensor = tensor.to(original_device, non_blocking=self.pin_memory)
            else:
...
class manage_activations_old(saved_tensors_hooks):
    """Context manager under which activation tensors created in the forward pass will be managed
    """
