    ...
    const double eps)
{
    for (auto i1 = blockIdx.y; i1 < n1; i1 += gridDim.y) {
        ...
        // Inter-warp reductions
        __shared__ U shared_buf[32];
        if (threadIdx.y == 0) {
            shared_buf[threadIdx.x] = sum_loss2;
        }
        __syncthreads();

        if (threadIdx.y == 0) {
            if (threadIdx.x < blockDim.y) {
                sum_loss2 = shared_buf[threadIdx.x];
            } else {
                sum_loss2 = U(0);
            }

            for (int offset = blockDim.y / 2; offset > 0; offset /= 2) {
                sum_loss2 += WARP_SHFL_XOR(sum_loss2, offset);
            }
        }

        // All threads now have the final sum_loss2 value
        const U fH = static_cast<U>(n2);
        const U term1 = (U(1) / fH) * c_invvar;
