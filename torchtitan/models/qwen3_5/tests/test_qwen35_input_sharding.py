...
def test_qwen35_has_no_build_forward_inputs():
    assert "_build_forward_inputs" not in vars(Qwen35Model)


def test_qwen35_has_no_input_sharding():
    assert "input_sharding" not in vars(Qwen35Model)


def test_qwen35_preprocess_builds_mask_and_returns(monkeypatch):
    # Fully self-contained preprocess_inputs: with the default spmd_backend the
    # SPMD branches (and the deltanet annotation) are skipped, so this exercises
    # the inlined mask gate, merged sharding path, ntokens, and pop without
    # building the heavy Qwen35 model.
    monkeypatch.setattr(
        Qwen35Model, "get_attention_masks", lambda self, positions: "MASK"
    )
    m = cast(Qwen35Model, object.__new__(Qwen35Model))
