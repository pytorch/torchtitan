...
class GptOssStateDictAdapter(MoEStateDictAdapter):
    ...
    def to_hf(self, state_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in state_dict.items():
            if "layers" in key:
                ...
                if "moe.experts" in key and (
                    "mlp1_weight" in key or "mlp2_weight" in key
                ):
                    if value.ndim == 3:  # Weight tensor (not bias)
                        value = value.transpose(1, 2).contiguous()
                        logger.info(
                            f"Transposed for HF export: {key} -> {hf_key} {tuple(value.shape)}"
                        )
    ...
    def from_hf(self, hf_state_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        # Note: After MXFP4 dequantization, _blocks keys become regular 3D tensors
        needs_transpose = None
        for key, value in hf_state_dict.items():
            if "gate_up_proj" in key and value.ndim == 3:
                if value.shape[1] < value.shape[2]:
                    needs_transpose = True
                    logger.info(
                        f"Detected HF format (needs transpose): {key} {tuple(value.shape)}"
                    )
                else:
                    needs_transpose = False
                    logger.info(
                        f"Detected TorchTitan format (no transpose needed): {key} {tuple(value.shape)}"
                    )
                break
        ...
        if needs_transpose is None:
            needs_transpose = True
            logger.info(
                "Could not detect format from gate_up_proj, defaulting to transpose"
            )
        ...
        for key, value in hf_state_dict.items():
            ...
            if "layers" in key:
                ...
                # Note: MXFP4 dequantized weights (_blocks) also need transpose -
                # dequantization only converts format, doesn't change orientation
                if "mlp.experts" in key and (
                    "gate_up_proj" in key or "down_proj" in key
                ):
                    # Check if this is a 3D weight tensor that needs transpose
                    if (
                        value.ndim == 3
                        and not key.endswith("_bias")
                        and needs_transpose
                    ):
                        logger.info(
                            f"Transposing HF->TorchTitan: {key} {tuple(value.shape)}"
                        )
        ...
        logger.info(f"from_hf() complete: {len(state_dict)} keys, {transpose_count} transposed")
        return state_dict
