...
def closest_ts_idx(ts_list, ts):
    """
    Finds the index of the closest timestamp in a list to a given timestamp.

    Parameters:
        ts_list (list or np.ndarray): List of timestamps.
        ts (float): The timestamp to compare against.

    Returns:
        int: Index of the closest timestamp in ts_list. Returns -1 if ts_list is empty.

    Example:
        >>> closest_ts_idx([1.0, 2.5, 3.0], 2.7)
        1
    """
    # Check if the list is empty; if so, return -1
    if len(ts_list):
        return np.argmin(np.abs(ts_list - ts))
    else:
        return -1


def extract_metadata_aligned(
    metadata_json_path, camera_key, metadata_keys=["gps", "vel", "yawRate"]
):
    """
    Extracts and aligns metadata based on camera timestamps from a JSON file.

    Parameters:
        metadata_json_path (str): Path to the metadata JSON file.
        camera_key (str): Key to identify the camera timestamps in the metadata.
        metadata_keys (list of str): List of metadata keys to extract and align. Default is ["gps", "vel", "yawRate"].

    Returns:
        dict: Dictionary containing aligned metadata with keys "lat", "lon", "vel", and "yawRate".

    Raises:
        ValueError: If the camera key is not found in the metadata.
        NotImplementedError: If an unrecognized metadata key is provided.

    Example:
        >>> extract_metadata_aligned('metadata.json', 'camera1')
        {'lat': [...], 'lon': [...], 'vel': [...], 'yawRate': [...]}
    """
    ...
    # Find the camera timestamps in the metadata
    for key in metadata.keys():
        if camera_key in "_".join(key[1:].split("/")):
            cam_ts = metadata[key]
            break

    if not cam_ts:
        raise ValueError(
            f"Could not find camera {camera_key} in metadata {metadata.keys()} path: {metadata_json_path}!"
        )

    res = {"lat": [], "lon": [], "vel": [], "yawRate": []}

    # Loop through each camera timestamp to align metadata
    for frame_ts in cam_ts:
        for metakey in metadata_keys:
            if metakey == "gps":
                ts = np.array(metadata["/gps/fix_ts"])
                idx = closest_ts_idx(ts, frame_ts)
                if len(metadata["/gps/fix_lat"]):
                    res["lat"].append(metadata["/gps/fix_lat"][idx])
                    res["lon"].append(metadata["/gps/fix_lon"][idx])
                else:
                    res["lat"].append(None)
                    res["lon"].append(None)
            elif metakey == "vel":
                ts = np.array(metadata["/vel_ts"])
                idx = closest_ts_idx(ts, frame_ts)
                if len(metadata["/vel_data"]):
                    res["vel"].append(metadata["/vel_data"][idx])
                else:
                    res["vel"].append(None)
            elif metakey == "yawRate":
                ts = np.array(metadata["/yawRate_ts"])
                idx = closest_ts_idx(ts, frame_ts)
                if len(metadata["/yawRate_data"]):
                    res["yawRate"].append(metadata["/yawRate_data"][idx])
                else:
                    res["yawRate"].append(None)
            else:
                raise NotImplementedError(f"{metakey} not recognized!")
...
class TrafficLightDataset(Dataset):
    ...
    Parameters:
        video_dir (str): Directory containing video files or a CSV/Parquet file with video paths and metadata.
        video_type (str): whether to load ampel (wide) or stereo videos
    ...
    Attributes:
        ...
        metadata (dict): Dictionary containing aligned metadata for each video group,
                         keyed by the path of the first camera in that group.
    ...
    def __init__(
        ...
        self.video_type_filter = dict(
            stereo=["stereo", "ZyncBoxCamera_10300"],
            ampel=["ampel", "ZyncBoxCamera_10200"],
        )[video_type]

        # Collect all relevant video paths first
        all_raw_video_paths = [
            str(path)
            for path in Path(video_dir).rglob("*.mp4")
            if any([x in str(path) for x in self.video_type_filter])
        ]
        ...
        for group_idx, video_group in enumerate(tqdm(self.video_paths)):
            ...
            try:
                ...
                # For now, we'll append NaNs or zeros to keep array lengths consistent,
                # but a more robust solution might remove the group entirely.
            ...
            # Load metadata once per group, using the first video's path to locate metadata.json
            metadata_path = os.path.join(
                os.path.dirname(first_video_path_in_group), "metadata.json"
            )
            if os.path.exists(metadata_path):
                self.metadata[first_video_path_in_group] = extract_metadata_aligned(
                    metadata_path,
                    os.path.splitext(os.path.basename(first_video_path_in_group))[0],
                )
            else:
                self.metadata[first_video_path_in_group] = {}
                rank_zero_print(
                    f"No metadata.json found for group starting with '{first_video_path_in_group}'."
                )
        ...
        #         for i in range(0, num_frame // ((frame_interval - 1) * frame_step + 1)):
        #             self.idx_map.append(
        #                 (group_idx, i * ((frame_interval - 1) * frame_step + 1))
        #             )
    ...
    def __len__(self):
        return self.cum_avail_startframes[-1] #len(self.idx_map)
    ...
    def _get_metadata(self, group_idx, start_frame_idx, frame_interval, frame_step):
        ...
        if (
            ...
            # Ensure metadata lists are long enough for the requested slice
            # This is a safeguard, especially with mock data or incomplete metadata files
            metadata_slice = {}
            for key, value_list in self.metadata[first_video_path_in_group].items():
                end_idx = start_frame_idx + frame_interval * frame_step + 1
                if end_idx > len(value_list):
                    rank_zero_print(
                        f"Warning: Metadata for key '{key}' in group {group_idx} is too short. "
                        f"Requested up to index {end_idx-1}, but list has length {len(value_list)}. "
                        f"Padding with last available value."
                    )
                    # Pad with the last available value or a default
                    padded_list = (
                        value_list + [value_list[-1]] * (end_idx - len(value_list))
                        if value_list
                        else [None] * end_idx
                    )
                    metadata_slice[key] = padded_list[
                        start_frame_idx:end_idx:frame_step
                    ]
                else:
                    metadata_slice[key] = value_list[start_frame_idx:end_idx:frame_step]
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Assuming final_frames shape is (num_cameras, num=_frames_per_interval, C, H, W)
...
if __name__ == "__main__":
    dataset_root = "TLD"
    dataset = TrafficLightDataset(dataset_root, frame_interval=50, frame_step=2)
    loader = DataLoader(dataset, 2, False, collate_fn=custom_collate_fn)
    batched_data = next(iter(loader))
    # data = dataset[0]
    # save_dataloader_iteration_as_video(data, 'tld_sample.mp4', fps=7)
