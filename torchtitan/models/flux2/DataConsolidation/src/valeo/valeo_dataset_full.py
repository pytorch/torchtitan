...
# Reads the metadata.parquet file finds the synchronized frames between camera and lidar, 
# slices from the merged video and loads them along with a common caption, 
# including the corresponding range images of lidar data too.
...
class ValeoDatasetPreprocessed(Dataset):
    """
    A fast dataloader that reads from a preprocessed Valeo dataset directory.
    This version is optimized to read camera data from a merged video and
    Velodyne LiDAR range images from a single HDF5 (.h5) file, ignoring Scala data.
    """
    def __init__(self, config: DatasetConfig, camera_grid_shape: tuple[int, int] = (2, 3)):
        super().__init__()
        self.config = config
        ...
        self.item_index = []
        self.rec_registry = {} # Stores video/h5 paths for each folder found

        # 1. Flexible Recursive Search
        print(f"Deep scanning {self.base_path} for recordings...")
        # We find every metadata.parquet regardless of folder depth
        metadata_files = list(self.base_path.rglob("metadata.parquet"))
        print(f"Found {len(metadata_files)} potential recording metadata files.")

        # 2. Aggregation Loop
        for metadata_path in tqdm(metadata_files, desc="Indexing Dataset"):
            rec_root = metadata_path.parent  # This is the specific recording folder
            
            # Flexible Video Search: Check root OR camera subfolder
            video_options = [
                rec_root / "camera" / "merged_cameras.mp4",
                rec_root / "merged_cameras.mp4"
            ]
            video_path = next((p for p in video_options if p.exists()), None)
            
            # LiDAR Path (Always in recording root)
            h5_path = rec_root / "velodyne_range_images.h5"

            # Only index if preprocessing is actually finished for this folder
            #if not (metadata_path.exists() and video_path.exists() and h5_path.exists()):
            if video_path is None:
                print(f"--- SKIPPING {rec_root.name}: Missing Data")
                continue
            ...
            try:
                ...
                # Store the paths for this specific recording
                self.rec_registry[rec_name] = {
                    "video_path": str(video_path),
                    "h5_path": str(h5_path),
                    "cap": None,      # Initialized lazily in __getitem__
                    "h5_handle": None # Initialized lazily in __getitem__
                }

                #Add frames to the global index and sequencing as they were written to the video file.
                for _, group in all_matches.group_by("ref_frame_id", maintain_order=True):
                    self.item_index.append({
                        'rec_id': rec_name,
                        'frame_idx': group['video_frame_idx'][0],
                        'sensor_frames': {row['sensor_id']: row['frame_id'] for row in group.iter_rows(named=True)}
                    })

            except Exception:
                continue

        # 3. Final Validation: Stop if no data was found
        if not self.item_index:
            # Single clear print stating the problem and the path
            print(f"\n!! ERROR: No preprocessed folders found in: {self.base_path}\n")
            
            # Raise an error to stop the loading process immediately
            raise ValueError(f"Dataset is empty. No valid recordings found at {self.base_path}")
        else:
            self.is_valid = True
            print(f"\n>>> Success: Loaded {len(self.item_index)} frames from {len(self.rec_registry)} valid recordings.")

        
        # Cam IDs for compatibility with original code
        self.cam_ids = ["CAM_ID_70", "CAM_ID_71", "CAM_ID_72", "CAM_ID_73", "CAM_ID_74", "CAM_ID_75"]
    ...
    def __getitem__(self, idx):
        if not hasattr(self, 'is_valid') or not self.is_valid:
            return {'jpg': torch.zeros((3, 512, 512)), 'txt': "empty", 'velodyne_range_image': torch.zeros((64, 1024))}

        item_info = self.item_index[idx]
        rec_data = self.rec_registry[item_info['rec_id']]

        # --- Lazy initialization of handles (Worker-local) ---
        if rec_data['cap'] is None:
            rec_data['cap'] = cv2.VideoCapture(rec_data['video_path'])
        if rec_data['h5_handle'] is None:
            rec_data['h5_handle'] = h5py.File(rec_data['h5_path'], 'r')

        final_out = {}
        
        # 1. Load and process camera data (Back to original logic)
        video_idx = item_info['frame_idx']
        #video_idx = idx
        rec_data['cap'].set(cv2.CAP_PROP_POS_FRAMES, video_idx)
        ret, grid_frame = rec_data['cap'].read()
        if not ret:
            raise ValueError(f"Could not read frame {video_idx}")
        ...
        if cam_tensors:
            ...
            final_out['jpg'] = torch.clamp(image, -1.0, 1.0)

        final_out['txt'] = "A photorealistic image from a front-facing car camera."

        # 2. Load Velodyne range image (Back to original logic)
        range_image = rec_data['h5_handle']['range_images'][video_idx]
        final_out['velodyne_range_image'] = torch.from_numpy(range_image.astype(np.float32))
            
        return final_out

    def __del__(self):
        for rec in self.rec_registry.values():
            if rec['cap'] is not None: rec['cap'].release()
            if rec['h5_handle'] is not None: rec['h5_handle'].close()
