...
class ValeoDatasetPreprocessed(Dataset):
    def __init__(self, config: DatasetConfig, index_file_path: Optional[str] = None, **kwargs):
        # ... (The __init__ logic for loading the pickle index remains the same) ...
        # The rec_registry loaded from the pickle file already contains the correct
        # paths to the original video and LiDAR HDF5 files. We will use these
        # to construct the path to the new camera HDF5 file.
        super().__init__()
        self.config = config
        self.base_path = Path(config.base_path)
        ...
        if index_file_path:
            ...
            self.index_file_path = self.base_path / "dataset_index.pkl"

        self.item_index = []
        self.rec_registry = {}
        
        if self.index_file_path.exists():
            print(f"Loading pre-generated index from: {self.index_file_path}")
            with open(self.index_file_path, 'rb') as f:
                loaded_data = pickle.load(f)
                self.item_index = loaded_data['item_index']
                self.rec_registry = loaded_data['rec_registry']
            print(f">>> Successfully loaded {len(self.item_index)} frames from {len(self.rec_registry)} recordings.")
        else:
            # You should ideally generate the index first, but the fallback is here.
            print(f"Index file not found at {self.index_file_path}. This will be slow.")
            self._live_index_dataset()

        if not self.item_index:
            raise ValueError("Dataset is empty.")

    def _live_index_dataset(self):
        # ... (This method is unchanged, but should ideally not be used) ...
        pass
    ...
    def __getitem__(self, idx):
        ...
        # --- WORKER-SAFE LAZY LOADING ---
        # LiDAR HDF5
        if 'lidar_h5_handle' not in rec_data:
            rec_data['lidar_h5_handle'] = h5py.File(rec_data['h5_path'], 'r')

        # Camera HDF5
        if 'camera_h5_handle' not in rec_data:
            # --- NEW: Construct path to the co-located camera HDF5 ---
            video_path = Path(rec_data['video_path'])
            camera_h5_path = video_path.parent / f"{video_path.stem}_frames.h5"
            if not camera_h5_path.exists():
                raise FileNotFoundError(f"Camera HDF5 file not found: {camera_h5_path}. Please run the extraction script.")
            rec_data['camera_h5_handle'] = h5py.File(camera_h5_path, 'r')
            
        # 1. Load Camera data from HDF5 (Fast)
        grid_frame = rec_data['camera_h5_handle']['frames'][frame_idx]
        
        # 2. Load Velodyne range image from HDF5 (Fast)
        range_image = rec_data['lidar_h5_handle']['range_images'][frame_idx]
            
        return {
            'jpg': torch.from_numpy(grid_frame),
            'velodyne_range_image': torch.from_numpy(range_image.astype(np.float32))
        }

    def __del__(self):
        for rec in self.rec_registry.values():
            if rec.get('lidar_h5_handle'):
                rec['lidar_h5_handle'].close()
            if rec.get('camera_h5_handle'):
                rec['camera_h5_handle'].close()
