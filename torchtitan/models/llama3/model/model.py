...
class Transformer(BaseModel):
        ...
        def update_from_config(
            ...
            **kwargs,
            ...
            if (
                ...
                and self.attn_config.attn_type == "varlen"
            ):
    ...
    def get_attention_masks(
        ...
        match self.config.attn_config.attn_type:
            case "flex":
                return self._get_flex_attention_masks(
                    input_batch, tokenizer, extra_inputs
                )
            case "varlen":
                if self.config.attn_mask_type != "block_causal":
                    raise ValueError(
                        f"varlen attention is only supported with block_causal \
                        attention mask type, got {self.config.attn_mask_type}"
                    )
                assert tokenizer.eos_id is not None
