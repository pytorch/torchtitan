...
class GroupedExperts(Module):
    ...
    def forward(
        ...
        w1_out_RF = maybe_remat_region(self._grouped_mm, "w1", owner=self)(
            A=x_RD.bfloat16(),
            B_t=w1_EFD.bfloat16().transpose(-2, -1),
            offs=offsets_E,
        )
        w3_out_RF = maybe_remat_region(self._grouped_mm, "w3", owner=self)(
            A=x_RD.bfloat16(),
            B_t=w3_EFD.bfloat16().transpose(-2, -1),
            offs=offsets_E,
        )
        maybe_recompute_needs(self, w1_out_RF, w3_out_RF)
        h_RF = F.silu(w1_out_RF) * w3_out_RF
        out_RD = maybe_remat_region(self._grouped_mm, "w2", owner=self)(
            A=h_RF, B_t=w2_EDF.bfloat16().transpose(-2, -1), offs=offsets_E
        )
        maybe_recompute_needs(self, out_RD)
...
class MoE(Module):
    ...
    def forward(self, x_BLD: torch.Tensor) -> torch.Tensor:
        ...
        (topk_scores_BLK, topk_expert_ids_BLK, scores_BLE) = maybe_remat_region(
            self.router, "router", owner=self
        )(x_BLD, self.expert_bias_E)
        maybe_recompute_needs(self, topk_scores_BLK, topk_expert_ids_BLK, scores_BLE)
