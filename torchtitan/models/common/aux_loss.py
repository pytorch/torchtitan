...
- Normalization: `inject` scales by `1 / per_step_denominator`, set by
  `Decoder.update_from_config` from `aggregation_level` (token: gbs*seq_len;
  sequence: gbs; batch: 1 -- not microbatch/PP-additive, see PR #3000).
...
class _AuxLossInjection(torch.autograd.Function):
    ...
    per microbatch, so AC recompute never double counts. `typecheck_forward`
    ...
    def typecheck_forward(
        ...
        out = _AuxLossInjection.apply(carrier, aux_loss, acc_value, acc_sum)
        spmd.assert_type(
            out,
...
class LoggedAuxLoss(Module):
    ...
    class Config(Module.Config):
        ...
        """Per-step normalization denominator, set by the decoder's `update_from_config`
(`-1` = unset; plain field so `build()`'s `replace` keeps it)."""
    ...
    def inject(self, raw_sum: torch.Tensor, *, carrier: torch.Tensor) -> torch.Tensor:
        ...
        if self.per_step_denominator <= 0:
            raise ValueError(
                "LoggedAuxLoss.per_step_denominator is not set: the model "
                "spec must call update_from_config (via the trainer) before "
                "building the loss."
            )
