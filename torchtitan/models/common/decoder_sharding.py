...
def rowwise_config(*, output_sp: bool = False) -> ShardingConfig:
    ...
    """
    out_tp = shard.S(1) if output_sp else shard.Inv()
    return ShardingConfig(
        state_shardings={
...
def set_gqa_attention_sharding(attention_cfg, *, enable_sp: bool) -> None:
    ...
    activation_tp = shard.S(1) if enable_sp else shard.Inv()
...
def set_gqa_inner_attention_local_map(
    ...
    """
    qkv: NamedPlacement = dense_activation_placement(tp=shard.S(2))
...
def set_decoder_sharding_config(
    ...
    *,
    ...
    """
    activation_tp = shard.S(1) if enable_sp else shard.Inv()
    logits_tp = shard.S(-1) if loss_parallel else shard.Inv()
    ...
    in_src = dense_activation_placement(tp=activation_tp)
