...
def set_gqa_inner_attention_local_map(inner_attention_cfg) -> None:
    ...
    DP/CP shard T and TP shards N. CP collectives run inside the CP kernels;
    TP collectives still run at module boundaries.

    Declares placements over the full dense SPMD axis set (DP/CP/TP) so the
    LocalMap composes under ``spmd_types``, where the surrounding mesh is
    multi-axis; ``partial_dtensor`` has a (tp,)-only mesh and consumes just the
    TP placement.
