...
<<<<<<< HEAD:torchtitan/models/common/moe/moe.py
=======
@dataclass
class MoEArgs:
    num_experts: int = 8
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if getattr(self.w1, "_partition_only", False):
            # Weights are sharded DTensors (via ColwisePartitionOnly/RowwisePartitionOnly)
            # without input/output hooks. Extract local shards and use F.linear
            # with plain tensors to avoid DTensor dispatch wrapping x as
            # DTensor(Replicate), whose backward would all-reduce d_x. d_x
            # should stay Partial and be reduced once at the MoE output boundary.
...
>>>>>>> 866352e5 (need to rebase):torchtitan/models/moe/moe.py
