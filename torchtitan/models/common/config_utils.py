...
fields set at config creation time.  Module ``__init__`` methods call
``build()`` directly — no expansion or mutation needed.
"""
...
def make_gqa_config(
    *,
    ...
    ``wqkv_param_init`` is shared for wq and wkv (wk/wv).
    ``wo_param_init`` is for the output projection.
    """
...
def make_ffn_config(
    *,
    ...
    hidden_dim: int,
    w1_param_init: dict,
    w2w3_param_init: dict,
) -> FeedForward.Config:
    """Build a fully-specified FeedForward.Config.

    ``w1_param_init`` is for w1.  ``w2w3_param_init`` is shared for w2 and w3.
    """
