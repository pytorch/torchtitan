...
class NemotronTopkRouter(nn.Module):
    ...
    def __init__(self, moe_args: MoEArgs, dim: int):
        super().__init__()
        self.top_k = moe_args.top_k
        self.n_routed_experts = moe_args.num_experts
        self.routed_scaling_factor = moe_args.route_scale
        self.n_group = moe_args.num_expert_groups or 1
        self.topk_group = moe_args.num_limited_groups or self.n_group
        self.norm_topk_prob = moe_args.route_norm
...
class NemotronMoE(MoE):
    ...
    def __init__(
        ...
        self.experts = nn.ModuleList(
            [
                NemotronMoEExpert(
                    dim=dim, hidden_dim=hidden_dim, activation=activation, bias=bias
                )
                for _ in range(moe_args.num_experts)
            ]
        )
        self.gate = NemotronTopkRouter(moe_args=moe_args, dim=dim)
