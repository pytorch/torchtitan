...
def _dist_muon_optimizer(
    ...
    *,
    ...
    def blocks_of(*num_rows_per_segment: int) -> ComputeLayout:
        """BlockShard blocks of ``sum(num_rows_per_segment)`` rows, one Muon
        matrix per segment."""
        return ComputeLayout(
            shardings_by_mesh_axis={
                dp_shard: BlockShard(dim=0, block_size=sum(num_rows_per_segment)),
            },
            num_rows_per_segment=(
                num_rows_per_segment if len(num_rows_per_segment) > 1 else None
            ),
        )
