...
def _dist_muon_optimizer(
    ...
    *,
    ...
    def blocks_of(*num_rows: int) -> ComputeLayout:
        """BlockShard blocks of ``sum(num_rows)`` rows, one Muon matrix per entry."""
        block_size = num_rows if len(num_rows) > 1 else num_rows[0]
        return ComputeLayout(
            shardings_by_mesh_axis={dp_shard: BlockShard(dim=0, block_size=block_size)},
        )
