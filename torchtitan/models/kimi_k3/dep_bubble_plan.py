...
rank's action list and is spent in order, the same walk an offline estimate of the
hideable share makes, reused here to decide placement.

The cost ratio ``r`` is in units of one text-stage forward. It is a parameter rather
than something inferred at runtime: a plan that depended on a measurement each rank
took locally would stop being identical across ranks.
"""
