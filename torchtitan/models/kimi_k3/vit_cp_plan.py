...
"""Planning for dynamic vision context parallelism (Kimi K3 report, section 5.2.3).
...
Two decisions are planned here: how one large image is partitioned along its
patch dimension across the ranks of a CP (sub)group, whose attention then
gathers keys and values across those ranks; and how the CP group is divided into
sub-groups over which several large images are distributed with balanced load.
Gathering keys over the whole CP group would make every rank exchange every large
image's keys, so the exchanged volume would grow with the group; partitioning one
image over a sub-group of 2 while another image occupies a different sub-group
keeps the exchange local and the ranks busy.
...
def row_partition(
    ...
    * **The temporal pool.** ``_temporal_pool_and_merge`` is ``sd2_tpool``: it takes
      ...
      concatenation is ``t`` times too many tokens, not the mean. Keeping all
      frames on every rank makes each rank's temporal mean the true one for its
      rows.
...
def subgroup_layout(num_large: int, cp_size: int) -> tuple[int, int]:
    ...
    One large image and a CP group of 8 gives (1, 8): the image is partitioned
    across all eight ranks. Four large images and 8 ranks gives (4, 2), so each
    image is exchanged inside a pair instead of across all eight.
...
def balance_images(sizes: list[int], num_groups: int) -> list[int]:
    ...
    against 20, while LPT gives 100 against 30.
    """
    if num_groups <= 1:
        return [0] * len(sizes)
    load = [0] * num_groups
