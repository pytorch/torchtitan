...
`DeepSeekV3Model` inherits from `MTPDecoder`. If `num_mtp_layers` is zero,
`MTPDecoder.forward()` falls back to the normal `Decoder.forward()` path,
preserving non-MTP behavior. Under PP, `self.mtp_layers` contains only the MTP
layers owned by the current virtual stage, so it can be empty while the full
model still has MTP enabled.

When MTP is enabled, forward has two parts:

1. The first or unsplit stage embeds the input token IDs, and each stage runs its local main decoder layers to produce the main hidden state.
2. The final or unsplit stage derives each shifted token input from the original source token IDs, then runs an `MTPTransformerBlock` on top of the previous hidden state to produce an auxiliary hidden state or logits.

The dataloader does not need to extend the input sequence for MTP. The main module still consumes the original token sequence provided by the trainer. In an unsplit model, MTP inputs are derived from `tokens`. On the final PP stage, they are derived from `mtp_source_tokens`, while `tokens` contains hidden activations received from the preceding stage.
...
`num_mtp_layers` remains a model-registry construction argument rather than an
`MTPDecoder.Config` field. `MTPDecoder` stores the effective count from
`len(config.mtp_layers)` so every pipeline stage retains whether the complete
model has MTP enabled, even when that stage owns no local MTP layers.
...
The current implementation rejects CP when MTP is enabled. Standard Trainer
eager PP is supported through a DeepSeek-V3-specific pipeline builder and the
model-owned pipeline runtime described below. GraphPP remains unsupported.
...
each shifted position still maps to a valid token. Invalid shifted token IDs
contain `fill_value`; the MTP block uses the mask to zero the corresponding
previous hidden states, and MTP loss uses `IGNORE_INDEX` for the corresponding
labels.
...
The model uses this helper to build shifted input tokens and validity masks.
The loss shifts labels within the sequence and applies the model-provided mask,
using `IGNORE_INDEX` for invalid targets.
...
### 3.4 Eager Pipeline Parallelism

`pipeline_deepseek_v3()` places all MTP layers, the final norm, the LM head,
and an embedding replica on the final virtual stage. The first virtual stage
keeps the canonical token embedding. Every PP rank already reads its local
microbatch, so the final stage receives the original token IDs as a local
keyword input; PP edges continue to carry only hidden activations.

`SharedParameterPipelineRuntime` synchronizes the embedding replica after
initialization and checkpoint load, sums the canonical and replica gradients
before optimization, and excludes the replica from the logical gradient norm.

## 4. MTP Loss Handling

### 4.1 Input Form

`MTPDecoder` returns a flat tuple containing the main prediction, one auxiliary
prediction per MTP depth, and one validity mask per MTP depth. Predictions are
logits on the ordinary path and hidden states when `_skip_lm_head=True` for
`ChunkedLossWrapper`. The model builds the masks from `positions`, so the loss
does not need stage-local model inputs to identify packed-document boundaries.
A plain tensor is intentionally rejected by `MTPLoss`.
...
```python
main_loss = cross_entropy(pred[0], labels[: pred[0].shape[0]])
```
...
auxiliary prediction with model-masked, depth-shifted labels and its weight
before sequence chunking.
...
### 2. GraphPP Integration

The runtime contract is integrated with Standard Trainer eager PP. GraphPP
still needs equivalent support for stage-local root inputs and shared-parameter
lifecycle hooks.
