...
MTP supports CP by constructing shifted inputs before CP sharding. Standard Trainer
...
The function can optionally return `valid_mask`. The mask indicates whether each shifted position still maps to a valid token. At invalid positions, the MTP block masks the previous hidden-state branch, while MTP loss ignores the corresponding labels through `IGNORE_INDEX`.
...
### 3.5 Eager Pipeline Parallelism
