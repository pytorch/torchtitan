...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        ) = model_args.get_nparams_and_flops(model, job_config.training.seq_len)  # type: ignore
        ...
        if parallel_dims.pp_enabled:
            ...
            for m in self.model_parts:
                ...
                with torch.no_grad():
                    ...
                    m.init_weights(buffer_device=buffer_device)  # type: ignore
                m.train()

            # confirm that user will be able to view loss metrics on the console
            # pyrefly: ignore [bad-argument-type]
            ensure_pp_loss_visible(parallel_dims, job_config, color)  # type: ignore
        else:
            ...
            with torch.no_grad():
                # pyrefly: ignore [not-callable]
                model.init_weights(buffer_device=buffer_device)  # type: ignore
            model.train()
    ...
    def train_step(
        ...
        for _microbatch in range(self.gradient_accumulation_steps):
            ...
            input_dict, labels = next(data_iterator)  # type: ignore
        ...
        if parallel_dims.dp_cp_enabled:
            ...
            global_avg_loss, global_max_loss, global_ntokens_seen = (
                dist_utils.dist_mean(loss, loss_mesh),
    ...
    def train(self):
        ...
        with (
            ...
            # pyrefly: ignore [bad-argument-type]
            data_iterator = self.batch_generator(self.dataloader)  # type: ignore
            while self.should_continue_training():
                ...
                if self.job_config.validation.enable and self.validator.should_validate(
                    ...
                    with self.loss_fn.no_rescale():  # type: ignore
                        # pyrefly: ignore [bad-argument-count]
                        self.validator.validate(self.model_parts, self.step)  # type: ignore
