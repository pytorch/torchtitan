...
`torchtitan/rl/generate.py --benchmark`. Append-only running log +
full per-rung numbers live in `torchtitan/rl/docs/inference_gap_ablation.md`.
...
`git checkout ablation-inference -- torchtitan/rl/generate.py` + the
patch modules (`models/{kernel_ablation,qwen3_vllm_2d,helion_rope,vllm_fused_ops}.py`),
fix the import (`from torchtitan.rl.examples.alphabet_sort import
...
conda `titan-rl` (see `torchtitan/rl/README.md` to build it).
