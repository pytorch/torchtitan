...
def clean_name(name: str, prefixes: tuple[str, ...] = ("_orig_mod",)) -> str:
    """Strip any matching segment from a dotted name path.

    Handles wrapper prefixes that can appear at any level of the module
    hierarchy, e.g. 'layers.0._orig_mod.attention.wq.weight' or
    '_orig_mod.layers.0.weight'.
    """
    parts = name.split(".")
...
def clean_model_state_dict(
    model: nn.Module,
    prefixes: tuple[str, ...] = ("_orig_mod",),
    **kwargs: Any,
) -> dict[str, Any]:
    """Get model state dict with wrapper prefixes stripped from keys.

    Calls model.state_dict(**kwargs) and applies clean_name to every key.
    """
    return {clean_name(k, prefixes): v for k, v in model.state_dict(**kwargs).items()}


def clean_model_load_state_dict(
    model: nn.Module,
    state_dict: dict[str, Any],
    prefixes: tuple[str, ...] = ("_orig_mod",),
    **kwargs: Any,
) -> None:
    """Load a clean-keyed state dict into a model that may have wrapper prefixes.
    ...
    for key in model.state_dict(keep_vars=True):
        clean_to_orig[clean_name(key, prefixes)] = key
