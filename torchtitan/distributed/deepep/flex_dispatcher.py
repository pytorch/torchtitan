...
class DeepEPDispatch(torch.autograd.Function):
    ...
    def backward(ctx, grad_recv_x, grad_recv_topk_idx, grad_recv_topk_weights, grad_num_recv, grad_handle):
        ...
        if grad_recv_x is not None:
            grad_x_combined, _, _ = ctx.buffer.combine(
        ...
        if grad_recv_topk_weights is not None:
            grad_recv_topk_weights_padded = torch.zeros(
                grad_recv_topk_weights.shape[0], ctx.hidden_dim,
                dtype=torch.bfloat16, device=grad_recv_topk_weights.device
            )
            grad_recv_topk_weights_padded[:, :ctx.top_k] = grad_recv_topk_weights.to(torch.bfloat16)
            grad_topk_weights_combined, _, _ = ctx.buffer.combine(
                x=grad_recv_topk_weights_padded, handle=ctx.handle,
                async_finish=False, allocate_on_comm_stream=False
            )
            grad_topk_weights = grad_topk_weights_combined[:, :ctx.top_k].to(ctx.input_dtype)
