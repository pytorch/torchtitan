...
from torchtitan.models.moe.utils import _permute as moe_permute, _unpermute as moe_unpermute
...
def _dispatch_op_impl(
    ...
    recv_x, recv_indices, recv_scores, num_recv_list, handle, after_event = \
        buffer.dispatch(
            x=x,
            topk_idx=topk_idx,
...
def _dispatch_backward(ctx, grad_recv_x, grad_recv_indices, grad_recv_scores, 
                       grad_num_recv, grad_cache_id):
...
def _indices_to_multihot(
    ...
    multihot_routing_map = torch.zeros((batch_size, num_local_experts), dtype=torch.long, device=indices.device)
    multihot_probs = torch.zeros((batch_size, num_local_experts), dtype=probs.dtype, device=indices.device)
...
def _permute_tokens(
    ...
    if probs is not None:
        sorted_probs = probs.T.contiguous().masked_select(routing_map_t)
    else:
        sorted_probs = None
    
    return sorted_tokens, sorted_probs, sorted_indices
...
def _unpermute_tokens(
    ...
    output_tokens = torch.zeros((num_tokens, hidden), dtype=permuted_tokens.dtype, device=permuted_tokens.device)
...
def dispatch_tokens(
    ...
    Args:
        ...
        use_alignment_padding: If True, add alignment padding for torch._grouped_mm.
            Recommended for large models (671B+) where padding overhead is amortized.
    
    Returns:
        (permuted_tokens, tokens_per_expert, state_for_combine)
    """
    # Ensure contiguous and proper shape
    router_topk = selected_experts_indices.shape[1] if selected_experts_indices.dim() == 2 else 1
    ...
    hidden_states, dispatched_indices, dispatched_scores, tokens_per_expert, cache_id = \
    ...
    hidden_states, permuted_scores, sorted_indices = _permute_tokens(
        hidden_states, dispatched_routing_map, probs=dispatched_probs_multihot
    )
    ...
    state = DispatchState(
        ...
        padding_input_shape=padding_input_shape,
...
def combine_tokens(
    ...
    if state.padding_indices is not None:
        hidden_states = moe_unpermute(hidden_states, state.padding_input_shape, state.padding_indices)
    
    hidden_states = _unpermute_tokens(hidden_states, state.sorted_indices, state.num_recv_tokens)
