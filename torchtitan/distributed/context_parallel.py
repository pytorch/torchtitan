...
class HeadTailCPLoadBalancer(ContextParallelLoadBalancer):
    ...
    def __init__(
        ...
        *,
        seq_len: int,
        context_metadata: AttentionMasksType | None,
    ) -> None:
        del config, context_metadata
        ...
        if isinstance(context_metadata, Mapping):
            if mask_key is None:
                raise ValueError(
                    ...
                    "to one of: "
                    f"{sorted(context_metadata.keys())}"
                )
            if mask_key not in context_metadata:
                raise ValueError(
                    f"PTRR mask key '{mask_key}' is not a key in context metadata. "
                    f"Available keys: {sorted(context_metadata.keys())}"
                )
            block_mask = context_metadata[mask_key]
        else:
            block_mask = context_metadata
