...
{''.join(copy_blocks)}
"""
    filename = f"<torchtitan_flexep_copy_full_counts_to_peers_ep{ep_size}>"
...
"""
    filename = f"<torchtitan_flexep_copy_rows_to_peers_ep{ep_size}>"
    lines = source.splitlines(keepends=True)
    linecache.cache[filename] = (len(source), None, lines, filename)
    namespace = {
        "triton": triton,
...
def _make_copy_rows_to_peer_ptrs_kernel(dtype: torch.dtype):
    ...
    if dtype_name is None:
        raise ValueError(f"Unsupported FlexEP row-copy dtype: {dtype}.")
...
def _copy_rows_to_peer_ptrs_kernel_{str(dtype).replace('.', '_')}(
    ...
    dst_byte_offset = (dst_row[:, None] * DST_ROW_STRIDE + col[None, :]) * {element_size}
    dst_ptr = (dst_base[:, None] + dst_byte_offset).to(tl.pointer_type({dtype_name}))
    tl.store(dst_ptr, values, mask=mask & (dst_rank[:, None] >= 0))
"""
    filename = f"<torchtitan_flexep_copy_rows_to_peer_ptrs_{dtype}>"
    lines = source.splitlines(keepends=True)
    linecache.cache[filename] = (len(source), None, lines, filename)
    namespace = {
        "triton": triton,
    ...
    *,
    ...
    if dst_ptrs is not None:
        ...
        if dst_ptrs.device != counts.device:
            raise ValueError("copy_full_counts_to_peers dst_ptrs must match counts device.")
