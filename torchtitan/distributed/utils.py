...
def maybe_enable_amp(
    ...
    if parallel_dims.fsdp_enabled:
        # FSDP handles mixed precision internally via MixedPrecisionPolicy
        logger.info("Mixed precision training is handled by fully_shard")
        return contextlib.nullcontext()
    else:
        # Enable autocast for non-FSDP cases (DDP, TP-only, single-device)
        # Note: PP uses its own schedule-based execution and doesn't use this context
        # torch.autocast works at the operator level and is orthogonal to parallelism strategy
