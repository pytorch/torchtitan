...
class FusedSiLUGate(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        _fused_silu_gate_bwd_kernel[grid](
            ...
            grad_x1, grad_x3,
            num_tokens, hidden_size,
            grad_output.stride(0), grad_x1.stride(0),
            BLOCK_SIZE,
        )
...
def _imbalance_ratio_kernel(
    ...
    for g in tl.static_range(EP_SIZE):
        ...
        g_counts = tl.load(counts_ptr + start + g_offs, mask=g_mask, other=0).to(tl.float32)
...
def triton_imbalance_ratio(
    ...
    counts = expert_counts[:num_experts] if expert_counts.size(0) > num_experts else expert_counts
...
def triton_assign_tokens(
    ...
    _assign_tokens_kernel[grid](
        sort_perm, sorted_experts,
        local_offsets, global_offsets,
        plan_gpu_ids, plan_starts, plan_ends, plan_count,
        target_gpus,
        N, num_local_experts,
        MAX_ASSIGN=MA_CONST,
