...
class CPVarlenMetadata(VarlenMetadata):
    ...
    ``cu_seq_k[-1]``. :meth:`VarlenInnerAttention.forward` detects this subclass via
...
# Same reason as the base class: keep the fields visible to pytree so CUDA
# graph input handling does not treat the metadata as one opaque leaf.
pytree.register_dataclass(CPVarlenMetadata)
