...
class ContextParallelLoadBalancer(Configurable, ABC):
    ...
    Subclasses provide the partition used to shard one batch.
    ...
    - ``None`` for contiguous CP sharding without load balancing.
    ...
    @staticmethod
    def from_config(
        config: ContextParallelLoadBalancerConfig | None,
        *,
        input_shardings: dict[str, SpmdType] | None,
        cp_mesh: DeviceMesh,
    ) -> "ContextParallelLoadBalancer":
        """Construct a load balancer, defaulting to contiguous sharding."""
        if config is None:
            config = _ContiguousLoadBalancer.Config()
        return config.build(
    ...
    def shard_inputs(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        self._load_balancer_impl = self._build_load_balancer_impl(input_dict=input_dict)
...
class _ContiguousLoadBalancer(ContextParallelLoadBalancer):
    """Shard contiguous sequence fragments without token reordering."""
