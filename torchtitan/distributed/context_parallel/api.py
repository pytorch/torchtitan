...
class ContextParallelLoadBalancer(Configurable):
    ...
    Built-in strategies, selected by ``context_parallel_load_balancer``, are:

    - ``"headtail"``: use ``HeadTailLoadBalancer`` for SDPA-based CP.
    - ``"ptrr"``: use ``PTRRLoadBalancer`` for FlexAttention-based CP.
    - ``"none"``: use contiguous CP sharding without load balancing.
