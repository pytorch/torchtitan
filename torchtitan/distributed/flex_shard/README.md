...
- `build_dist_muon` consumes optimizer-agnostic per-parameter `ComputeLayout`
  ...
  parameter can use `Shard(0)` to distribute complete matrices or `Owned` for
  whole-batch compute. A single 2D matrix without `BlockShard` uses
  whole-matrix compute such as `Owned`. The builder validates named DTensor
