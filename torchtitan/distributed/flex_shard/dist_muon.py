...
class DistMuon(Optimizer):
    ...
    ``[M, R, C]`` parameter uses ``Shard(0)`` to distribute complete matrices,
    or ``Owned`` to assign the complete batch to one rank. Replicated storage
    and storage shards along either matrix dimension can redistribute to
    ``Shard(0)`` compute on one mesh axis. A single 2D matrix without
    ``BlockShard`` uses whole-matrix compute such as ``Owned``.
