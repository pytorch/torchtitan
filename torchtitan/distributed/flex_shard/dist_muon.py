...
from ._optimizer_reshard_schedule import (
    ...
    _device_mesh_ranks,
    _dtensor_global_runs_by_dim,
    _dtensor_storage_regions,
    _DTensorStorageDomain,
    _mesh_coordinate_for_participant,
    ...
    for mesh_axis_size, placement in zip(
        ...
        if type(placement) not in (Shard, _StridedShard):
            raise ValueError(
                ...
                "requires Shard, _StridedShard, or Replicate placements"
            )
        sharded_placement = cast(Shard | _StridedShard, placement)
        if sharded_placement.dim % param.ndim != 0:
            raise ValueError(
                f"Muon parameter {fqn!r} with matrix-batch storage "
...
def _merge_intervals(
    ...
    for start, end in sorted(
        interval for interval in intervals if interval[0] < interval[1]
    ):
...
def _ordered_block_shard_row_intervals(
    param: DTensor,
    participants: Sequence[int],
    ordered_storage_mesh_axes: Sequence[int],
    *,
    matrix_rows: int,
) -> dict[int, tuple[tuple[int, int], ...]]:
    """Return each participant's target global row interval."""
    num_matrices = param.shape[0] // matrix_rows
    intervals: dict[int, tuple[tuple[int, int], ...]] = {}
    for participant in participants:
        mesh_coordinate = _mesh_coordinate_for_participant(
            param.device_mesh,
            participant,
        )
        matrix_offset = 0
        local_num_matrices = num_matrices
        for storage_mesh_axis in ordered_storage_mesh_axes:
            local_num_matrices, local_offset = Shard.local_shard_size_and_offset(
                local_num_matrices,
                param.device_mesh.size(storage_mesh_axis),
                mesh_coordinate[storage_mesh_axis],
            )
            matrix_offset += local_offset
        intervals[participant] = (
            (
                matrix_offset * matrix_rows,
                (matrix_offset + local_num_matrices) * matrix_rows,
            ),
        )
    return intervals


def _participant_groups_for_transport_axes(
    param: DTensor,
    participants: Sequence[int],
    transport_storage_mesh_axes: Sequence[int],
) -> tuple[tuple[int, ...], ...]:
    """Group participants that differ only on the transport mesh axes."""
    fixed_storage_mesh_axes = tuple(
        storage_mesh_axis
        for storage_mesh_axis in range(param.device_mesh.ndim)
        if storage_mesh_axis not in transport_storage_mesh_axes
    )
    ...
    for participant in participants:
        ...
        group_key = tuple(
            mesh_coordinate[storage_mesh_axis]
            for storage_mesh_axis in fixed_storage_mesh_axes
        )
        groups.setdefault(group_key, []).append(participant)
    return tuple(tuple(group) for group in groups.values())


def _ordered_block_shard_transport_mesh_axes(
    fqn: str,
    param: DTensor,
    ordered_storage_mesh_axes: tuple[int, ...],
    *,
    matrix_rows: int,
) -> tuple[int, ...]:
    """Select the innermost axis when ordered ownership needs transport.

    Holding outer coordinates fixed preserves their matrix ownership. The
    innermost axis can redistribute within each outer partition. Aligned
    storage requires no transport.
    """
    participants = _device_mesh_ranks(param.device_mesh)
    storage_intervals = {
        participant: tuple(
            (global_offset, global_offset + size)
            for global_offset, _local_offset, size in _dtensor_global_runs_by_dim(
                param,
                participant,
            )[0]
        )
    ...
    compute_intervals = _ordered_block_shard_row_intervals(
        param,
        participants,
        ordered_storage_mesh_axes,
        matrix_rows=matrix_rows,
    )

    if all(
        _merge_intervals(storage_intervals[participant])
        == _merge_intervals(compute_intervals[participant])
        for participant in participants
    ):
        return ()

    transport_mesh_axes = ordered_storage_mesh_axes[-1:]
    groups = _participant_groups_for_transport_axes(
        param,
        participants,
        transport_mesh_axes,
    )
    if all(
        _merge_intervals(
            ...
            for interval in storage_intervals[participant]
        )
        == _merge_intervals(
            interval
            for participant in group
            for interval in compute_intervals[participant]
        )
        for group in groups
    ):
        return tuple(sorted(transport_mesh_axes))

    raise NotImplementedError(
        f"Muon parameter {fqn!r} ordered BlockShard requires storage to "
        "preserve the outer matrix partitions; omit shard_order_by_tensor_dim "
        "to redistribute over all BlockShard axes"
    )


def _resolve_sharded_transport_mesh_axes(
    fqn: str,
    param: DTensor,
    ...
    ordered_block_shard_storage_mesh_axes: tuple[int, ...],
) -> tuple[int, ...]:
    """Return storage mesh axes that require sharded compute transport."""
    if compute_view is None:
        ...
        return tuple(changed_storage_mesh_axes)
    ...
    for storage_mesh_axis, placement in enumerate(param.placements):
        if storage_mesh_axis in active_block_shard_mesh_axes:
            continue
    ...
    if nonreplicated_other_mesh_axes:
        if not active_block_shard_mesh_axes:
            raise NotImplementedError(
        ...
        raise NotImplementedError(
            f"Muon parameter {fqn!r} matrix-batch compute requires every "
            "non-replicated storage mesh axis to use BlockShard; "
            f"non-replicated axes: {nonreplicated_other_mesh_axes}"
        )

    if ordered_block_shard_storage_mesh_axes:
        if set(ordered_block_shard_storage_mesh_axes) != set(
            active_block_shard_mesh_axes
        ):
            raise ValueError(
                f"Muon parameter {fqn!r} must order every active BlockShard "
                "mesh axis"
            )
        return _ordered_block_shard_transport_mesh_axes(
            fqn,
            param,
            ordered_block_shard_storage_mesh_axes,
            matrix_rows=compute_view.matrix_rows,
        )

    if len(active_block_shard_mesh_axes) != 1:
        return active_block_shard_mesh_axes

    storage_mesh_axis = active_block_shard_mesh_axes[0]
    storage_sharding = cast(
        Replicate | Shard | _StridedShard,
    ...
    compute_sharding = target_sharding_by_storage_mesh_axis[storage_mesh_axis]
    assert type(compute_sharding) is BlockShard
    if type(storage_sharding) is _StridedShard:
        return (storage_mesh_axis,)
    ...
    return () if storage_intervals == compute_intervals else (storage_mesh_axis,)
...
# Lowered: ordered ``Shard`` entries have become DTensor placements, so an axis
# applied later than storage-mesh order is now ``_StridedShard``. Ordered
# ``BlockShard`` entries stay explicit for matrix-level planning.
...
def _build_parameter_redistribution_plan(
    ...
    storage_domain = _dtensor_storage_regions(
    ...
    if type(compute_sharding) is Owned:
        ...
        return _build_owned_redistribution_plan(
            ...
            logical_shape=storage_domain.logical_shape,
        )
    ...
    assert compute_layout.compute_view is not None
    storage_shape = torch.Size(storage_domain.logical_shape)
    return _build_batched_matrix_redistribution_plan(
        ...
        compute_shape=tuple(
            compute_layout.compute_view.matrix_batch_shape(storage_shape)
        ),
    )


def _build_batched_matrix_redistribution_plan(
    storage_domain: _DTensorStorageDomain,
    *,
    participants: tuple[int, ...],
    ...
    storage_by_participant: dict[int, list[_StorageRegionMapping]] = {
        participant: [] for participant in participants
    }
    for mapping in storage_domain.mappings:
        holders = mapping.holders
        logical_region = mapping.logical_region
        _require_valid_plan(
            bool(holders)
        ...
        for participant in holders:
            _require_valid_plan(
                participant in storage_by_participant,
                "matrix-batch storage holder must be a participant",
            )
            storage_by_participant[participant].append(mapping)

    _require_valid_plan(
        all(
            mappings or not math.prod(storage_domain.tensor_shape(participant))
            for participant, mappings in storage_by_participant.items()
        ),
    ...
    storage_partitions = tuple(
        _ParticipantPartition(
            ...
            tensor_shape=storage_domain.tensor_shape(participant),
            logical_regions=tuple(
                mapping.logical_region
                for mapping in storage_by_participant[participant]
            ),
        )
        for participant in participants
    )
    ...
    for storage_mapping in storage_domain.mappings:
        source_holders = storage_mapping.holders
        storage_region = storage_mapping.logical_region
        storage_tensor_base = storage_mapping.tensor_region
...
def _lower_shard_order_to_strided_shards(
    ...
    DTensor applies same-dimension ``Shard`` axes in storage-mesh order, so an
    axis applied later than that default becomes ``_StridedShard``. Ordered
    ``BlockShard`` remains explicit for matrix-level planning. Axes declared
    for other mesh variants are absent here and drop out of the order.
    """
    ...
    for tensor_dim, axis_names in compute_layout.shard_order_by_tensor_dim.items():
        ...
        for order_index, storage_mesh_axis in enumerate(ordered_mesh_axes):
            ...
            if split_factor > 1 and type(lowered_shardings[storage_mesh_axis]) is Shard:
...
def _resolve_storage_to_compute_transition(
    ...
    ordered_block_shard_storage_mesh_axes = tuple(
        storage_axis_by_name[axis_name]
        for axis_name in compute_layout.shard_order_by_tensor_dim.get(0, ())
        if axis_name in storage_axis_by_name
        and type(
        ...
        is BlockShard
        and param.device_mesh.size(storage_axis_by_name[axis_name]) > 1
    )
    applicable_compute_shardings_by_storage_mesh_axis = (
    ...
    sharded_transport_mesh_axes = _resolve_sharded_transport_mesh_axes(
        fqn,
        param,
        compute_view,
        normalized_target_sharding_by_storage_mesh_axis,
    ...
    transport_mesh_axes = tuple(
        sorted(set(sharded_transport_mesh_axes).union(active_owned_storage_mesh_axes))
    )
    if len(transport_mesh_axes) == 1 and compute_view is None:
        transport_mesh_axis = transport_mesh_axes[0]
        source_placement = param.placements[transport_mesh_axis]
        transport_target_sharding = normalized_target_sharding_by_storage_mesh_axis.get(
            transport_mesh_axis
        )
        for preserved_mesh_axis, preserved_placement in enumerate(param.placements):
            if preserved_mesh_axis == transport_mesh_axis:
                continue
            if (
                type(source_placement) is Shard
                and source_placement.dim % param.ndim == 1
                and type(transport_target_sharding) is Shard
                and transport_target_sharding.dim == 0
                and type(preserved_placement) is Shard
                and preserved_placement.dim % param.ndim == 0
                and transport_mesh_axis < preserved_mesh_axis
            ):
                preserved_axis_name = mesh_axis_names[preserved_mesh_axis]
                transport_axis_name = mesh_axis_names[transport_mesh_axis]
                raise ValueError(
                    f"Muon parameter {fqn!r} must declare "
                    "shard_order_by_tensor_dim={0: "
                    f"({preserved_axis_name!r}, {transport_axis_name!r})}} "
                    f"when preserving Shard(0) on mesh axis "
    ...
    for storage_mesh_axis, placement in enumerate(param.placements):
        ...
        if type(source_sharding) is _UnsupportedStoragePlacement:
            raise NotImplementedError(
                f"Muon parameter {fqn!r} cannot redistribute storage "
                f"placement {source_sharding.representation} on mesh axis "
                f"{mesh_axis_names[storage_mesh_axis]!r}"
            )
        if storage_mesh_axis in transport_axis_set:
            continue
        target_sharding = normalized_target_sharding_by_storage_mesh_axis.get(
            storage_mesh_axis,
            source_sharding,
        )
        if compute_view is not None and type(target_sharding) is BlockShard:
            continue
        if target_sharding != source_sharding:
            raise RuntimeError(
                f"Muon parameter {fqn!r} changed mesh axis "
                f"{mesh_axis_names[storage_mesh_axis]!r} outside its "
                "optimizer transport group"
            )
    ...
    if active_owned_storage_mesh_axes:
        nonreplicated_axes = [
            axis_name
            for storage_mesh_axis, (axis_name, target_sharding) in enumerate(
                resolved_target_signature
            )
            if storage_mesh_axis not in owned_axis_set
            and type(target_sharding) is not Replicate
        ]
        if nonreplicated_axes:
            raise ValueError(
                f"Muon owned compute for parameter {fqn!r} requires every "
                "non-owned mesh axis to be replicated; non-replicated axes: "
                f"{nonreplicated_axes}"
            )
