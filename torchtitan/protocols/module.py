...
from torchtitan.distributed.spmd_state import mesh as spmd_mesh, spmd_state
...
def named_placement_to_spmd(named):
    ...
    Size-1 axes are kept — spmd_types auto-drops them.
    """
    m = spmd_mesh()
    types = {}
    partition_spec = None
...
class Module(nn.Module, Configurable):
    ...
    def distribute_state(
        ...
        *,
        ...
        """
        if self._sharding_config.use_spmd:
    ...
    def distribute_state_spmd(
        ...
        *,
        ...
        # assert type
        types, partition_spec = named_placement_to_spmd(named_placement)
        if types:
            dp_axes_set = set(state.dp_axes)
            types = {
                a: (spmd.R if t is spmd.I and a in dp_axes_set else t)
                for a, t in types.items()
            }
            spmd.assert_type(tensor, types, partition_spec=partition_spec)
    ...
    def _shard_outputs_spmd(self, outputs: Any) -> Any:
        sharding_config = self._sharding_config
        assert sharding_config is not None
        assert sharding_config.out_src_shardings is not None, (
            "SPMD path requires out_src_shardings for output redistribution"
        )
        if isinstance(outputs, torch.Tensor):
            src_types, _ = named_placement_to_spmd(sharding_config.out_src_shardings)
            dst_types, _ = named_placement_to_spmd(sharding_config.out_dst_shardings)
