...
class MetricsProcessor(Configurable):
    ...
    def log(
        ...
        metrics = {
            ...
            "memory/host_rss(GiB)": host_stats.rss_gib,
            "memory/host_max_rss(GiB)": host_stats.max_rss_gib,
            "cpu/utilization(%)": host_stats.cpu_utilization_pct,
        }
        if mfu is not None:
            metrics["mfu(%)"] = mfu
        ...
        logger.info(
            ...
            f"{color.yellow}host: {host_stats.rss_gib:5.2f}GiB "
            f"cpu: {host_stats.cpu_utilization_pct:.0f}%  "
