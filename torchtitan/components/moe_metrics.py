...
class TensorBoardMoESink:
    ...
    def _log_to_tensorboard(self, cube, steps, layers, num_local_experts) -> None:
        ...
        try:
            ...
            writer.flush()
            # Expert slices render only two heatmaps (first/last row) whereas the
            # layer slices render one per MoE layer (26 for DeepSeek-V3-16B), so
            # emit + flush the cheap section first. Ordering it after the layer
            # slices meant a mid-render SIGTERM dropped `moe_layer_load`
            # entirely: observed on runs 20260810_162236 and 20260813_232009,
            # where a peer rank hit SIGSEGV during teardown and rank 0 was killed
            # partway through the per-layer heatmaps.
...
def maybe_record_grouped_gemm(
    *,
    ...
    Called from the shared ``RoutedExperts.forward`` path, so it covers every
    ...
    graph op. When no collector is installed (the default),
