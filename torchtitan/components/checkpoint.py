...
class CheckpointManager(Configurable):
    ...
    def save_for_external_eval(self, curr_step: int) -> str:
        ...
        if filesystem.isdir(checkpoint_id) and filesystem.isfile(
            ...
            logger.info("Reusing existing external eval checkpoint: %s", checkpoint_id)
    ...
    @sl.log_trace_span("checkpoint_average_save")
    @torch.no_grad()
    def save_averaged_model_checkpoint(
        self,
        *,
        ...
        """Weighted-average model weights from checkpoints, save model-only DCP.

        Checkpoint ``source_steps[i]`` is weighted ``decay ** age`` where ``age``
        counts backwards from ``curr_step``, then the result is normalized. This
        is one two-parameter family:

        * ``decay == 1.0`` -> uniform "model soup", every checkpoint gets 1/N.
        * ``decay < 1.0`` -> exponentially weighted average, truncated to the N
          checkpoints still on disk. It approaches a true EMA as N grows, but
          note that N is bounded by ``checkpoint.keep_latest_k``, so a small
          decay with a small N drops a large share of the tail mass. The
          retained mass is ``1 - decay ** N``.

        Unlike a stateful on-disk EMA this stays a pure function of the source
        checkpoints, so it is idempotent, recomputable, and safe to re-run after
        a job restart.

        Checkpoints are loaded one at a time into the live model. Floating-point
        local shards are accumulated on CPU in float32, while non-floating state
        is taken from the newest checkpoint. The current checkpoint is restored
        before returning, so optimizer and training state remain untouched.
        """
        ...
        source_checkpoint_ids = [
            self.get_checkpoint_id(source_step) for source_step in source_steps
        ]
        for source_step, checkpoint_id in zip(
            source_steps, source_checkpoint_ids, strict=True
        ):
            if not filesystem.isfile(filesystem.join(checkpoint_id, ".metadata")):
                raise FileNotFoundError(
                    f"Checkpoint step {source_step} required for averaging was not "
                    f"found at {checkpoint_id}. Increase checkpoint.keep_latest_k "
                    "or align checkpoint.interval with the merge interval."
                )

        # Weight source_steps[i] by decay ** age, with age counting backwards
        # from curr_step, then normalize by the sum. decay == 1.0 reproduces the
        # uniform soup bit for bit.
        num_sources = len(source_steps)
        weights = [decay ** (num_sources - 1 - index) for index in range(num_sources)]
        weight_normalizer = sum(weights)

        # Keep the legacy "avg-N" name for the uniform case so existing merged
        # checkpoints stay discoverable, and give each decay its own tree so a
        # sweep over decay values does not collide.
        merge_tag = (
            f"avg-{num_sources}" if decay == 1.0 else f"ema-{num_sources}-{decay:g}"
        )
        output_folder = filesystem.join(
            filesystem.join(self.folder, "external_eval_merged"),
            merge_tag,
        )
        checkpoint_id = self.get_checkpoint_id(curr_step, folder=output_folder)
        if filesystem.isfile(filesystem.join(checkpoint_id, ".metadata")):
            logger.info("Reusing averaged checkpoint: %s", checkpoint_id)
            return checkpoint_id

        model = cast(ModelWrapper, self.states[MODEL])
        normalized_weights = [weight / weight_normalizer for weight in weights]

        logger.info(
            "Averaging model checkpoints at steps %s for external eval "
            "(decay=%s, normalized weights=%s).",
            source_steps,
            decay,
    ...
    def _write_weighted_model_merge(
        ...
        *,
        ...
        """Accumulate pre-normalized weighted model checkpoints into model-only DCP.

        Sources are loaded into the live model one at a time and their
        floating-point local shards are accumulated on CPU in float32, scaled by
        the accompanying weight. ``weighted_sources`` must already sum to 1.0 --
        callers own the weighting policy. Non-floating state (integer counters,
        for example) is inherited from the last source loaded.

        The live model is clobbered during accumulation and restored from
        ``restore_checkpoint_id`` before returning, so optimizer and training
        ...
        try:
            ...
            logger.info(
                "Restoring live model from %s after merge.", restore_checkpoint_id
            )
            current_state = model.state_dict()
            dcp.load(current_state, checkpoint_id=restore_checkpoint_id)
            model.load_state_dict(current_state)

    @sl.log_trace_span("checkpoint_ema_save")
    @torch.no_grad()
    def save_ema_model_checkpoint(
        self,
        *,
        new_steps: list[int],
        curr_step: int,
        ...
        """Fold new checkpoints into an on-disk EMA and save it as model-only DCP.

        Implements ``W_ema <- decay * W_ema + (1 - decay) * avg(new_steps)``,
        where ``W_ema`` is read back from the merged checkpoint written at
        ``prev_ema_step``. Because the running average lives on disk rather than
        in memory, this needs no persistent shadow copy of the model and no
        matching DTensor placements -- unlike an online EMA.

        ``new_steps`` should be the regular checkpoints written since the
        previous merge so that each one is folded in exactly once, which makes
        the effective per-merge decay exactly ``decay``.

        When ``prev_ema_step`` is None (first merge) or its checkpoint is
        missing (chain break), this bootstraps from the plain average of
        ``new_steps``. Initializing from the first observation rather than zeros
        avoids the usual EMA warmup bias, so no bias correction is applied.

        Merged outputs are written per step and are never touched by
        ``keep_latest_k`` purging, so re-running a merge is idempotent: the
        existing output is reused.
        """

        if not self.enable:
            raise ValueError("Cannot save an EMA checkpoint when checkpointing is disabled.")
        if self.load_only:
            raise ValueError("Cannot save an EMA checkpoint in load_only mode.")
        if not new_steps:
        ...
        new_checkpoint_ids = [self.get_checkpoint_id(step) for step in new_steps]
        for step, source_checkpoint_id in zip(
            new_steps, new_checkpoint_ids, strict=True
        ):
            if not filesystem.isfile(
                filesystem.join(source_checkpoint_id, ".metadata")
            ):
                raise FileNotFoundError(
                    f"Checkpoint step {step} required for the EMA update was not "
                    f"found at {source_checkpoint_id}. Increase "
                    "checkpoint.keep_latest_k or align checkpoint.interval with "
                    "external_eval.freq."
                )
        ...
        if prev_ema_step is not None:
            candidate = self.get_checkpoint_id(prev_ema_step, folder=output_folder)
            if filesystem.isfile(filesystem.join(candidate, ".metadata")):
                ...
                logger.warning(
                    ...
                    "Restarting the EMA chain from the plain average of %s; the "
                    "effective averaging window is shorter than requested until "
                    "enough merges accumulate again.",
                    prev_ema_step,
                    candidate,
                    new_steps,
                )
        ...
        if previous_ema_checkpoint_id is None:
            logger.info(
                "Bootstrapping EMA (decay=%s) at step %s from checkpoints %s.",
                decay,
                curr_step,
                new_steps,
            )
            ...
            logger.info(
                "Updating EMA (decay=%s) at step %s: %s * previous EMA from step "
                "%s + %s * average of %s.",
                decay,
                curr_step,
                decay,
        ...
        self._write_weighted_model_merge(
            ...
            output_checkpoint_id=checkpoint_id,
            # Restore the live model from the newest *regular* checkpoint, which
            # is the training state, not from the EMA output.
