...
def _get_multiplier_value(multipliers, name: str, default: float = 1.0) -> float:
    """Get multiplier value from dict or dataclass, handling both TOML parsing results."""
    if multipliers is None:
        return default
    # Handle dict (from TOML parsing) vs dataclass
    if isinstance(multipliers, dict):
        return multipliers.get(name, default)
    return getattr(multipliers, name, default)
...
def _has_non_default_multipliers(multipliers, field_names: list[str]) -> bool:
    ...
    if multipliers is None:
        return False
    return any(
        _get_multiplier_value(multipliers, name, 1.0) != 1.0 for name in field_names
    )


def _log_non_default_multipliers(
    multipliers, field_names: list[str], label: str
) -> None:
    """Log non-default multiplier values."""
    if multipliers is None:
        return
    non_default = [
        (name, _get_multiplier_value(multipliers, name))
        for name in field_names
        if _get_multiplier_value(multipliers, name, 1.0) != 1.0
    ]
    if non_default:
...
_PARAM_GROUP_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    # Matches both standard .bias (e.g., wo.bias) and MoE _bias (e.g., mlp1_bias)
    ...
    (
        "attention",
        re.compile(
            r"layers\.\d+\.(_checkpoint_wrapped_module\.)?attention\.(wq|wk|wv|wo|sinks)"
        ),
    ),
    (
        "experts",
        re.compile(
            r"layers\.\d+\.(_checkpoint_wrapped_module\.)?moe\.experts\.mlp[12]_weight"
        ),
    ),
    (
        "routers",
        re.compile(r"layers\.\d+\.(_checkpoint_wrapped_module\.)?moe\.router\.gate"),
    ),
    (
        "norms",
        re.compile(
            r"layers\.\d+\.(_checkpoint_wrapped_module\.)?(attention_norm|ffn_norm)\.weight"
        ),
    ),
]
...
def classify_parameters_for_groups(
    ...
    def get_weight_decay(group_name: str) -> float:
        ...
        mult = _get_multiplier_value(weight_decay_multipliers, group_name, 1.0)
        return base_weight_decay * mult

    def get_lr(group_name: str) -> float:
        mult = _get_multiplier_value(lr_multipliers, group_name, 1.0)
        return base_lr * mult
    ...
    for group_name, group_dict in param_groups_dict.items():
        ...
        if using_differential_wd:
            wd_mult = (
                ...
                else (0.0 if wd == 0 else 1.0)
            )
            lines.append(
                f"  {group_name:12s}: {count:4d} params, lr={lr:.2e} ({lr_mult:.1f}x), wd={wd:.2e} ({wd_mult:.1f}x)"
            )
        else:
            lines.append(
                f"  {group_name:12s}: {count:4d} params, lr={lr:.2e} ({lr_mult:.1f}x)"
            )
    logger.info("\n".join(lines))
    ...
    if unmatched_params:
        ...
        suffix = (
            f" ... and {len(unmatched_params) - 10} more"
            if len(unmatched_params) > 10
            else ""
        )
        logger.warning(
            f"Found {len(unmatched_params)} unmatched parameters assigned to 'default' group: {unmatched_names}{suffix}"
        )

    param_groups = [
        group for group in param_groups_dict.values() if len(group["params"]) > 0
    ]
...
class OptimizersContainerWithParamGroups(OptimizersContainer):
    ...
    def __init__(
        ...
        base_kwargs = {
            k: v for k, v in optimizer_kwargs.items() if k not in ("lr", "weight_decay")
        }
...
class CPUOffloadOptimizersContainer(OptimizersContainer):
    ...
    def __init__(
        ...
        logger.info(
            f"Created {len(cpu_params)} CPU shadow parameters (fp32 for optimizer precision)"
        )

        # Determine if using parameter groups with differential LR or weight decay
        using_param_groups = (
            lr_multipliers is not None or weight_decay_multipliers is not None
        )
        ...
        if using_param_groups:
            ...
            effective_lr_multipliers = (
                lr_multipliers if lr_multipliers is not None else LRMultipliers()
            )
            ...
            logger.info(
                "CPUOffloadOptimizer: Using single parameter group with uniform LR"
            )

        # Create CPU optimizer using DeepSpeed's optimized CPUAdam
        # Remove fused/foreach flags as they don't apply to CPU optimizers
        cpu_kwargs = {
            k: v for k, v in optimizer_kwargs.items() if k not in ("fused", "foreach")
        }
    ...
    def step(self, *args, **kwargs) -> None:
        ...
        for gpu_param, cpu_param in self._gpu_to_cpu_param.items():
            ...
            if cpu_param.grad is None:
                ...
                cpu_param.grad = torch.empty_like(
                    cpu_param.data, dtype=torch.float32
                ).pin_memory()
    ...
    def cleanup(self) -> None:
        ...
        logger.info(
            "CPUOffloadOptimizer: Released pinned memory and cleaned up resources"
        )
...
def build_optimizers(
    ...
    **Note**
    ...
    if cpu_offload:
        ...
        weight_decay_multipliers = getattr(
            optimizer_config, "weight_decay_multipliers", None
        )

        # Check and log differential LR
        if _has_non_default_multipliers(lr_multipliers, _LR_MULTIPLIER_FIELDS):
            _log_non_default_multipliers(
                lr_multipliers, _LR_MULTIPLIER_FIELDS, "learning rates"
            )
        else:
            lr_multipliers = None  # All multipliers are 1.0, disable feature

        # Check and log differential weight decay
        if _has_non_default_multipliers(
            weight_decay_multipliers, _WD_MULTIPLIER_FIELDS
        ):
            _log_non_default_multipliers(
                weight_decay_multipliers, _WD_MULTIPLIER_FIELDS, "weight decay"
            )
        else:
