...
def _default_backend_config(
    ...
    *,
    ...
    if isinstance(save_config, AsyncCheckpointSaverConfig):
        structured_logger_init_args = _get_structured_logger_init_args()
        if structured_logger_init_args is not None:
            subprocess_init_args = (
                structured_logger_init_args,
                subprocess_init_fn,
                subprocess_init_args,
            )
...
class TorchCheckpointingManager(BaseCheckpointManager):
    ...
    def _save_last_step(self, curr_step: int) -> None:
        ...
        if self.last_save_in_hf:
            ...
            # actually written to -- its staging directory when a write barrier
            # is configured, the final path otherwise -- so consolidate from
            # that path as given, deriving nothing from it.
