...
def _init_subprocess_logging(
    structured_logger_init_args: tuple[str, str, int],
    ...
    source, output_dir, rank = structured_logger_init_args
    sl.init_structured_logger(source=source, output_dir=output_dir, rank=rank)
...
def _with_structured_logging(
    ...
    Any existing ``subprocess_init_fn`` is chained rather than replaced.
    """
    init_args = _get_structured_logger_init_args()
    if init_args is None or not isinstance(config.save, AsyncCheckpointSaverConfig):
        return config
    return replace(
        config,
...
class TorchCheckpointingManager(BaseCheckpointManager):
    ...
    def __init__(
        ...
        *,
        ...
        manager_config = _with_structured_logging(manager_config)
    ...
    def _load(self, step: int = -1) -> bool:
        ...
        if from_hf:
            ...
            model_spec = replace(
                self._manager_config.items[MODEL],
                resharder=HFSafetensorsDTensorResharder(),
            )
            hf_config = replace(
                _with_sync_save(self._manager_config, use_barrier=False),
                items={MODEL: model_spec},
                default=None,
            )
    ...
    def _find_native_load_step(self) -> int:
        ...
        for filename in self._storage.listdir(self.folder):
            ...
            if self._is_native_checkpoint(checkpoint_id):
                valid_steps.append(step)
        return max(valid_steps) if valid_steps else -1

    def _is_native_checkpoint(self, checkpoint_id: str) -> bool:
        return self._storage.isfile(
            filesystem.join(checkpoint_id, TORCH_CHECKPOINTING_METADATA_FILE_NAME)
        )

    def _is_hf_checkpoint(self, checkpoint_id: str) -> bool:
        return self._storage.isfile(
            filesystem.join(checkpoint_id, _HF_INDEX_FILE_NAME)
        ) or self._storage.isfile(filesystem.join(checkpoint_id, _HF_SINGLE_FILE_NAME))
    ...
    def _is_valid_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        return self._is_native_checkpoint(checkpoint_id) or self._is_hf_checkpoint(
            checkpoint_id
        )
