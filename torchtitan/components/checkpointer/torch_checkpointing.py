...
class _BackendCheckpointStorage:
    ...
    @staticmethod
    def _backend_path(path: str) -> Path:
        """Convert to the ``Path`` the backend's ``Storage`` API expects.

        ``Path`` collapses the double slash in a URI -- ``gs://bucket/x``
        becomes ``gs:/bucket/x`` -- so a remote checkpoint id would silently
        address the wrong location. The backend is ``Path``-typed throughout and
        ships only a local filesystem storage, so remote URIs are not something
        it can express; refuse them here rather than corrupt them.
        """
        if filesystem.is_remote(path):
            raise ValueError(
                "torch_checkpointing does not support remote checkpoint paths; "
                f"got {path!r}. Use the DCP checkpoint manager for remote "
                "storage, or pass a storage_config that handles this URI."
            )
        return Path(path)

    def isdir(self, path: str) -> bool:
        return self._storage.isdir(self._backend_path(path))

    def isfile(self, path: str) -> bool:
        target = self._backend_path(path)
        return self._storage.exists(target) and not self._storage.isdir(target)

    def listdir(self, path: str) -> list[str]:
        return self._storage.ls(self._backend_path(path))

    def remove(self, path: str) -> None:
        self._storage.rmdir(self._backend_path(path))
...
def _init_subprocess_logging(
    ...
    # It inherits none of the parent's logging levels either: the root logger
    # starts at WARNING, so the backend's INFO checkpoint events would be
    # discarded by the logger before any handler could forward them. Raise the
    # level only when it is not already permissive enough, so an explicitly
    # more verbose setting survives.
    backend_logger = logging.getLogger(_BACKEND_LOGGER_NAME)
    if not backend_logger.isEnabledFor(logging.INFO):
...
class TorchCheckpointingManager(BaseCheckpointManager):
        ...
        *,
        ...
        Args:
            storage_config: Backend storage for reading and writing checkpoints.
                Defaults to the local filesystem. An init parameter rather than
                a ``Config`` field because ``Configurable.Config`` is Tyro-parsed
                and a backend storage object is not a command-line surface;
                callers that need remote storage pass it programmatically.
        """
    ...
    def _save(self, curr_step: int, last_step: bool = False) -> bool:
        ...
        self._purge_stale_checkpoints(save_pending=True)
    ...
    def _maybe_wait_for_staging(self) -> None:
        ...
        # save has drained; there is no separate staging future to await.
    ...
    def _close(self) -> None:
        try:
            ...
            try:
                ...
                # before building the backend manager.
                if hasattr(self, "_manager") and not self._manager_closed:
                    try:
                        self._manager.close()
                    finally:
