...
class BaseCheckpointManager(Configurable, ABC):
    ...
    def _purge_stale_checkpoints(
        ...
        *,
        ...
        """Delete abandoned entries and reserve one retained slot for the next save."""
        if self._should_purge():
            ...
            for checkpoint_dir in abandoned:
                logger.info(
                    "Checkpointer is deleting the abandoned %s.", checkpoint_dir
                )
                try:
                    self._storage.remove(checkpoint_dir)
                except Exception as error:
                    logger.warning(
                        "Checkpointer failed to delete %s: %s. Skipping.",
                        checkpoint_dir,
                        error,
                    )
