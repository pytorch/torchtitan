...
```
* `recipe_name`: Float8 recipe name. Options: `"rowwise"` (default), `"rowwise_with_gw_hp"`, `"tensorwise"`.
...
* `enable_fsdp_float8_all_gather`: Enables all-gather in float8 precision for FSDP. Options: `False` (default), `True`.
* `precompute_float8_dynamic_scale_for_fsdp`: Register post optimization hook to precompute dynamic scales. Options: `False` (default), `True`.
...
For parallelisms, for float8 with rowwise scaling, all distributed communication is done in high precision unless  `enable_fsdp_float8_all_gather` is set to `True`.
