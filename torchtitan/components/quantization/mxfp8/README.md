...
- **FSDP-managed dense weights**: After FSDP all-gathers a dense Linear weight
  in BF16, TorchTitan's post-all-gather hook uses a pluggable weight strategy
  to create FPROP and DGRAD representations. FSDP owns those buffers, so their
  lifetime follows the normal reshard-after-forward and
...
The practical default combines 32x32 square scale tiles for weights with 1x32
tiles for activations. Square weight tiles still introduce quantization error,
but they are orientation-symmetric: FPROP and DGRAD use the same quantized
values and can share one cached qdata allocation. This avoids choosing two
independently quantized weight representations for the two GEMM orientations.

### MXFP8 for Linear Modules

The built-in `32x32` dense-weight strategy uses square tiles. Both local
weight dimensions must currently be divisible by 32. Activations always use
standard 1D scaling. Additional weight strategies can be registered without
changing `MXFP8Linear`; strategies whose operands share storage must describe
that aliasing through their FSDP inner-tensor contract.
...
The converter and linear config field docstrings describe the available options
and the memory trade-off between saving BF16 and MXFP8 input activations.
