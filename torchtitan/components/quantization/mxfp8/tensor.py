...
class MXFP8FSDPWeight(torch.Tensor):
    ...
    def __torch_dispatch__(
        ...
        wrapped_weights = [
            ...
            if isinstance(value, MXFP8FSDPWeight)
        ]
        weight_quantizations = {
            weight.weight_quantization for weight in wrapped_weights
        }
        if len(weight_quantizations) != 1:
            raise AssertionError(
                "Expected one MXFP8 weight quantization strategy per operation, "
                f"got {sorted(weight_quantizations)}."
            )
        (weight_quantization,) = weight_quantizations

        def unwrap(tensor: MXFP8FSDPWeight) -> torch.Tensor:
            return tensor._data

        def wrap(tensor: torch.Tensor) -> MXFP8FSDPWeight:
            return cls(tensor, weight_quantization)
    ...
    def __tensor_unflatten__(
        ...
        return cls(inner_tensors["_data"], metadata)
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        strategy = get_mxfp8_weight_quantization_strategy(self.weight_quantization)

        if out is None:
            # On the first unshard, return the logical compute weight plus the
            # strategy-specific inner tensors whose storage FSDP should manage.
            operands = quantize_mxfp8_weight(weight_NK, self.weight_quantization)
            ...
            return compute_weight, strategy.fsdp_managed_tensors(operands)
        ...
        if local_out.weight_quantization != self.weight_quantization:
            raise AssertionError(
                "FSDP compute weight strategy changed across unshards: "
                f"{local_out.weight_quantization!r} != "
                f"{self.weight_quantization!r}."
            )

        operands = quantize_mxfp8_weight(weight_NK, self.weight_quantization)
        quantized_tensors = strategy.fsdp_managed_tensors(operands)
...
class MXFP8FSDPComputeWeight(torch.Tensor):
    """Logical BF16 weight backed by strategy-specific MXFP8 operands."""
    ...
    def fsdp_managed_tensors(self) -> tuple[torch.Tensor, ...]:
        ...
        The compute weight always exposes four logical GEMM operands, but some
        may be views of the same storage. The built-in 32x32 strategy omits the
        DGRAD qdata because it is a transpose view of the FPROP qdata. A custom
        strategy with orientation-specific qdata would return both allocations.
        FSDP keeps these tensor objects stable and allocates, frees, or refills
        their storage across unshard and reshard transitions.

        The returned order is the strategy's FSDP storage contract and must
        match the tensors produced by its quantization implementation.
        """
        strategy = get_mxfp8_weight_quantization_strategy(self.weight_quantization)
        return strategy.fsdp_managed_tensors(self.operands())
    ...
    def __repr__(self, *, tensor_contents: Any | None = None) -> str:
        ...
        return (
            ...
            f"weight_quantization={self.weight_quantization!r})"
        )
    ...
    def __tensor_flatten__(self):
        ...
        strategy = get_mxfp8_weight_quantization_strategy(self.weight_quantization)
        return list(strategy.fsdp_managed_tensor_names), metadata
    ...
    def __tensor_unflatten__(
        ...
        strategy = get_mxfp8_weight_quantization_strategy(weight_quantization)
        operands = strategy.reconstruct_operands_from_inner_tensors(inner_tensors)
        return cls(
            operands,
