...
``_UnshardedFSDPTensor``
    ...
    the all-gather output. Generic: it derives its managed tensors from the
...
Instance timeline, from module construction to reshard::
    ...
    fsdp_pre_all_gather  S._tensor.to(param_dtype)  ->  comm tensor
                         |
                         v
    (all-gather)         replicated param_dtype weight, temporary
                         |
    ...
    forward/backward     compute reads C.operands
                         |
    ---- reshard ----------------------------------------------------------
                         FSDP frees the storage of C's managed tensors.
...
reconstructs the replicated weight itself, then quantizes and wraps it from
...
all-gather output, and hence the logical weight a format quantizes. Nothing
...
``_build_operands(logical_weight, out=None)``
...
managed tensors
    ...
    expose: a derived view belongs in a property, and properties are not
    managed, since FSDP must not free one allocation twice. These are exactly
    the tensors ``__tensor_flatten__`` reports, which FSDP holds as
...
# unsharded weight, as GraphTrainer's SimpleFSDP does.
__all__: list[str] = []
...
def _managed_tensor_names(operands_cls: type) -> tuple[str, ...]:
    """Return the names of the managed tensors an operands dataclass owns."""
...
def _managed_tensors(operands: Any) -> tuple[torch.Tensor, ...]:
    """Return the managed tensors an operands dataclass owns.

    Field order defines the managed tensor order, so no format restates
    ...
    """
    tensors = tuple(
        getattr(operands, name) for name in _managed_tensor_names(type(operands))
    )
...
def _validate_refilled_tensor_identity(
    ...
    if len(managed_tensors) != len(refilled_tensors) or any(
        ...
        raise RuntimeError("FSDP unsharded-operands refill replaced managed storage")
...
class _ShardedFSDPTensor(_FSDPTensorBase):
    ...
    * ``_build_operands(logical_weight, out=None)``, which
    ...
    def fsdp_pre_all_gather(self, mesh, outer_size, outer_stride, module, mp_policy):
        ...
        shard_dim = self._sharded_dim(outer_size)
        if shard_dim not in (None, 0):
            raise NotImplementedError(
                "FSDP unsharded tensors support sharding dimension 0 only, but "
                f"this parameter of shape {tuple(outer_size)} is sharded on "
                f"dimension {shard_dim}. TorchTitan selects Shard(1) for "
    ...
    def _sharded_dim(self, outer_size) -> int | None:
        """Return the dimension FSDP sharded, or None when it did not shard.

        FSDP does not pass the shard dimension, but the local shard differs
        from the logical size exactly along it.
        """
        dims = [
            dim
    ...
    def fsdp_post_all_gather(
        ...
        if out is None:
            with torch.no_grad():
                operands = self._build_operands(gathered_weight)
            return (
                _UnshardedFSDPTensor(gathered_weight, operands),
                _managed_tensors(operands),
            )
        ...
        managed_tensors = _managed_tensors(existing)
        with (
            torch.no_grad(),
            ...
            refilled = self._build_operands(gathered_weight, out=existing)
        _validate_refilled_tensor_identity(managed_tensors, _managed_tensors(refilled))
...
class _UnshardedFSDPTensor(_FSDPTensorBase):
    ...
    Generic by design: the managed tensors come from the operands'
    ...
    def __init__(
        ...
        **logical_metadata: Any,
        ...
        # __tensor_flatten__ reports the managed tensors by attribute name and
        ...
        for name in _managed_tensor_names(type(operands)):
            setattr(self, f"_{name}", getattr(operands, name))

    def __tensor_flatten__(self):
        operands_cls = type(self._operands)
        names = [f"_{name}" for name in _managed_tensor_names(operands_cls)]
    ...
    def __tensor_unflatten__(inner_tensors, metadata, outer_size, outer_stride):
        ...
        managed_tensors = [
            inner_tensors[f"_{name}"] for name in _managed_tensor_names(operands_cls)
        ]
        operands = operands_cls(*managed_tensors)
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs=None):
        ...
        def wrap_view(tensor: torch.Tensor):
            ...
            layout_source = _managed_tensors(operands)[0]
            return _UnshardedFSDPTensor(
                layout_source,
                operands,
