...
class BaseActOffloadPolicy():

    """
    An interface for activation offload policies. The policy is responsible for
    deciding whether to offload activations or not.
    """

    def __init__(self, *args, **kwargs):
        pass

    def should_offload(self, device_tensor: torch.Tensor) -> bool:
        """
        This methods decides whether to offload a given tensor or not.
        """
        return True


class ExampleActOffloadPolicy(BaseActOffloadPolicy):

    """
    An example policy that offloads tensors based on tensor type, size and ratio.
    """

    def __init__(self,
                 min_tensor_size_bytes: int = 1 * 1024 * 1024,
                 ignore_types: list[torch.dtype] = [torch.complex64, torch.int64],
                 offload_ratio: float = 1.0):
        self.min_tensor_size_bytes = min_tensor_size_bytes
    ...
    def should_offload(self, device_tensor: torch.Tensor) -> bool:
        if device_tensor.numel() < self.min_tensor_size_bytes:
            return False
        elif device_tensor.dtype in self.ignore_types:
            return False
...
class ActOffloadContext():

    def __init__(self,
                 offload_stream: Optional[torch.cuda.Stream] = None,
                 offload_policy: Optional[BaseActOffloadPolicy] = None
            ) -> None:
        ...
        # The solver used to decide whether to offload a tensor or not
        if offload_policy is None:
            self.offload_policy = BaseActOffloadPolicy()
        else:
            self.offload_policy = offload_policy
        # Used for module ordering
        self.module_id_to_module: dict[int, Module] = {}
        self.next_module_id = 0
        # Used in forward to keep device tensors alive through D2H copies
        self.module_to_pack_infos: dict[Module, list[PackInfo]] = defaultdict(list)
        ...
        self.module_to_cpu_tensors: dict[Module, list[torch.Tensor]] = defaultdict(list)
        # Used in backward to preallocate device tensors in the default stream
        self.cpu_tensor_to_unpack_info: dict[torch.Tensor, UnpackInfo] = {}
...
class activation_offload_with_overlap(saved_tensors_hooks):
    ...
    def __init__(self, ao_context: ActOffloadContext, module: torch.nn.Module) -> None:
        self.ctx = ao_context

        module_id = self.ctx.next_module_id
        self.ctx.module_id_to_module[module_id] = module
        self.ctx.next_module_id += 1
        ...
        def pack_to_cpu(tensor: torch.Tensor) -> tuple[torch.device, torch.Tensor]:
            ...
            # Use offloading policy to decide whether to offload.
            ...
            if not self.ctx.offload_policy.should_offload(device_tensor):
            ...
            if module_id_to_free in self.ctx.module_id_to_module:
                # Have the first of module i to free all of module i-1
                # logger.info(f"Trying to free {module_id_to_free}...")
                module_to_free = self.ctx.module_id_to_module[module_id_to_free]
                self.free_packed_device_tensors(module_to_free)

            self.ctx.offload_stream.wait_stream(current_stream)
            with torch.cuda.stream(self.ctx.offload_stream):
                ...
                d2h_event = self.ctx.offload_stream.record_event()
                self.tensors_offloaded += 1

            self.ctx.module_to_cpu_tensors[module].append(cpu_tensor)
            self.ctx.module_to_pack_infos[module].append(PackInfo(d2h_event, device_tensor))
        ...
        def unpack_from_cpu(packed) -> torch.Tensor:
            ...
            if module_id_to_prealloc in self.ctx.module_id_to_module:
                module_to_prealloc = self.ctx.module_id_to_module[module_id_to_prealloc]
                if module_to_prealloc in self.ctx.module_to_cpu_tensors:
                    cpu_tensors = self.ctx.module_to_cpu_tensors[module_to_prealloc]
                    for _cpu_tensor in cpu_tensors:
                        self.ctx.cpu_tensor_to_unpack_info[_cpu_tensor] = UnpackInfo(
                            current_stream.record_event(),
                            torch.empty_like(_cpu_tensor, device=device),
                        )
                    del self.ctx.module_to_cpu_tensors[module_to_prealloc]

            if cpu_tensor in self.ctx.cpu_tensor_to_unpack_info:  # prefetched
                event, device_tensor = self.ctx.cpu_tensor_to_unpack_info[cpu_tensor]
                self.offload_stream.wait_event(event)
                del self.ctx.cpu_tensor_to_unpack_info[cpu_tensor]
            else:
                device_tensor = torch.empty_like(cpu_tensor, device=device)
                # Preallocate the rest of the 1st backward module
                for _cpu_tensor in self.ctx.module_to_cpu_tensors[module]:
                ...
                del self.ctx.module_to_cpu_tensors[module]
    ...
    def free_packed_device_tensors(self, module: torch.nn.Module):
        if module in self.ctx.module_to_pack_infos:
            # logger.info(f"Trying to free packed device tensors from module {id(module):#x}")
            if infos := self.ctx.module_to_pack_infos[module]:
            ...
            del self.ctx.module_to_pack_infos[module]
