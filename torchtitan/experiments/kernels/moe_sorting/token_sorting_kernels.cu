...
__global__ void count_tokens_per_expert_kernel(
  ...
  extern __shared__ int s_expert_counts[];
  ...
  // Process tokens - each thread handles one token
  for (int token_idx = blockIdx.x * blockDim.x + threadIdx.x;
       token_idx < seq_len; token_idx += blockDim.x * gridDim.x) {

    // Create a local array to track which experts we've seen
    bool seen_experts[512] = {false}; // Supports up to 512 experts

    // Mark which experts are assigned to this token
    for (int j = 0; j < k; j++) {
      int expert_id = static_cast<int>(topk_ids[token_idx * k + j]);
      if (expert_id >= 0 && expert_id < n_experts) {
        seen_experts[expert_id] = true;
      }
    }

    // Update shared memory counters for this token
    for (int expert_id = 0; expert_id < n_experts; expert_id++) {
      if (seen_experts[expert_id]) {
  ...
  // Contribute local counts to global counts
  for (int i = threadIdx.x; i < n_experts; i += blockDim.x) {
...
// Kernel to gather token features based on sorted indices
...
sort_tokens_by_expert_cuda(torch::Tensor topk_ids, torch::Tensor x,
  ...
  const int count_blocks =
      std::min(128, (seq_len + count_threads - 1) / count_threads);
  ...
  // counting kernel
  count_tokens_per_expert_kernel<<<count_blocks, count_threads,
                                   shared_mem_size>>>(
  ...
  */
  ...
  // Step 2: Use PyTorch's argsort - verify no cpu-gpu sync
  ...
  // Optimization: For very large hidden dimensions, adjust block size
  int max_threads_per_block;
  cudaDeviceGetAttribute(&max_threads_per_block, cudaDevAttrMaxThreadsPerBlock,
                         device.index());

  // Calculate optimal thread configuration
  const int gather_token_threads = std::min(16, max_threads_per_block / 16);
  const int gather_feature_threads =
      std::min(16, max_threads_per_block / gather_token_threads);
