...
struct Schedule {
  ...
  using EpilogueSchedule = cute::conditional_t<PONG, PongEpilogueSchedule,
                                               CooperativeEpilogueSchedule>;
};

int ceildiv(int a, int b) { return (a + b - 1) / b; }

int round_up_to_nearest_multiple(int a, int b) { return ceildiv(a, b) * b; }

template <bool a_row_major, bool b_row_major, bool Pong, typename TB_M,
          typename TB_N,
          typename TB_K>
void bf16bf16_grouped_gemm_impl_sm90(at::Tensor mat_a, // bf16
  ...
  using LayoutA = cute::conditional_t<a_row_major, cutlass::layout::RowMajor,
                                      cutlass::layout::ColumnMajor>;
  constexpr int AlignmentA = 16 / sizeof(DtypeA);

  using LayoutB = cute::conditional_t<b_row_major, cutlass::layout::RowMajor,
                                      cutlass::layout::ColumnMajor>;
  ...
  using CollectiveEpilogue =
      typename cutlass::epilogue::collective::CollectiveBuilder<
          ...
          LayoutOutput *, AlignmentOutput, EpilogueSchedule,
          cutlass::epilogue::fusion::LinearCombination<
              DtypeOutput, DtypeAccum>>::CollectiveOp;

  using CollectiveMainloop =
      typename cutlass::gemm::collective::CollectiveBuilder<
          ArchTag, OperatorClass, DtypeA, LayoutA *, AlignmentA, DtypeB,
          LayoutB *, AlignmentB, DtypeAccum, TileShape, ClusterShape,
  ...
  static_assert(sizeof(StrideA) == 8,
                "expected StrideA to be 8 bytes for alignment");
  StrideA *stride_A =
  ...
  ProblemShape::UnderlyingProblemShape *problem_sizes =
      reinterpret_cast<ProblemShape::UnderlyingProblemShape *>(stride_output +
                                                               group_count);
  ...
  at::cuda::detail::prepare_grouped_gemm_data<<<1, group_count, 0, stream>>>(
      ...
      static_cast<float *>(nullptr), // type for template inference
      inputA_ptrs, inputB_ptrs, output_ptrs,
      static_cast<float **>(nullptr), // type for template inference
      static_cast<float **>(nullptr), // type for template inference
      problem_sizes, stride_A, stride_B, stride_output,
      offs.has_value() ? offs->const_data_ptr<int32_t>() : nullptr, M, N, K,
      tensor_StrideA, tensor_StrideB, tensor_StrideOutput, 0, 0, a_row_major,
      b_row_major);
  ...
  typename Gemm::Arguments arguments{cutlass::gemm::GemmUniversalMode::kGrouped,
                                     ...
                                     {(const DtypeA **)inputA_ptrs, stride_A,
                                      (const DtypeB **)inputB_ptrs, stride_B},
                                     {{},
...
void dispatch_bf16_grouped_kernel_on_tile_size(
  ...
  if (small) {
    bf16bf16_grouped_gemm_impl_sm90<a_row_major, b_row_major,
                                    /*Pong*/ true, cute::_64, cute::_128,
                                    cute::_128>(mat_a, mat_b, offs, bias, out);
  } else {
    bf16bf16_grouped_gemm_impl_sm90<a_row_major, b_row_major,
                                    /*Pong*/ false, cute::_128, cute::_256,
...
void dispatch_bf16_grouped_kernel_on_ab_transpose(
  ...
  if (a_row_major && b_row_major) {
    dispatch_bf16_grouped_kernel_on_tile_size<true, true>(mat_a, mat_b, offs,
                                                          bias, out);
  } else if (a_row_major && !b_row_major) {
    dispatch_bf16_grouped_kernel_on_tile_size<true, false>(mat_a, mat_b, offs,
                                                           bias, out);
  } else if (!a_row_major && b_row_major) {
    dispatch_bf16_grouped_kernel_on_tile_size<false, true>(mat_a, mat_b, offs,
                                                           bias, out);
  } else {
    dispatch_bf16_grouped_kernel_on_tile_size<false, false>(mat_a, mat_b, offs,
                                                            bias, out);
  }
}
...
void bf16bf16_grouped_mm(at::Tensor mat_a, // bf16
                         at::Tensor mat_b, // bf16
                         std::optional<at::Tensor> offs,
                         std::optional<at::Tensor> bias, // BF16
                         at::Tensor &out) {
