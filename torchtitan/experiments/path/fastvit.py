...
__all__ = ['FastVit', 'fastvit_t12', 'reparameterize_model']

def _allnorm_act2d(num_features: int, **kwargs) -> AllNormAct2d:
    return AllNormAct2d(num_features, **kwargs)
...
class MobileOneBlock(nn.Module):
    ...
    def __init__(
        ...
        if inference_mode:
            ...
            self.identity = (
                AllNorm2d(num_features=in_chs, **dd)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
            ...
            mlp_ratio: float = 4.0,
            act_layer: Type[nn.Module] = nn.GELU,
            norm_layer: Type[nn.Module] = AllNorm2d,
            proj_drop: float = 0.0,
            drop_path: float = 0.0,
            layer_scale_init_value: float = 1e-5,
            device=None,
...
def reparameterize_model(model: nn.Module) -> int:
    count = 0
    for module in reversed(list(model.modules())):
        reparameterize = getattr(module, "reparameterize", None)
        if reparameterize is None or getattr(module, "reparam_conv", None) is not None:
            continue
        reparameterize()
        count += 1
    return count
...
if __name__ == "__main__":
    ...
    model = fastvit_t12(pretrained=False, in_chans=24, num_classes=512).eval()
    print(model)
    n_reparam = reparameterize_model(model)
    x = torch.zeros(1, 24, 128, 256)
    y = model(x)
    print(f"fastvit_t12 reparameterized {n_reparam} modules")
