...
class PolicyTrainer(Actor, Stateful, Configurable):
    ...
    def _validate_batch_policy_version(self, policy_version: int) -> None:
        if policy_version != self.policy_version:
            raise ValueError(
                "stale or mixed rollout batch: "
                f"trainer expects policy_version={self.policy_version}, "
                f"got {policy_version}"
            )
    ...
    async def forward_backward(
        ...
        self._validate_batch_policy_version(local_batch.policy_version)

        token_ids = local_batch.token_ids.to(self.device)  # [B, L]
        ...
        response_mask = build_response_mask(
            local_batch.prompt_lens.to(self.device),
            local_batch.response_lens.to(self.device),
            token_ids.shape[1],
            self.device,
        )
