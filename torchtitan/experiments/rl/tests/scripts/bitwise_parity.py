...
def build_trainer_model(
    ...
    # Mirror PolicyTrainer._build_model: fill sharding configs (and any
    # other parallelism-driven config mutations) on the model config BEFORE
    # build, so each Module is constructed with its ShardingConfig /
    # LocalMapConfig. Without this the trainer side would run
    # un-parallelized while the vLLM generator runs fully TP-parallelized,
    # breaking trainer-vs-vLLM parity.
...
def _build_padded_varlen_metadata(batch_size, max_len, device):
    ...
    so each row boundary is at multiples of max_len. Causal masking
    prevents padding tokens from affecting valid positions.
    """
...
def compute_trainer_prefill_logprobs(model, token_ids, device):
    ...
    # plain tensor — materialize once here. Same pattern as
    # ``compute_logprobs`` in ``actors/utils.py``.
...
class BitwiseParityChecker:
    """Multi-rank bitwise parity checks run inside a torchrun worker group."""
    ...
    def create(cls):
        ...
        if missing_env:
            raise RuntimeError(
                "bitwise_parity.py must be launched with torchrun; "
                f"missing environment variables: {', '.join(missing_env)}"
            )
        if not torch.cuda.is_available():
            raise RuntimeError(
                "bitwise_parity.py requires visible CUDA devices; "
                "TestBitwiseParityIntegration must run on an H100 GPU runner"
            )
        ...
        # Batch-invariant kernels must be selected before distributed init.
        ...
        # Test runs trainer and generator in the same process; cap vLLM's
        # GPU memory so the trainer model has room to materialize.
        config.generator.gpu_memory_limit = 0.5

        checker = cls()
        checker.model, checker.device = build_trainer_model(config)
        checker.engine = build_inference_engine(config)

        tokenizer = checker.engine.get_tokenizer()
        checker.prompt_ids = _make_prompt_tokens(
    ...
    def check_batch_invariance(self):
        ...
        Guards that model kernels are batch-invariant: the same sequence
        must produce bit-identical logits regardless of what other
    ...
    def check_trainer_vs_vllm_prefill(self):
        """Trainer prefill == vLLM prefill (prompt-only).

        Ensures the trainer model forward and generator model forward
    ...
    def check_vllm_decode_vs_prefill(self):
        """vLLM decode == vLLM 2nd-pass prefill (generated positions).

        Ensures prefill-stage attention and decode-stage KV-cache
        attention produce bitwise identical logprobs.
        """
        engine = self.engine
...
CHECKS = (
    ("batch_invariance", BitwiseParityChecker.check_batch_invariance),
    ("trainer_vs_vllm_prefill", BitwiseParityChecker.check_trainer_vs_vllm_prefill),
    ("vllm_decode_vs_prefill", BitwiseParityChecker.check_vllm_decode_vs_prefill),
)
...
def run_checks(result_path: Path) -> int:
    completed: list[dict[str, str]] = []
    try:
        checker = BitwiseParityChecker.create()
        for name, check in CHECKS:
            check(checker)
            if _rank() == 0:
                completed.append({"name": name, "status": "passed"})
        _write_summary(
            result_path,
            {
                "status": "passed",
        ...
        logger.exception("bitwise parity check failed")
        return 1
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON file written by rank 0 with check status and failure details.",
    )
    args = parser.parse_args()
    return run_checks(args.result_path)
