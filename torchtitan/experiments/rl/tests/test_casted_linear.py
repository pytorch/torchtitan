...
"""Unit tests for ``CastedLinear`` and ``LMHeadCastConverter``."""
...
from torchtitan.experiments.rl.models.casted_linear import (
    CastedLinear,
    LMHeadCastConverter,
)
...
def test_converter_swaps_only_lm_head():
    ...
    assert not isinstance(cfg.lm_head, CastedLinear.Config)
    ...
    # Only the lm_head was swapped to CastedLinear.
    assert isinstance(cfg.lm_head, CastedLinear.Config)
    casted = [fqn for fqn, _, _, _ in cfg.traverse(CastedLinear.Config)]
    assert casted == ["lm_head"]
    # The swap doesn't add or drop Linear nodes (CastedLinear.Config is a
...
def test_forward_matmul_runs_in_fp32():
    lm_head = CastedLinear.Config(in_features=8, out_features=16).build()
