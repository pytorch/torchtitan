...
# vLLM's fused RMSNorm kernel (Triton-based, single kernel launch)
from vllm.model_executor.layers.layernorm import RMSNorm as VLLMRMSNorm


class Qwen3TransformerBlockVLLMRMSNorm(Qwen3TransformerBlock):
    """Qwen3TransformerBlock with vLLM's fused RMSNorm instead of nn.RMSNorm."""
...
class Qwen3ModelVLLMRMSNorm(Qwen3Model):
    """Qwen3Model with vLLM's fused RMSNorm kernel everywhere.

    Differences from Qwen3Model:
    - TransformerBlock uses VLLMRMSNorm instead of nn.RMSNorm
    - Final norm uses VLLMRMSNorm instead of nn.RMSNorm
    """
    ...
    def __init__(self, config: Qwen3Model.Config):
        ...
        # Replace all layer norms with the ablation block
        self.layers = torch.nn.ModuleDict()
        for layer_id in range(config.n_layers):
            self.layers[str(layer_id)] = Qwen3TransformerBlockVLLMRMSNorm(
                config.layer,
                layer_id=layer_id,
                dim=config.dim,
...
def ablation_model_registry(flavor: str) -> ModelSpec:
    """Return a ModelSpec identical to qwen3's but using the VLLMRMSNorm ablation model."""
    ...
    # This makes config.build() instantiate Qwen3ModelVLLMRMSNorm.
    base_config = base_spec.model

    class _AblationConfig(type(base_config)):
        _owner = Qwen3ModelVLLMRMSNorm
