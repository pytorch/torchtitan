...
def _rotary_embedding(
    ...
    q_out = query.clone()
    k_out = key.clone()
    torch.ops._C.rotary_embedding(positions, q_out, k_out, head_size, cos_sin_cache, True)
    return q_out, k_out
...
def _gqa_forward_with_vllm_rope(
    ...
    if self.q_norm is not None:
        xq = F.rms_norm(xq, self.q_norm.normalized_shape, _local(self.q_norm.weight), self.q_norm.eps)
    if self.k_norm is not None:
        xk = F.rms_norm(xk, self.k_norm.normalized_shape, _local(self.k_norm.weight), self.k_norm.eps)
...
def replace_rope_with_vllm_rotary(model) -> None:
    ...
    """
    cos_sin_cache = _convert_rope_cache_to_vllm(
        model.freqs_cis, dtype=torch.bfloat16
    )
