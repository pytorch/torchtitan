...
def generate() -> None:
    ...
    if gen_config.chunked_prefill_max_num_batched_tokens is not None:
        engine_kwargs[
            "max_num_batched_tokens"
        ] = gen_config.chunked_prefill_max_num_batched_tokens
    if not has_cuda_capability(9, 0):
        engine_kwargs["block_size"] = 256
    vllm_compilation_config = gen_config.cudagraph.get_vllm_compilation_config(
        max_num_seqs=max_num_seqs,
        max_num_batched_tokens=gen_config.chunked_prefill_max_num_batched_tokens,
    )
