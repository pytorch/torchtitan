...
def _get_rl_config(model_size: str):
    """Get the current Qwen3 RL config used by the legacy benchmark paths."""
    from torchtitan.experiments.rl.examples.alphabet_sort.config_registry import (
        rl_grpo_qwen3_0_6b_varlen,
    )
    from torchtitan.models.qwen3 import model_registry

    rl_config = rl_grpo_qwen3_0_6b_varlen()
    if model_size != "0.6B":
        rl_config.model_spec = model_registry(model_size, attn_backend="varlen")
    return rl_config


def _get_model_spec(model_family: str, model_size: str):
    if model_family == "qwen3":
        from torchtitan.models.qwen3 import model_registry

        return model_registry(model_size, attn_backend="varlen")
    if model_family == "deepseek_v3":
        from torchtitan.models.deepseek_v3 import model_registry

        return model_registry(model_size, attn_backend="flex")
    raise ValueError(f"Unsupported model family: {model_family}")


def _default_model_path(model_family: str, model_size: str) -> str:
    if model_family == "qwen3":
        if model_size == "0.6B":
            return "torchtitan/experiments/rl/example_checkpoint/Qwen3-0.6B"
        if model_size == "30B-A3B":
            return "torchtitan/experiments/rl/example_checkpoint/Qwen3-30B-A3B"
        return f"Qwen/Qwen3-{model_size}"
    if model_family == "deepseek_v3":
        if model_size == "16B":
            return "./assets/hf/deepseek-moe-16b-base"
        if model_size == "671B":
            return "./assets/hf/DeepSeek-V3.1-Base"
    raise ValueError(
        "No default model path for "
        f"model_family={model_family!r}, model_size={model_size!r}; "
...
class BenchmarkConfig:
    ...
    # Model size key for registry, e.g. qwen3: "0.6B"; deepseek_v3: "16B".
    model_size: str = "0.6B"
    ...
    def setup(self):
        try:
            ...
            model_spec = _get_model_spec(
                self.config.model_family, self.config.model_size
            )
            model_path = _normalize_model_path(
                self.config.model_path
                or _default_model_path(self.config.model_family, self.config.model_size)
            )

            if self.config.ep not in (1, self.config.dp * self.config.tp):
                raise ValueError(
                    f"--ep must be 1 or --dp * --tp for vLLM TorchTitan inference; "
                    f"got ep={self.config.ep}, dp={self.config.dp}, tp={self.config.tp}"
                )

            parallelism = InferenceParallelismConfig(
                data_parallel_degree=self.config.dp,
                tensor_parallel_degree=self.config.tp,
                expert_parallel_degree=self.config.ep,
                spmd_backend=self.config.spmd_backend,
                enable_async_tensor_parallel=self.config.enable_async_tp,
            ...
            load_hf_weights = not (
                self.config.spmd_backend == "spmd_types" and self.config.tp > 1
            )
            checkpoint_config = (
            ...
            use_cudagraph = (
                use_compile
                and not self.config.enable_async_tp
                and not self.config.disable_cudagraph
            )
            mode_str = (
                f"compile({self.config.compile_backend}) + cudagraph"
                if use_cudagraph
                else f"compile({self.config.compile_backend}) + no cudagraph"
                if use_compile
                else "eager (no compile, no cudagraph)"
            )

            print(
                f"Loading vLLM with TorchTitan {self.config.model_family} "
                f"{self.config.model_size} model..."
            )
            print(f"Model: {model_path}")
            if not load_hf_weights:
                print(
                    "HF weight loading: disabled for spmd_types TP benchmark "
                    "(initialized weights; same tensor shapes/sharding)"
                )
            print(
                f"Parallelism: DP={self.config.dp}, TP={self.config.tp}, "
                f"EP={self.config.ep}, SP={not self.config.disable_sp}, "
                f"SPMD={self.config.spmd_backend}, "
                f"async_tp={self.config.enable_async_tp}"
            )
            print(f"Mode: {mode_str}")
            if use_compile and not use_cudagraph:
                if self.config.enable_async_tp:
                    print(
                        "CUDA graph capture: disabled for async TP because the compiled "
                        "collective path performs host-side event synchronization"
                    )
                else:
                    print("CUDA graph capture: disabled by --disable-cudagraph")
            ...
            if self.config.profile:
                ...
                profile_name = (
                    f"vllm_torchtitan_{self.config.model_family}_{self.config.model_size}"
                    f"_dp{self.config.dp}_tp{self.config.tp}_ep{self.config.ep}"
                    f"_{compile_tag}_bsz{self.config.batch_size}"
                    f"_maxtok{self.config.max_tokens}"
                )
                self.profile_dir = Path(self.config.profile_dir) / profile_name
                self.profile_dir.mkdir(parents=True, exist_ok=True)
            ...
            engine_kwargs = dict(
                ...
                dtype="bfloat16",
                tensor_parallel_size=self.config.tp,
                data_parallel_size=self.config.dp,
                enable_expert_parallel=self.config.ep > 1,
                distributed_executor_backend="external_launcher",
            ...
            if use_compile:
                ...
                engine_kwargs["compilation_config"] = CompilationConfig(
                    ...
                    cudagraph_mode="PIECEWISE" if use_cudagraph else "NONE",
                    cudagraph_capture_sizes=sorted(
                        {
                            1 << i
                            for i in range((self.config.batch_size - 1).bit_length())
                        }
            ...
            self.sampling_params = SamplingParams(
                ...
                ignore_eos=(
                    self.config.spmd_backend == "spmd_types"
                    and self.config.enable_async_tp
                ),
            )
            print("✓ vLLM TorchTitan model loaded successfully")
        except Exception as e:
...
class TorchTitanNativeBenchmark:
    ...
    def __init__(self, config: BenchmarkConfig):
        ...
        self.spmd_context = None

    def setup(self):
        if self.config.model_family == "deepseek_v3":
            return self._setup_deepseek_v3()
        ...
        try:
            ...
            # Apply TP using parallelize_qwen3 with enable_sp flag
            if world_size > 1:
            ...
            if local_rank == 0:
                print(
                    f"✓ TorchTitan native Qwen3 model loaded successfully ({sp_mode})"
                )
        except Exception as e:
    ...
    def _setup_deepseek_v3(self):
        try:
            print("Loading TorchTitan DeepSeek V3 model directly...")
            ...
            local_rank = int(os.environ.get("LOCAL_RANK", 0))
            expected_world_size = self.config.dp * self.config.tp
            if world_size != expected_world_size:
                raise RuntimeError(
                    f"DP={self.config.dp} and TP={self.config.tp} require "
                    f"{expected_world_size} processes, but torchrun launched "
                    f"{world_size}."
                )
            if self.config.ep not in (1, expected_world_size):
                raise ValueError(
                    "DeepSeek native inference requires --ep=1 or --ep=dp*tp; "
                    f"got DP={self.config.dp}, TP={self.config.tp}, EP={self.config.ep}."
                )
            ...
            model_path = _normalize_model_path(
                self.config.model_path
                or _default_model_path("deepseek_v3", self.config.model_size)
            )
            model_spec = model_registry(self.config.model_size, attn_backend="flex")
            parallelism = ParallelismConfig(
                data_parallel_replicate_degree=1,
                data_parallel_shard_degree=self.config.dp,
                tensor_parallel_degree=self.config.tp,
                expert_parallel_degree=self.config.ep,
                enable_sequence_parallel=not self.config.disable_sp,
                spmd_backend=self.config.spmd_backend,
            ...
            class DeepSeekInferenceConfig:
                parallelism: ParallelismConfig

            model_spec.model.update_from_config(
                config=DeepSeekInferenceConfig(parallelism=parallelism)
            )
            self.tokenizer = HuggingFaceTokenizer(tokenizer_path=model_path)
            ...
            with torch.device("meta"), utils.set_default_dtype(
                TORCH_DTYPE_MAP[training.dtype]
            ):
                self.model = model_spec.model.build()

            # HF tensors have global shapes. Load them before spmd_types turns
            # TP/EP parameters into plain local shards without global metadata.
            self.model.to_empty(device=device)
            with torch.no_grad():
            ...
            if load_hf_weights:
                checkpoint_path = _normalize_model_path(
                    self.config.torchtitan_checkpoint_path or model_path
                )
                assert model_spec.state_dict_adapter is not None
                sd_adapter = model_spec.state_dict_adapter(model_spec.model, model_path)
                state_dict = get_model_state_dict(self.model)
                hf_state_dict = sd_adapter.to_hf(state_dict)
                dcp.load(
                    hf_state_dict,
                ...
                set_model_state_dict(
                    self.model,
                    model_state_dict=sd_adapter.from_hf(hf_state_dict),
                    options=StateDictOptions(strict=False),
                )
            elif local_rank == 0:
                print(
                    "WARNING: Skipping HF weight loading for spmd_types with TP. "
                    "The benchmark uses initialized weights; performance and fusion "
                    "results are valid, but generated outputs are not meaningful."
                )

            self.model = model_spec.parallelize_fn(
                self.model,
                parallel_dims=parallel_dims,
                training=training,
                parallelism=parallelism,
                compile_config=CompileConfig(
                    enable=self.config.use_compile_cudagraph,
                    backend=self.config.compile_backend,
                ),
                ac_config=None,
                dump_folder=self.config.profile_dir,
                skip_dp=True,
            )
            self.model.eval()
            torch.cuda.empty_cache()
            if local_rank == 0:
                print("TorchTitan native DeepSeek V3 model loaded successfully")
        except Exception as error:
            print(f"Failed to load TorchTitan native DeepSeek V3 model: {error}")
            import traceback

            traceback.print_exc()
            raise

    def _forward_model(self, tokens: torch.Tensor) -> torch.Tensor:
        if self.config.model_family != "deepseek_v3":
            return self.model(tokens)

        assert self.spmd_context is not None
        assert self.parallel_dims is not None
        batch_size, seq_len = tokens.shape
        if self.parallel_dims.tp_enabled:
            pad_tokens = (-seq_len) % self.parallel_dims.tp
            if pad_tokens:
                tokens = torch.nn.functional.pad(tokens, (0, pad_tokens))
        padded_seq_len = tokens.shape[1]
        positions = torch.arange(padded_seq_len, device=tokens.device).expand(
            batch_size, -1
        )
        attention_masks = self.model.get_attention_masks(positions)
        with self.spmd_context():
            ...
            placements = tuple(
                ...
                and placement.dim in (-1, logits.ndim - 1)
                else placement
    ...
    def generate_with_timing(
        ...
        logits = self._forward_model(generated_tokens)
    ...
    def run_inference(
        ...
        def run_generation():
            ...
            if self.config.model_family == "deepseek_v3" and self.config.dp > 1:
                assert self.parallel_dims is not None
                dp_rank = self.parallel_dims.get_mesh("batch").get_local_rank()
                local_prompts = prompts[dp_rank :: self.config.dp]
            else:
...
class BenchmarkRunner:
    ...
    def load_prompts(self) -> list[str]:
        ...
        # The compiled spmd_types async-TP path currently assumes even token
        # partitions across TP ranks. Use a uniform prompt workload for this
        # profiling mode so prefill/decode token counts stay TP-divisible.
        batch_size = self.config.batch_size
        if self.config.spmd_backend == "spmd_types" and self.config.enable_async_tp:
    ...
    def run_benchmark(self, benchmark_cls, key: str):
        ...
        if self.config.use_compile_cudagraph:
            cudagraph_mode = (
                "no cudagraph"
                if self.config.enable_async_tp or self.config.disable_cudagraph
                else "cudagraph"
            )
            print(
                f"Mode: compile({self.config.compile_backend}) + {cudagraph_mode}"
            )
        else:
            print("Mode: eager (no compile, no cudagraph)")
        if self.config.enable_async_tp:
            print("Async TP: enabled")
        if self.config.profile:
            print(
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to HuggingFace checkpoint. Defaults by --model-family/--model-size "
            "when known."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        default="qwen3",
        choices=["qwen3", "deepseek_v3"],
        help="TorchTitan model family for vllm-torchtitan (default: qwen3).",
    )
    parser.add_argument(
        "--model-size",
        type=str,
        default="0.6B",
        help=(
            "Model size key for registry, e.g. qwen3: 0.6B; "
            "deepseek_v3: 16B. Default: 0.6B"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="vLLM data parallelism size for vllm-torchtitan (default: 1)",
    )
    parser.add_argument(
        "--ep",
        type=int,
        default=1,
        help=(
            "Expert parallelism size for vllm-torchtitan. Must be 1 or dp * tp "
            "(default: 1)"
        ),
    )
    parser.add_argument(
        ...
        type=str,
        default="default",
        choices=["default", "spmd_types"],
        help="SPMD backend for TorchTitan vLLM wrapper (default: default)",
    )
    parser.add_argument(
        "--enable-async-tp",
        action="store_true",
        default=False,
        help="Enable async tensor parallel fusion in the TorchTitan vLLM wrapper.",
    )
    parser.add_argument(
        "--disable-sp",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep torch.compile enabled by --use-compile-cudagraph, but disable "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable fused vLLM kernels (QKV, RoPE, FFN, attention) for TorchTitan "
            "model. Bypasses DTensor dispatch with direct local weight ops and "
            "vllm::all_reduce."
        ),
    )
    parser.add_argument(
    ...
    # Validate arguments
    if (
        "torchtitan-native" in test_cases
        and args.model_family == "qwen3"
        and args.torchtitan_checkpoint is None
    ):
        parser.error("--torchtitan-checkpoint is required for Qwen3 torchtitan-native.")
    ...
    config = BenchmarkConfig(
        ...
        enable_async_tp=args.enable_async_tp,
