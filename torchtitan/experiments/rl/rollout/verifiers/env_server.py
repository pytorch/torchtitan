...
class VerifiersEnvServer(Configurable):
    """Controller-owned Verifiers EnvServer process."""

    @dataclass(kw_only=True, slots=True)
    class Config(Configurable.Config):
        config_path: str
        """Server-side Verifiers TOML configuration."""

        bind_address: str = "tcp://127.0.0.1:0"
        """ZMQ address on which the EnvServer listens."""
    ...
    async def start(self) -> str:
        """Start the EnvServer as a child process on the controller host.

        TODO: A future launcher may provision the EnvServer on a separate host
        and return its ZMQ address through this boundary. The current
        implementation supports only a locally managed server.
        """
