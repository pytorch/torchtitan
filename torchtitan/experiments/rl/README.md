...
The logger (`observability/vllm_otel_stat_logger.py`) is registered by default via generator's config field `vllm_stat_loggers` and is a **no-op unless you opt in** with an exporter env var. To use it, install the SDK first:

```bash
uv pip install opentelemetry-sdk                       # required
