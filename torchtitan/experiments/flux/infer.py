...
def main():
    ...
    if prompts:
        ...
        padding_size = math.ceil(total_prompts / world_size) - images.shape[0]
        if padding_size > 0:
            ...
            images = torch.cat([images, padding_tensor])
    else:
        # if there are not enough prompts for all ranks, pad with empty tensors
        # Use the image size from config
        img_size = config.training.img_size
        images = torch.zeros(
            math.ceil(total_prompts / world_size),
            3,
            img_size,
            img_size,
            device=trainer.device,
    ...
    # Create a list of tensors to gather results (only on rank 0)
    ...
    # Gather images from all processes to rank 0
    torch.distributed.gather(images, gathered_images, dst=0)

    # re-order the images to match the original ordering of prompts
    if global_id == 0:
        # Get image dimensions from the first gathered tensor
        _, channels, height, width = gathered_images[0].shape
        all_images = torch.zeros(
            size=[total_prompts, channels, height, width],
            dtype=torch.float32,
            device=trainer.device,
        )
        ...
        for i, img in enumerate(pil_images):
            ...
            img.save(path / f"img_{i}.png", exif=exif_data, quality=95, subsampling=0)
