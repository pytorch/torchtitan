...
def filter_dataset_by_indices(dataset: Dataset, indices_to_remove: List[int]) -> Dataset:
    """
...
def filter_low_resolution_batch(batch, output_size):
    ...
    return [img.size[0] >= output_size and img.size[1] >= output_size for img in batch['jpg']]


def resize_images_batch(batch, output_size):
    """Resize and center crop images in batch."""
    processed_images = []
    for img in batch['png']:
        resized_img = F.resize(img, output_size, interpolation=F.InterpolationMode.BICUBIC)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filter_file', type=str, default="torchtitan/experiments/flux/scripts/problematic_indices.txt",
                      help='Filter file (default: torchtitan/experiments/flux/scripts/problematic_indices.txt)')
    parser.add_argument('--output_size', type=int, default=256,
                      help='Output size (default: 256)')
    parser.add_argument('--subset_size', type=int, default=1099776,
                      help='Subset size (default: 1099776)')
    args = parser.parse_args()

    dataset = load_dataset("webdataset", data_dir=args.input_dir, split=f"train[:{args.subset_size}]", num_proc=args.num_workers)
