...
from torchtitan.experiments.evaluation.evaluator.torchtitan_lm import TorchTitanLM

is_main_process = True
accelerate_rank = os.environ.get("RANK")
if accelerate_rank is not None:
    try:
        _is_main_process = int(accelerate_rank) == 0
    except ValueError:
        # Log a warning if RANK is set but not an integer
        # logging.warning(f"Environment variable RANK is set to '{accelerate_rank}', which is not an integer.")
...
def cli_evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if args.wandb_args:
        try:
            ...
            # If wandb_logger is not defined, it means we are not using W&B logging.
            wandb_args_dict = simple_parse_args_string(args.wandb_args)
            wandb_config_args_dict = simple_parse_args_string(args.wandb_config_args)
            wandb_logger = WandbLogger(wandb_args_dict, wandb_config_args_dict)
    ...
    # get model with args.model_args for TorchTitan
    model_args = simple_parse_args_string(args.model_args)
    generator = load_transformer_generator(
        model_type_size=model_args["model_type_size"],
        exp_name=model_args["exp_name"],
        max_seq_len=model_args.get("max_seq_len", 8192),
        args={
            "dtype": model_args.get("dtype", "bf16"),
            "model_type_size": model_args["model_type_size"],
            "compile_prefilling": bool(model_args.get("compile_prefilling", False)),
            "reduce_generation_overhead": bool(model_args.get("reduce_generation_overhead", False)),
        }
    )
    
    model = TorchTitanLM(generator)
    ...
    if results is not None:
        ...
        print(
            f"({args.model_args}), gen_kwargs: ({args.gen_kwargs}), limit: {args.limit}, num_fewshot: {args.num_fewshot}, "
