...
def convert_to_titan_fqns(fqn: str) -> list[str]:
    ...
    if "input_layernorm.weight" in fqn:
    ...
    elif "mlp.gate_proj" in fqn:
        return [f"layers.{layer}.feed_forward.w1.weight"]
    elif "mlp.up_proj" in fqn:
        return [f"layers.{layer}.feed_forward.w3.weight"]
    elif "mlp.down_proj" in fqn:
...
if __name__ == "__main__":
    ...
    https://github.com/pytorch/torchtitan/blob/main/torchtitan/experiments/llama4/scripts/convert_hf_to_dcp_with_gpus.py.
    """
    init_logger()
    ...
    class Checkpoint:
        ...
        """To perfectly match, we need to bring inv_freq from the original model."""
    ...
    try:
        ...
        _ = state_dict.pop("output.weight")
        if "freqs_cis" in state_dict:
            state_dict.pop("freqs_cis")

        if "rotary_emb.inv_freq" in state_dict:
            state_dict.pop("rotary_emb.inv_freq")
        model = AutoModelForCausalLM.from_pretrained(
            config.checkpoint.hf_model_name,
        ...
        # HuggingFace uses weight tying by default.
        output_weight = state_dict["tok_embeddings.weight"]
        state_dict["output.weight"] = output_weight
        state_dict["rotary_emb.inv_freq"] = inv_freq
