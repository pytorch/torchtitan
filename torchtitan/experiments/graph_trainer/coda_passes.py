...
def _canonicalize_primitive_rmsnorm_backward(
    ...
    for grad_weight_mul in list(match.normalized_cast.users):
        ...
        cursor = max(
            (grad, match.norm_input, match.rstd, match.weight),
...
def _canonicalize_primitive_rmsnorm(gm: GraphModule) -> None:
    ...
    for match in matches:
        ...
        width = norm_shape[-1]
        graph_order = {node: index for index, node in enumerate(gm.graph.nodes)}
        cursor = max((match.norm_input, match.weight), key=graph_order.__getitem__)
        with gm.graph.inserting_after(cursor):
...
def _insert_flex_gemm(
    ...
    *,
    ...
    graph_order = {node: index for index, node in enumerate(gm.graph.nodes)}
    latest_input = max(actual_inputs, key=graph_order.__getitem__)
...
def _insert_projection_rmsnorm_fusion(
    ...
    *,
    ...
    graph_order = {node: index for index, node in enumerate(gm.graph.nodes)}
    insertion_input = max(
        [*first_path.root.all_input_nodes, weight], key=graph_order.__getitem__
    )
    ...
    graph_order = {node: index for index, node in enumerate(gm.graph.nodes)}
    cursor = max([weighted_2d, second_rhs, new_rstd_2d], key=graph_order.__getitem__)
    with gm.graph.inserting_after(cursor):
    ...
    fused_second = _call_after(
        ...
        (
            ...
            {},
            _kernel_options(pattern, config_index=1),
        ),
    )
...
def F_mm_residual_rmsnorm(  # noqa: N802
    ...
    for norm in list(gm.graph.nodes):
        ...
        if path is None or not _claim_coda_match(pattern, norm):
            continue
        _insert_forward_rmsnorm_fusion(gm, norm=norm, path=path, pattern=pattern)
        counts[pattern] += 1
...
def B_mm_dx_rmsnorm(  # noqa: N802
    ...
    for node in list(gm.graph.nodes):
        ...
        _insert_backward_rmsnorm_fusion(
            gm, norm_backward=node, path=path, pattern=pattern
        )
        counts[pattern] += 1


@register_coda_pattern(
    best_configs={10: (_quack_config(256, 256, dynamic=False),)},
)
        ...
        try:
            if compile_time_benchmark:
                return apply_benchmarked_rewrites(
                    ...
                    namespace=("coda_flex_gemm", coda_autotune),
