...
- Creates a feature branch from `main`: `graph_trainer/<topic>`
...
1. Push the branch.
2. **Create a draft PR** against `main` with a clear title and description
   summarizing the changes.
3. **Link the PR to the board item** — add the PR URL as a comment on the
   board item, and reference the board item in the PR description.
...
- **Commit prefix**: `[GraphTrainer][AutoDev]` for all graph_trainer commits.
