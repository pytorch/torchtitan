...
Use `--training.steps 20` for benchmarks (first few steps include compilation overhead — look at the last few steps for steady-state performance). The `c4_test` dataset is local and avoids network dependency crashes.
