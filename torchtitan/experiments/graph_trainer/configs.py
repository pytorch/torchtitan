...
class GraphTrainerCompileConfig(CompileConfig):
    ...
    memory_policy: Literal[
        ...
        "greedy",
        "greedy_solver",
        "auto_perf_maxing_greedy",
        "auto_perf_maxing_ilp",
    ] = "default"
    """
    Memory optimization policy for activation management (SAC, offload).
        ...
        recompute_probe: measurement only — save everything, then recompute
            just the node selected by AUTOAC_RECOMPUTE_PROBE. Diff the peak
            against the unset (all-save) baseline to measure one node's true
            contribution.
    """
