...
def plan_and_tag_inner(
    ...
    # )
    _, _, _, _, _, _, candidates_by_layer, layer_act, total_freeable = (
        get_fixed_bytes_tuple
    )
    fixed_bytes = mem_est.peak_bytes - total_freeable
...
def plan_and_tag_outer(
    ...
    _, _, fixed_bytes_max_by_layer, act_bytes_per_layer, storages_by_layer = (
...
def get_fixed_bytes(
    ...
    nodes = list(gm.graph.nodes)
    indeces_per_node = {n: i for i, n in enumerate(nodes)}
    ...
    for node in nodes:
        ...
        for t in pytree.tree_leaves(node.meta.get("val")):
            ...
            if node in output_inputs or node in persistent_state:
                ...
                death_of[key] = find_last_use_index(node, sid, indeces_per_node)

            last_fwd_index: int = find_last_fwd_use_index(node, sid, indeces_per_node)
            if last_fwd_index < 0:
                last_fwd_index = index
            first_bwd_index = find_first_bwd_use_index(node, sid, indeces_per_node)
            ...
            # node_index = indeces_per_node[node]
...
def find_last_fwd_use_index(node, sid, indeces, _memo=None) -> int:
    ...
    for user in node.users:
        ...
        last_fwd_index = max(last_fwd_index, indeces[user])
        ...
        if _has_sid(pytree.tree_leaves(user.meta.get("val"))):
            last_fwd_index = max(
                last_fwd_index, find_last_fwd_use_index(user, sid, indeces, _memo)
            )
    _memo[node] = last_fwd_index
    return last_fwd_index


def find_first_bwd_use_index(node, sid, indeces, _memo=None) -> int:
    ...
    for user in node.users:
        ...
        if _is_backward_node(user):
            first_bwd_index = min(first_bwd_index, indeces[user])
        ...
        if _has_sid(pytree.tree_leaves(user.meta.get("val"))):  # check outputs
            first_bwd_index = min(
                first_bwd_index,
                find_first_bwd_use_index(user, sid, indeces, _memo),
            )
    _memo[node] = first_bwd_index
    return first_bwd_index


def find_last_use_index(node, sid, indeces, _memo=None):
    ...
    for user in node.users:
        ...
        if not _has_sid(user_in):  # this user does not have sid as input
            continue
        last = max(last, indeces[user])  # count ALL readers
        if _has_sid(pytree.tree_leaves(user.meta.get("val"))):
            last = max(last, find_last_use_index(user, sid, indeces, _memo))
    _memo[node] = last
    return last
...
for node in block_nodes:
    ...
    index = indeces_per_block_node[node]
    for t in pytree.tree_leaves(node.meta.get("val")):
        if not isinstance(t, torch.Tensor) or t.device.type != "cuda":
            continue
        sid = t.untyped_storage()._cdata
        ...
        live_key[sid] = key
        death_of[key] = find_last_use_index(node, sid, indeces_per_block_node)

        last_fwd_index: int = find_last_fwd_use_index(node, sid, indeces_per_block_node)
        first_bwd_index = find_first_bwd_use_index(node, sid, indeces_per_block_node)
        new_object = StorageObject(
            sid=sid,
            size=_get_size(t),
            producer_node=node, # this could also be an int index
