...
def two_level_ilp_greedy(
    ...
    if _rank == 0:
        ...
        if _fr is None:
            ...
            # total_bytes, _ = _measure_tagged_peak(
            #                         _gm,
            #                         trace,
            #                         opt_bytes,
            #                         prefetch_lookahead,
            #                         defer_n_layers,
            #                     )
            # diff = abs(total_bytes - memory_budget)
            # logger.info(f"diff: {diff} diff/M: {diff / MEM_MULTIPLIER}")

            # if diff / MEM_MULTIPLIER >= 1: # safe iff less that 1 GiB
            #     new_budget = memory_budget - (total_bytes - memory_budget)
            #     logger.info(f"new_budget: {new_budget}")
            #     _fr, _tc, _ts = plan_outer(
            #         mem_est,
            #         opt_bytes,
            #         runtime,
...
def plan_outer(
    ...
    # logger.info(f"----------------------------------------------------------------------------------------")
    # logger.info(f"----------------------------------------------------------------------------------------")
    # logger.info(f"calling per_node_memory...")
    # per_node_memory(gm, runtime_estimation_mode, cpu_offload_budget_gb, requested_budget, optimizer, model_parts, trace.num_static_inputs, trace, interp_ctx)
    # logger.info(f"----------------------------------------------------------------------------------------")
    # logger.info(f"----------------------------------------------------------------------------------------")
        ...
        if layer_id != -1:
            for n in blk_nodes:
                if _is_backward_node(n):
                    ...
                    bwd_rt_by_block[layer_id] += runtime_per_node.get(n.name, 0.0)
                else:
                    total_fwd_time += runtime_per_node.get(n.name, 0.0)
                    fwd_rt_by_block[layer_id] += runtime_per_node.get(n.name, 0.0)
        else:
            for n in blk_nodes:
                if _is_backward_node(n):
    ...
    logger.info(f"total_fwd_time: {total_fwd_time}")
    logger.info(f"total_bwd_time: {total_bwd_time}")
    logger.info(f"total_loss_bwd_time: {total_loss_bwd_time}")
    logger.info(f"total_loss_fwd_time: {total_loss_fwd_time}")
    ...
    def _unsharded_bytes(node, sharded_numel: int, unshard_factor: int) -> int:
        """Bytes of ``node``'s all-gathered form, read off the graph.

        A parameter placeholder feeds a short chain -- optional dtype cast,
        all_gather_into_tensor, wait_tensor -- whose output holds the same
        logical tensor unsharded, in the compute dtype. Matching on element
        count rather than dtype keeps this correct whatever mixed-precision
        policy is in force. Returns 0 when no such consumer exists (e.g. the
        parameter is replicated rather than sharded).
        """
        target_numel = sharded_numel * unshard_factor
        best, seen, queue = 0, set(), [(u, 0) for u in node.users]
        while queue:
            u, depth = queue.pop(0)
            if u in seen or depth > 4:
                continue
            seen.add(u)
            for t in pytree.tree_leaves(u.meta.get("val")):
                if not isinstance(t, torch.Tensor) or t.device.type != "cuda":
                    continue
                if t.numel() == target_numel:
                    best = max(best, get_size(t))
            queue.extend((uu, depth + 1) for uu in u.users)
        return best

    seen_sids = set()
    param_bytes = 0
    non_block_param_bytes = 0  # embedding / lm_head: not block-shaped
    lm_head_param_bytes = 0  # sharded, master dtype
    lm_head_param_numel = 0
    lm_head_nodes = []
    for n in persistent_state:
        owner_fqn = _param_owner_fqn(n)
        is_block = owner_fqn.startswith("layers.")
        is_lm_head = owner_fqn.split(".")[0] in ("lm_head", "output")
        for t in pytree.tree_leaves(n.meta.get("val")):
            ...
            if is_lm_head:
                lm_head_param_bytes += sz
                lm_head_param_numel += t.numel()
                lm_head_nodes.append(n)

    logger.info(f"non_block_param_bytes: {non_block_param_bytes / MEM_MULTIPLIER}")
    ...
    # Unsharded lm_head residency during the loss/lm_head backward.
    #
    # loss_peak below prices only sharded layer parameters plus a single
    # layer's unshard allowance -- every term is in units of
    # param_bytes_per_layer, a *transformer layer*. The lm_head is not one: on
    # llama3-8b it is 525M parameters against a layer's 243M, and during its
    # backward both the all-gathered weight (compute dtype) and the unsharded
    # gradient (master dtype) are resident. Neither is priced anywhere, and
    # neither is visible to the FX estimator either, because FSDP allocates
    # them inside bucketing custom ops whose declared fake outputs are views
    # into a flat buffer. The result was a fixed ~2.9 GiB of real memory that
    # no constraint accounted for, independent of the keep/recompute split.
    #
    # The weight is measured off the graph so the compute dtype does not have
    # to be assumed; the gradient is reduced in the master dtype.
    lm_head_unshard_param_bytes = sum(
        _unsharded_bytes(n, lm_head_param_numel, _local_ranks) for n in lm_head_nodes
    )
    lm_head_unshard_grad_bytes = lm_head_param_bytes * _local_ranks
    lm_head_backward_bytes = lm_head_unshard_param_bytes + lm_head_unshard_grad_bytes
    logger.info(
        "loss block lm_head residency: unsharded param %.3f GiB + unsharded grad "
        "%.3f GiB = %.3f GiB (sharded %.3f GiB x %d ranks)",
        lm_head_unshard_param_bytes / MEM_MULTIPLIER,
        lm_head_unshard_grad_bytes / MEM_MULTIPLIER,
        lm_head_backward_bytes / MEM_MULTIPLIER,
        lm_head_param_bytes / MEM_MULTIPLIER,
        _local_ranks,
    )
    ...
    n_peak_cons = 0
    # forward peak layer: 0: 77.40846011042595
    # cpu_offload_prefetch_n_layers: int = 1
    # cpu_offload_defer_n_layers: int = 1

    logger.info(f"act_bytes_per_layer: {act_bytes_per_layer}")

    for L in layer_ids:
        must_save = per_layer_must_keep.get(L, 0)
        aL = max(act_bytes_per_layer[L], 1)
        logger.info(f"Must save bytes for layer {L} is {min(1.0, must_save / aL)}")
        prob += k[L] >= min(1.0, must_save / aL), f"mustkeep_{L}"
    ...
    for L in layer_ids:
        ...
        if L > 0:
            params_total_backward += (
                _local_ranks - 1
            ) * param_bytes_per_layer  # add prev layer's unsharded
        ...
        logger.info(f"act_bytes_per_layer[-1]/M: {act_bytes_per_layer[-1]/M}")
        prob += (
            prev_layer_offload + kept_below + aL + params_total_forward / M
            # + act_bytes_per_layer[-1] / M
            # + 10
            <= B_g
        ), f"fwd_peak_{L}"
        logger.info(
            f"forward peak layer: {L}: {prev_layer_offload + kept_below + aL + params_total_forward / M}"
        )
        ...
        total_grad_bytes = grad_size_per_layer.get(-1, 0) + grad_size_per_layer.get(
            L, 0
        )

        logger.info(f"grad_size_per_layer.get(-1, 0): {grad_size_per_layer.get(-1, 0)}")
        if L < last_layer_id - 1:
            for sharded_layer in range(L + 2, last_layer_id + 1):
                total_grad_bytes += grad_size_per_layer[sharded_layer]
        if L < last_layer_id:
            total_grad_bytes += 2 * _local_ranks * grad_size_per_layer[L + 1]

        # if L == last_layer_id:
        #     total_grad_bytes += (
        #         2 * grad_size_per_layer.get(-1, 0) * (_local_ranks - 1)
        #     )  # lm head unsharding

        total_grads = 0
        if L < last_layer_id:
            # Gradient reduce-scatter staging, invisible to the graph we model:
            # joint_transformer_block_bucketing_reordering_pass runs after this
            # solver, and bucketing::_pre_bucket_reduce_scatter then cats a
            # bucket's gradients into a *fresh* flat tensor -- one full
            # unsharded bucket, i.e. _local_ranks x the sharded gradient, not
            # (_local_ranks - 1). The reordering overlaps buckets, so
            # RS_INFLIGHT of them are alive at once at the backward peak.
            total_grads = (_local_ranks - 1) * grad_size_per_layer[L] / M

        prob += (
            kept_below + aL + bwd_temp_max_per_layer[L] / M  # recompute term is new
            # + (_local_ranks - 1) * param_bytes_per_layer / M  # AG buckets
            # + total_grads
            # + _local_ranks * grad_size_per_layer[L] / M  # RS staging
            + len(layer_ids) * param_bytes_per_layer / M + prefetch_activation
            <= B_g
        ), f"bwd_peak_{L}"
        logger.info(f"fixed_max_bwd: {L}: {fixed_max_bwd.get(L, 0) / M}")
        logger.info(f"recompute_working_set: {L}: {recompute_working_set.get(L, 0)}")
        bwd_memory_constraint = (
            kept_below
            + aL
            + bwd_temp_max_per_layer[L] / M  # recompute term is new
            + len(layer_ids) * param_bytes_per_layer / M
            # + total_grads
            # + (_local_ranks - 1) * param_bytes_per_layer / M  # AG bucketsbut
            # + _local_ranks * grad_size_per_layer[L] / M  # RS staging
            + prefetch_activation
        )
        _bwd_exprs[L] = bwd_memory_constraint
        logger.info(f"bwd peak layer: {L}: {bwd_memory_constraint}")
    ...
    logger.info(
        f"loss breakdown; all_mem_at_peak: {all_mem_at_peak} params: {params/M} last_layer_id: {last_layer_id}"
    )
    prob += (
        kept_below + all_mem_at_peak + params / M  # +lm_head_backward_bytes / M
        # + o[last_layer_id] * act_bytes_per_layer[last_layer_id] / M
        <= B_g
    ), "loss_peak"

    # Must mirror the constraint exactly: a dropped term makes the SOLVED
    # report rank an incomplete loss value against the real constraint and
    # print phantom slack, the same trap called out at bwd_peak_L.
    loss_constraint = (
        kept_below
        + all_mem_at_peak
        + params / M
        + lm_head_backward_bytes / M
        # + o[last_layer_id] * act_bytes_per_layer[last_layer_id] / M
    )

    logger.info(f"loss_constraint: {loss_constraint}")

    logger.info(
        f"loss constraint: all_mem_at_peak: {all_mem_at_peak} params: {params/M} last_layer_id: {last_layer_id}"
    )
    ...
    if LpStatus[status] == "Optimal":
        ...
        for kind, L, v in rows:
            logger.info(
                "SOLVED %s_peak_%-3s = %7.3f GiB   slack %+7.3f", kind, L, v, B_g - v
            )
    ...
    # DEBUG (local): the prefix is "fwd_peak_"/"bwd_peak_"/"loss_peak", never
    # "peak_", so the original filter matched nothing and left the sentinel.
    ...
    _rows = sorted(
        (
            (_c.value() + B_g, _cn)
            for _cn, _c in prob.constraints.items()
            if "peak" in _cn and _c.value() is not None
        ),
        reverse=True,
    )
    logger.info("ABL status=%s objective=%s", LpStatus[status], value(prob.objective))
    for _v, _cn in _rows[:6]:
        logger.info("ABL tight: %-16s value=%8.3f  B_g=%8.3f  slack=%8.3f",
                    _cn, _v, B_g, B_g - _v)
    _ws_val = 0.0  # value(ws) or 0.0
    _dbg(
    ...
    _dbg(
        "Outer LP | budget=%.2f eff=%.2f | act=%.2f GiB -> "
    ...
    _dbg(
        "outer LP decisions for fractions per-layer:\n",
    )
    for b in layer_ids:
        _dbg(
...
def plan_and_tag_inner(
    ...
    # Hoisted out of the grouped branch: the per-layer branch calls
    # _recompute_ms, _tag_collective_chain and _propagate_to_getitem_view too,
    # and when they were nested inside `if not each_layer_separately:` every
    # per-layer solve died with UnboundLocalError. Bodies are unchanged.
    ...
    def _knapsack(nodes, limit, size_of, cost_of, sense, unit=1 << 20):
        """Exact 0/1 knapsack over a `unit`-quantised size axis.

        sense "at_least": cheapest subset whose sizes sum to at least `limit`,
            tightest sum first. Frees memory for the least added runtime.
        sense "at_most": most valuable subset whose sizes sum to at most
            `limit`. Spends leftover keep budget on the dearest recomputes.

        Rank-order first-fit cannot do either: it takes a node only if the node
        fits in what is left, so it stops short of an "at_least" target and
        leaves an "at_most" budget partly unspent.

        Candidate sizes here are multiples of a few MiB, so 1 MiB quantisation
        is exact in practice. Nodes below one unit are free: always taken under
        "at_most", ignored under "at_least". Returns [] if nothing qualifies.
        """
        if limit <= 0:
            return []
        free = [n for n in nodes if int(size_of(n)) < unit]
        items = [
            (int(size_of(n)) // unit, cost_of(n), n)
            for n in nodes
            if int(size_of(n)) >= unit
        ]
        ...
        if sense == "at_most":
            ...
            for q, c, _ in items:
                prev = rows[-1]
                cur = prev[:]
                for s in range(q, cap + 1):
                    if prev[s - q] + c > cur[s]:
                        cur[s] = prev[s - q] + c
                rows.append(cur)
            end = max(range(cap + 1), key=lambda s: (rows[-1][s], -s))
        else:
            if not items:
                return []
            # Floor, not ceil: `limit` carries float residue from kf * act_g,
            # and rounding up put a 3.50001 GiB target one unit beyond the
            # 3.5 GiB node that covers it, making the solve infeasible.
            # Under-freeing by less than a unit is harmless.
            need = int(limit) // unit
            cap = need + max(q for q, _, _ in items)
            INF = float("inf")
            rows = [[INF] * (cap + 1)]
            rows[0][0] = 0.0
            for q, c, _ in items:
                ...
                for s in range(q, cap + 1):
                    if prev[s - q] + c < cur[s]:
                        cur[s] = prev[s - q] + c
                rows.append(cur)
            end = next((s for s in range(need, cap + 1) if rows[-1][s] < INF), None)
            if end is None:
                return []

        chosen, s = [], end
        for i in range(len(items) - 1, -1, -1):
            if rows[i + 1][s] != rows[i][s]:
                chosen.append(items[i][2])
                s -= items[i][0]
        return chosen + (free if sense == "at_most" else [])

    each_layer_separately = True
    ...
    if not each_layer_separately:
        ...
        def build_inner_ilp(candidates, kf, of, remaining_off_g, act):
            """
            candidates: candidates for the given layer
            kf: keep fraction (between 0 and 1); how much of the total layer activation memory to be kept
            of: offload fraction (between 0 and 1); how much of the total layer activation memory to be offloaded
            remaining_off_g: remaining offload budget
            act: activation memory for this layer
            """
            ...
            offload_budget = of * act_g
            keep_budget = kf * act_g
            recompute_budget = (1 - of - kf) * act_g
            logger.info(
                f"requested keep: {keep_budget/M:.2f}, offload: {offload_budget/M:.2f}, recompute: {recompute_budget/M:.2f}"
            )
            ...
            ranked_activations = sorted(
                ...
                key=lambda n: (
                    -runtime_per_node.get(n.name, 0.0)
                    / (all_candidates_by_size[n] / M),
                    n.name,
                ),
            )

            def _fill(gap, ordered, decision, allow_overshoot=False):
                """Move up to `gap` bytes out of keep, in `ordered` preference.

                Every pass here is the same problem: free a target number of
                bytes for the least runtime cost. Plain rank-order first-fit
                gets it wrong in two directions -- it abandons the tail of the
                budget (every remaining node is too big for what is left), and
                when it does take a node it takes an arbitrarily large one for
                a small gap.

                `allow_overshoot` decides what happens to the remainder, and
                the right answer differs by pass. Offload and recompute must
                stop short: exceeding the offload target spends pinned host
                pool and D2H bandwidth the outer LP never priced, and whatever
                is left over is picked up by the next pass anyway. The final
                keep-closing pass is the opposite -- it *must* cross the line,
                because keeping more than planned is what breaks the memory
                budget, so there it takes the smallest node that covers the
                remainder and the overshoot is bounded by that node.

                Returns the bytes actually placed.
                """
                placed = 0.0
                if gap <= 0:
                    return placed
                ...
                if rem > 0 and allow_overshoot:
                    ...
                    if over:
                        # best = min(
                        #     over, key=lambda n: (all_candidates_by_size[n], n.name)
                        # )
                        best = min(
                            over,
                            key=lambda n: (
                                _recompute_ms(n),
                                all_candidates_by_size[n],
                                n.name,
                            ),
                        )
                        overshoot = all_candidates_by_size[best] - rem
                        if (
                            overshoot < rem
                        ):  # taking it lands closer to target than leaving it
                            decisions_per_act[best] = decision
                            placed += all_candidates_by_size[best]

                return placed

            # Offload the activations that are most expensive to recompute
            # (highest runtime per byte, i.e. the head of the ranking): those
            # are the ones the recompute pass below would charge most for.
            # remaining_off_g is GiB and the budgets here are bytes.
            ...
            recompute_order = [
                n
                for n in reversed(ranked_activations)
                if _is_recomputable(n, must_keep)
            ]
            achieved_recompute = _fill(recompute_budget, recompute_order, 3)

            logger.info(
                f"achieved  keep: {(act_g - achieved_recompute - achieved_offload)/M:.2f} offload: {achieved_offload/M:.2f}, recompute: {achieved_recompute/M:.2f}"
            )

            # Anything the two passes could not place stays resident, so keep
            # can still sit above what the outer LP asked for -- and keeping
            # more than planned is the direction that breaks the memory budget,
            # since bwd_peak_L prices kept activations of the layers below.
            # Close that gap with recompute.
            kept_act_budget = act_g - achieved_recompute - achieved_offload
        ...
        for b, candidates in candidates_by_layer.items():
            logger.info(
                f"Going over the candidate layer: {b}: candidates: {candidates}"
            )
        ...
        _dbg(
            f"We want to achieve overall ratios: keep: {total_keep/num_layers:.4f} offload: {total_offload/num_layers:.4f} recompute: {total_recompute/num_layers:.4f}"
        )
        ...
        for sig, (candidates, kf, of, sig_act) in sig_meta.items():
            ...
            k, r, o = build_inner_ilp(candidates, kf, of, remaining_off_g, sig_act)
            # The pinned host pool is global, but this loop solves one
            # *signature* and the resulting pattern is stamped onto every layer
            # sharing it -- so charge the pool for all of them, not once.
            # Without this every layer saw the full pool and the cap never bound.
            _off_bytes = sum(s for n, s in candidates if o.get(n, 0.0) > 0.5)
            ...
            logger.info(
                f"budgets offload: {of*sig_act / M:.2f} keep: {kf*sig_act / M:.2f} GiB"
            )
            for node, size in candidates:
                logger.info(
                    f"_recompute_ms(node): {_recompute_ms(node)} for node: {node.name} decision: {_tag_of(k[node], r[node], o[node])} size: {size / M}"
                )
            logger.info(
                f"################################################################################################################"
            )
        ...
        for node in gm.graph.nodes:
            ...
            if (
                ...
                and upstream_collective in per_node_decision
            ):
        ...
        # Accumulates across every layer, like the grouped branch.
        per_node_decision = {}
        total_keep = 0
        total_recompute = 0
        total_offload = 0

        total_achieved_keep = 0
        total_achieved_recompute = 0
        total_achieved_offload = 0
        total_activations = 0
        ...
        for b, candidates in candidates_by_layer.items():
            ...
            # Same lookup the grouped path does. Without it this branch read a
            # bare `of`/`kf`, which only exist in the grouped branch's loop, so
            # every per-layer solve raised UnboundLocalError at `of * act_g`.
            if per_layer_fractions is not None:
                _li = int(b.split(".")[1]) if isinstance(b, str) else b
                kf, _rf, of = per_layer_fractions.get(
                    _li, (keep_fraction, recompute_fraction, offload_fraction)
                ...
                kf, of = keep_fraction, offload_fraction
            _dbg(
                f"layer {b}: keep {kf:.4f} recompute {1 - kf - of:.4f} offload {of:.4f}"
            )
            ...
            def _fill(gap, ordered, decision, allow_overshoot=False):
                ...
                for n in ordered:
                    ...
                    if (
                        size_n <= gap - placed
                    ):  # + MEM_MULTIPLIER:  # allow 0.25GB ceiling
                        decisions_per_act[n] = decision
                        placed += size_n
                rem = gap - placed
                if rem > 0 and allow_overshoot:
                    ...
                    if over:
                        ...
                        for node in over:
                            if rem < 0:
                                break
                            if decisions_per_act[node] != 0:  # skip already-decided
                                continue
                            decisions_per_act[node] = decision
                            placed += all_candidates_by_size[node]
                            rem -= all_candidates_by_size[node]
                        # overshoot = all_candidates_by_size[best] - rem
                        # if (
                        #     overshoot < MEM_MULTIPLIER / 2
                        # ):  # taking it lands closer to target than leaving it
                        # decisions_per_act[best] = decision
                        # placed += all_candidates_by_size[best]
            ...
            # all_candidates_by_size is a defaultdict, so read it explicitly
            # rather than passing .get, which would return None for a miss.
            def _size_of(n):
                return all_candidates_by_size[n]

            # 3a: keep above budget -> free at least the difference
            if kept_act_budget > keep_budget:
                _pool = [n for n in recompute_order if decisions_per_act[n] == 0]
                _freed = _knapsack(
                    _pool,
                    kept_act_budget - keep_budget,
                    _size_of,
                ...
                # The pool cannot always reach the target -- must_keep and
                # non-recomputable tensors put a floor under this layer's keep.
                # Fall back to the whole pool, which gets as close as the layer
                # allows; returning nothing would leave keep where pass 2 left
                # it, which is how a 1.00 GiB budget ended up holding 4.50.
                for n in _freed or _pool:
                    decisions_per_act[n] = 3
                    achieved_recompute += all_candidates_by_size[n]
                kept_act_budget = act_g - achieved_recompute - achieved_offload

            # 3b: keep below budget -> buy back the dearest recomputes
            if kept_act_budget < keep_budget:
                for n in _knapsack(
                    [n for n, d in decisions_per_act.items() if d == 3],
                    keep_budget - kept_act_budget,
                    _size_of,
                    _recompute_ms,
                    "at_most",
                ):
                    decisions_per_act[n] = 1
                    achieved_recompute -= all_candidates_by_size[n]
            ...
            ach = {"keep": 0, "recompute": 0, "offload": 0}
            for node, size in candidates:
                tag = _tag_of(k, r, o, node)
                # No pattern lookup here: the grouped branch stamps a solved
                # signature onto many layers, but this branch solved THIS layer,
                # so k/r/o already hold its decisions.
                per_node_decision[node] = tag
                ach[tag] += size
            ...
            logger.info(
                f"layer: {b} requested budgets offload: {of*act_g / M:.2f} keep: {kf*act_g / M:.2f} recompute: {(act_g - of*act_g + kf*act_g)/M:.2f} GiB"
            )
            achieved_recompute = 0
            achieved_keep = 0
            achieved_offload = 0
            for node, size in candidates:
                logger.info(
                    f"_recompute_ms(node): {_recompute_ms(node)} for node: {node.name} decision: {_tag_of(k, r, o, node)} size: {size / M}"
                )
                if _tag_of(k, r, o, node) == "keep":
                    achieved_keep += size
                elif _tag_of(k, r, o, node) == "offload":
                    achieved_offload += size
                else:
                    achieved_recompute += size
            logger.info(
                f"layer: {b} achieved budgets offload: {achieved_offload / M:.2f} keep: {achieved_keep / M:.2f} recompute: {achieved_recompute/M:.2f} GiB"
            )
            logger.info(
                f"################################################################################################################"
            )

            total_achieved_keep += achieved_keep
            total_achieved_recompute += achieved_recompute
            total_achieved_offload += achieved_offload

        logger.info(
            f"Requested total keep: {total_keep/num_layers:.2f} recompute: {total_recompute/num_layers:.2f} offload: {total_offload/num_layers:.2f}"
        )

        logger.info(
            f"Achieved total keep: {total_achieved_keep/total_activations*100:.2f} recompute: {total_achieved_recompute/total_activations*100:.2f} offload: {total_achieved_offload/total_activations*100:.2f}"
        )
...
def get_fixed_bytes(
    ...
    for node in nodes:
        ...
        for t in pytree.tree_leaves(node.meta.get("val")):
            ...
            # #     category = GRAD
                ...
                if death_of[key] >= end:
                    category = GRAD
                # elif not (prod.op != "call_function"):
                #     tgt = str(prod.target)
                #     if (
                #         "reduce_scatter" not in tgt
                #         and "wait_tensor" not in tgt
                #         and "reload" not in tgt
                #         and "all_gather" not in tgt
                #     ):
                else:
                    # logger.info(f"for backward temp node: {prod}")
                    category = BWD_TEMP
    ...
    for object in storages:
        if object.category != BWD_TEMP and object.category != GRAD:
            continue
    ...
    # def bwd_working_set_per_layer(nodes, storages):
    #     # BWD_TEMP is wider than the gap in plan_outer: it also holds the transient
    #     # unsharded gradients (charged again by total_grad_bytes as
    #     # _local_ranks * grad_size_per_layer[L+1]), the remat duplicates (charged by
    #     # r[L] * aL) and the offload landing buffers (charged by prefetch_activation).
    #     # Excluding those leaves the working set of the backward op itself, which is
    #     # the only part no constraint term pays for.
    #     already_charged = set()
    #     for n in nodes:
    #         if n.op != "call_function":
    #             continue
    #         tgt = str(n.target)
    #         if "reduce_scatter" in tgt:
    #             already_charged.update(n.all_input_nodes)  # the unsharded gradient
    #         elif "ao.reload" in tgt:
    #             already_charged.add(n)
    #         elif "_recomputed" in n.name:
    #             already_charged.add(n)

    #     # NEW: also the sharded gradient coming OUT of the reduce_scatter, and the
    #     # wait/alias chain carrying it. total_grad_bytes already pays for these via
    #     # grad_size_per_layer -- including grad_size_per_layer[-1] for the lm_head,
    #     # whose shard lands in BWD_TEMP because it dies into the tied-weight
    #     # accumulate instead of surviving to the return.
    #     frontier = [
    #         n
    #         for n in nodes
    #         if n.op == "call_function" and "reduce_scatter" in str(n.target)
    #     ]
    #     while frontier:
    #         n = frontier.pop()
    #         for u in n.users:
    #             if u in already_charged:
    #                 continue
    #             if _is_view(u) or "wait_tensor" in str(u.target):
    #                 already_charged.add(u)
    #                 frontier.append(u)

    #     bwd_temp_add_at, bwd_temp_remove_at = defaultdict(list), defaultdict(list)
    #     for object in storages:
    #         if object.category != BWD_TEMP or object.producer_node in already_charged:
    #             continue
    #         bwd_temp_add_at[object.produced_index].append(object)
    #         bwd_temp_remove_at[object.death_index].append(object)
    ...
    #     bwd_temp_max_per_layer = defaultdict(int)
    #     for i, node in enumerate(nodes):
    #         # Advance at every index. Skipping non-backward nodes drops the births
    #         # and deaths landing on them, so cur ratchets up and the max is junk.
    #         for b in bwd_temp_add_at.get(i, ()):
    #             cur += b.size  # born at i -> live during i
    #         L = _get_layer_id(node)
    #         if _is_backward_node(node) and L >= 0 and cur > bwd_temp_max_per_layer[L]:
    #             bwd_temp_max_per_layer[L] = cur
    #             bwd_temp_argmax[L] = (i, node.name)
    #         for b in bwd_temp_remove_at.get(i, ()):
    #             cur -= b.size  # dies at i -> still live *during* i
    #     if cur != 0:
    #         logger.warning(
    #             "bwd working set leaked %d bytes; liveness is inconsistent", cur
    #         )

    #     for L in sorted(bwd_temp_argmax):
    #         i, name = bwd_temp_argmax[L]
    #         members = sorted(
    #             (
    #                 o
    #                 for o in storages
    #                 if o.category == BWD_TEMP
    #                 and o.producer_node not in already_charged
    #                 and o.produced_index <= i <= o.death_index
    #             ),
    #             key=lambda o: -o.size,
    #         )[:5]
    #         logger.info(
    #             "bwd working set L%-3d %.2f GiB at %s: %s",
    #             L,
    #             bwd_temp_max_per_layer[L] / MEM_MULTIPLIER,
    #             name,
    #             ", ".join(
    #                 f"{o.producer_node.name}={o.size / MEM_MULTIPLIER:.2f}"
    #                 for o in members
    #             ),
    #         )

    # bwd_working_set_per_layer(nodes, storages)
