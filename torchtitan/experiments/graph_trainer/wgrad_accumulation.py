...
"""Lower annotated graph-owned accumulation into compatible WGrad producers.

Fusion rules have one contract::

    annotated add_(accumulator, wgrad_producer) -> accumulating producer

The annotation, not an FX traversal, identifies the parameter gradient. The
producer must directly feed the accumulation and have no other users. Each
supported producer registers one lowering in ``_WGRAD_FUSION_RULES``;
unsupported producers retain the explicit ``add_``. A successful lowering
retargets the existing producer in place and removes only that ``add_``.
...
def insert_graph_gradient_accumulation(
    ...
    *,
    ...
    if num_param_grads > len(outputs):
        raise ValueError(
            "Parameter gradient count exceeds backward graph outputs: "
            f"{num_param_grads} > {len(outputs)}"
        )

    grad_outputs = outputs[:num_param_grads]
    ...
    output_node.args = (tuple([*accumulated_outputs, *outputs[num_param_grads:]]),)
    gm.graph.lint()
    gm.recompile()
...
def _fuse_scaled_mm_sink(
    ...
    producer.args = (accumulator, *producer.args[:4])
    producer.kwargs = {"use_fast_accum": use_fast_accum}
