...
def plan_outer(
    ...
    for L in layer_ids:
        ...
        prefetch_activation = 0  # o[L] * act_bytes_per_layer[L] / M
        if L > 0:
            prefetch_activation += o[L - 1] * act_bytes_per_layer[L - 1] / M

        total_grad_bytes = grad_size_per_layer.get(-1, 0)
        # grad_size_per_layer.get(L, 0) +
        ...
        if L < last_layer_id:
            total_grad_bytes += _local_ranks * grad_size_per_layer[L + 1]  # 2 *
    ...
    prob += (
        kept_below + all_mem_at_peak + params / M
        # + (2 * grad_size_per_layer.get(-1, 0) * (_local_ranks)) / M  # lm head unsharding
...
def plan_and_tag_inner(
    ...
    if not each_layer_separately:
        ...
        def build_inner_ilp(candidates, kf, of, remaining_off_g, act):
            ...
            total_keep = lpSum(k[node] * (size / M) for node, size in candidates)
            total_off = lpSum(o[node] * (size / M) for node, size in candidates)
            total_rec = lpSum(r[node] * (size / M) for node, size in candidates)
            objective = lpSum(
                r[node] * _recompute_ms(node) for node, size in candidates
            )
            ...
            prob += off_short >= total_off - target_off_g, f"offload_uplimit"
            prob += (
                off_short <= target_off_g * 0.01,
                f"1_%_allowed_only",
            )  # only 5% above the offload budget
            ...
            prob += objective  # minimize the objective
