...
def one_level_ilp(
    ...
    if _rank == 0:
        ...
        gm, _tc, _ts = plan_and_tag_one_level(
            mem_est,
            opt_bytes,
            runtime,
...
def plan_and_tag_one_level(
    ...
    prob = LpProblem("one_level", LpMinimize)

    k, r, o = {}, {}, {} # per-act candidate decision variable
    for layer_id, candidates in candidates_by_layer.items():
        for node, size in candidates:
            k[node] = LpVariable(f"k_{node}", cat=LpBinary)
            r[node] = LpVariable(f"r_{node}", cat=LpBinary)
            o[node] = LpVariable(f"o_{node}", cat=LpBinary)
            prob += k[node] + r[node] + o[node] == 1, f"split_{node}"

            if node in must_keep:
                prob += k[node] == 1, f"mustkeep_{node}"
            if not _can_offload_node(node):
                prob += o[node] == 0, f"nooff_{node}"
            if not _is_recomputable(node, must_keep):
                prob += r[node] == 0, f"norec_{node}"

            # if layer_id == last_layer_id:
            #     prob += o[node] == 0, f"nolastoff_{node}"
    ...
    for L in layer_ids:
        ...
        kept_below = lpSum(
            ...
            for l in layer_ids
            if l < L
            for act, size in candidates_by_layer[l]
        )
        ...
        # The D2H wait is deferred defer_n_layers layers past the producer, so
        # those layers' offloaded bytes have not left the device yet.
        prev_layer_offload = lpSum(
            (o[act] * size) / M
            for l in range(max(L - defer_n_layers, 0), L)
            for act, size in candidates_by_layer.get(l, ())
        )
    ...
    last_layer_offload = lpSum(
                            (o[act] * size) / M for act, size in candidates_by_layer[last_layer_id]
                        )
    logger.info(
        f"loss breakdown; all_mem_at_peak: {all_mem_at_peak} params: {params/M} last_layer_id: {last_layer_id}"
    )
    logger.info(f"last_layer_offload: {last_layer_offload}")
    prob += (
        kept_below
        + all_mem_at_peak
        + params / M
    ...
    # backward:
    for L in reversed(layer_ids):
        aL = act_bytes_per_layer[L] / M

        # Backward runs layers descending, so by the time we reach L every
        # layer above it has been freed and every layer below it is still
        # waiting its turn. Layer L itself is not included here because aL
        # already charges all of its activations.
        kept_below = lpSum(
            (k[act] * size) / M
            for l in layer_ids
            if l < L
        ...
        # One layer of reload prefetch: while layer L runs, layer L-1's
        # offloaded activations are already coming back. Layer L's own reloaded
        # bytes are not added here -- they are part of aL.
        prefetch_activation = lpSum(
            (o[act] * size) / M
            for l in range(max(L - 1, 0), L)
            for act, size in candidates_by_layer.get(l, ())
        )

        total_grad_bytes = grad_size_per_layer.get(L, 0)
        ...
        if L < last_layer_id - 1:
            for sharded_layer in range(L + 2, last_layer_id + 1):
                total_grad_bytes += grad_size_per_layer.get(sharded_layer, 0)
        if L < last_layer_id:
            total_grad_bytes += _local_ranks * grad_size_per_layer.get(L + 1, 0)
        ...
        prob += (
            kept_below
            + aL
            + total_grad_bytes / M + params_total_backward / M + prefetch_activation
            <= B_g
        ), f"bwd_peak_{L}"
    ...
    offloaded_acts = lpSum(
        (o[act] * size) / M
        for l in layer_ids
        for act, size in candidates_by_layer[l]
    )

    prob += (offloaded_acts <= bw_d2h_g * total_fwd_time)
    prob += (offloaded_acts <= bw_h2d_g * total_bwd_time)

    logger.info(
        f"layer: {b}, bw_d2h_g: {bw_d2h_g}, total_fwd_time: {total_fwd_time}"
    )
    logger.info(
        f"layer: {b}, bw_h2d_g: {bw_h2d_g}, total_bwd_time: {total_bwd_time}"
    )

    prob += (
        offloaded_acts <= host_cap_gib,
        "host_cap",
    )
    logger.info("outer: host pinned-memory cap %.2f GiB/rank", host_cap_gib)

    recomputation_time = lpSum(
        (r[act] * runtime_per_node.get(act.name, 0.0))
        for l in layer_ids
        for act, size in candidates_by_layer[l]
    )

    # PuLP treats an expression with no comparison as the objective, and the
    # objective takes no name. Naming it made this a 2-tuple, added twice.
    prob += recomputation_time
    ...
    status = prob.solve(PULP_CBC_CMD(msg=0, timeLimit=720))
    ...
    for layer_id, candidates in candidates_by_layer.items():
        for node, size in candidates:
            tag = _tag_of(k[node], r[node], o[node])
