...
def _trace_flex_decoder_block_stage(
    *,
    batch_size: int = 2,
    seq_len: int = 128,
    include_input_grad: bool = True,
) -> _FlexDecoderBlockTrace:
    """Trace a real Llama3 FlexAttention decoder block as a GraphPP stage.
    ...
    rather than a maskless SDPA shortcut.
    """
    if not torch.cuda.is_available():
        raise unittest.SkipTest("FlexAttention decoder-block tracing requires CUDA")

    maybe_register_blockmask_pytree_node()
    torch.manual_seed(0)

    with _stable_flex_attention_compile_config():
        model_spec = llama3_model_registry("debugmodel", attn_backend="flex")
        ...
        output_grad = torch.randn_like(block(x, attention_masks, positions))
...
class GraphPPPartitionTest(unittest.TestCase):
    def test_real_flex_decoder_block_partition_matches_joint_graph(self) -> None:
