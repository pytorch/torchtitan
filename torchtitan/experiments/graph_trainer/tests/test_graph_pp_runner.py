...
class GraphRuntimeTraceTest(unittest.TestCase):
    ...
    def test_precompile_requires_simple_spmd_schedule(self) -> None:
        parallel_dims = types.SimpleNamespace(pp_enabled=False, fsdp_enabled=False)
        with self.assertRaisesRegex(ValueError, "FULL_FORWARD_BACKWARD"):
            make_graph_runtime(
                [mock.Mock()],
                num_microbatches=2,
    ...
    def test_joint_stage_graphs_bind_runtime_meshes(self) -> None:
        ...
        with mock.patch(
            "torchtitan.experiments.graph_trainer.graph_pp.graph_builder.run_traced"
        ) as run_traced_mock:
            GraphTrainerJointStageGraphs(
                traced=traced,
                module=module,
