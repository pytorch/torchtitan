...
class TestGraphGradientAccumulation(unittest.TestCase):
    def test_aot_fx_runs_as_one_stage_pipeline(self):
        ...
        pipeline_runner = MagicMock(return_value=torch.tensor(3.0))
        ...
        trainer.config = SimpleNamespace(
            ...
            training=SimpleNamespace(max_context_length=8),
        )
        ...
        trainer._graph_pipeline_runner = pipeline_runner
        microbatches = [
        ...
        loss = trainer.forward_backward_step(
            input_dict=microbatches,
            global_valid_tokens=torch.tensor(4),
        )

        self.assertEqual(loss, torch.tensor(3.0))
        self.assertEqual(trainer.ntokens_seen, 4)
        pipeline_runner.assert_called_once()

    def test_aot_fx_single_microbatch_uses_graph_pipeline(self):
        from contextlib import nullcontext
        from types import SimpleNamespace
        from unittest.mock import MagicMock
        ...
        model = SimpleNamespace(
            preprocess_inputs=lambda batch, **kwargs: (
                batch["input"],
                batch["labels"],
                {},
            )
        )
        graph_pipeline_runner = MagicMock(return_value=torch.tensor(2.0))
        trainer = object.__new__(GraphTrainer)
        trainer.config = SimpleNamespace(
            compile=SimpleNamespace(mode="aot_fx_trace"),
            parallelism=SimpleNamespace(),
        ...
        trainer.train_context = nullcontext
        trainer.dataloader = SimpleNamespace(max_num_documents=None)
        trainer._graph_pipeline_runner = graph_pipeline_runner

        loss = trainer.forward_backward_step(
            input_dict={"input": torch.ones(2), "labels": torch.ones(2)},
            global_valid_tokens=torch.tensor(2),
        )

        self.assertEqual(loss, torch.tensor(2.0))
        prepared, _ = graph_pipeline_runner.call_args.args
...
class TestTraceFSDP(FSDPTest):
    ...
    def _run_one_stage_graph_pipeline_fsdp_case(
        self,
        *,
        num_microbatches: int,
        defer_gradient_sync: bool,
    ) -> None:
        ...
        runtime = graph_train_step_runtime(
            ...
            compile_config=GraphTrainerCompileConfig(
                enable=False,
                enable_passes=False,
                enable_deferred_fsdp_gradient_sync=defer_gradient_sync,
            ),
        ...
        global_valid_tokens = torch.tensor(36, device="cuda")
        ...
        for _ in range(4):
            ...
            for inputs, targets, _ in microbatches:
                loss = loss_fn(model_ref(inputs), targets, global_valid_tokens)
                loss.backward()
                expected_loss += loss.detach()
            actual_loss = runner(tuple(microbatches), global_valid_tokens)
            torch.cuda.synchronize()

            torch.testing.assert_close(actual_loss, expected_loss)
            parameter_test_grads = tuple(
                parameter.grad for parameter in model_test.parameters()
            )
            for parameter_ref, parameter_test_grad in zip(
                model_ref.parameters(), parameter_test_grads, strict=True
            ):
                assert parameter_ref.grad is not None
                assert parameter_test_grad is not None
                torch.testing.assert_close(
                    parameter_test_grad.to_local(),
                    parameter_ref.grad.to_local(),
                )
            if grad_buffers is None:
                grad_buffers = parameter_test_grads
            else:
                for parameter_test_grad, buffer in zip(
                    parameter_test_grads, grad_buffers, strict=True
                ):
                    self.assertIs(parameter_test_grad, buffer)

        stage = runtime.schedule._stages[0]
        assert stage.graphs is not None
        self.assertEqual(stage.graphs.requires_grad_reduction, defer_gradient_sync)
        action_names = [
            action.computation_type.name
            for action in runtime.schedule.pipeline_order_with_comms[0]
        ]
        self.assertEqual(action_names.count("REDUCE_GRAD"), int(defer_gradient_sync))

    def test_one_stage_graph_pipeline_fuses_fsdp_gradient_sync(self):
        self._run_one_stage_graph_pipeline_fsdp_case(
            num_microbatches=3,
            defer_gradient_sync=False,
        )

    def test_one_stage_graph_pipeline_defers_fsdp_gradient_sync(self):
        self._run_one_stage_graph_pipeline_fsdp_case(
            num_microbatches=3,
        ...
        *,
        ...
        try:
            with tempfile.TemporaryDirectory() as dump_folder:
                ...
                assert trainer._graph_pipeline_runner is not None
                stage = trainer._graph_pipeline_runner.runtime.schedule._stages[0]
                assert stage.graphs is not None
                fw_graph = stage.graphs.modules.fw
                code_lines = fw_graph.graph.python_code("self").src.splitlines()
                sdpa_line = next(
                    (
                        idx
                ...
                code = fw_graph.graph.python_code("self").src
                trainer.close()
                trainer = None
                return {
                    "code": code,
