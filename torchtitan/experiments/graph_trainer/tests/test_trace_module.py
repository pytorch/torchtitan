...
# TODO: re-enable once graph_trainer adopts the spmd_types backend. CP
# shardings are declared in ShardingConfig and only that backend applies them,
# so GraphTrainer now rejects context_parallel_degree > 1. The assertion below
# also needs rewriting: it looks for an all_gather before the SDPA op, whereas
# the declarative path all-gathers k/v at the local_map boundary.
