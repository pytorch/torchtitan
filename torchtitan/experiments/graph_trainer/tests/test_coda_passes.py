...
class TestCODAFlexGemmPass(TestCase):
    ...
    def test_compile_time_benchmark_rejects_invalid_rewrite(self):
        ...
        def failing_apply(gm, pattern, *, log_matches=True, benchmark_regions=None):
            del log_matches, benchmark_regions
            cast = next(
  ...
  REJECTED (1):
    ...
    def test_configured_pass_uses_pinned_quack_config(self):
        inputs = (
            torch.randn(8, 16, dtype=torch.bfloat16),
            torch.randn(16, 12, dtype=torch.bfloat16),
        )
