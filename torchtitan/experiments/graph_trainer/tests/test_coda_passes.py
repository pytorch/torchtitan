...
class TestCODAFlexGemmPass(TestCase):
    ...
    def test_forward_residual_norm_benchmark_uses_declared_region(self):
        ...
        with mock.patch.object(
            ...
            side_effect=AssertionError("unexpected inferred region"),
        ):
            result = _apply_benchmarked_coda_pattern(
                gm,
    ...
    def test_compound_activation_validates_all_sites_before_rewriting(self):
        ...
        gm = make_fx(_SwiGLU())(*inputs)
        matmuls = _matmuls(gm)
        matmuls[1].meta["coda_consumed"] = True
        matmuls[1].meta["coda_owner"] = "existing_pattern"
        ...
        self.assertEqual(gm.meta["coda_pattern_counts"]["F_swiglu"], 0)
        self.assertEqual(_flex_gemm_patterns(gm), [])
    ...
    def test_backward_residual_benchmark_does_not_claim_fp32_gemm(self):
        ...
        selection = BenchmarkCandidateSelection(set())
        regions = []

        _CODA_PATTERNS["B_mm_dx_residual_add"].apply(
            gm,
            Counter(),
            regions,
            selection,
    ...
    def test_pattern_registrations_are_self_describing(self):
        ...
        for pattern in patterns:
            ...
            self.assertIn("Match:", pattern.matcher.__doc__)
            self.assertIn("Rewrite:", pattern.matcher.__doc__)
    ...
    def test_compile_time_benchmark_rejects_invalid_rewrite(self):
        ...
        def failing_apply(
            ...
            *,
            ...
            del log_matches, benchmark_regions, autotune
            cast = next(
                node for node in gm.graph.nodes if node.target is aten._to_copy.default
            )
            if coda_passes._claim_coda_match(pattern.name, cast, selection):
  ...
  REJECTED (1):
    ...
    def test_direct_rewrite_mutates_original(self):
        ...
        def matcher(
            ...
            del benchmark_regions, selection, autotune
    ...
    def test_local_benchmark_region_does_not_share_mutable_metadata(self):
        ...
        region = coda_passes._make_local_rewrite_benchmark_region(
            ...
            (neg,),
            "test_pattern",
            lambda _candidate, _candidate_nodes: None,
        )
    ...
    def test_resolves_registry_order(self):
        requested = [
            "B_reshape_bf16_to_fp32",
            "F_router_sigmoid_bias",
            "F_situ",
    ...
    def test_empty_pattern_list_disables_coda(self):
        self.assertEqual(get_coda_pattern_passes([]), [])
    ...
    def test_rejects_unknown_and_duplicate_patterns(self):
        ...
        with self.assertRaisesRegex(ValueError, "Unknown.*f3-dense"):
            get_coda_pattern_passes(["f3-dense"])
