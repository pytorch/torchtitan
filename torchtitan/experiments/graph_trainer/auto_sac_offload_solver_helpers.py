...
def optimizer_state_bytes(opt_config: object | None, param_numel: int) -> int:
    ...
    ``param_numel`` is the per-rank parameter element count (local shards only,
    summed over every pipeline stage this rank owns). It is passed in rather
    than walked from a model so this stays a pure function of the config.
    ...
    Callers must treat that as an error: it is subtracted from a budget, so
    passing it through would *raise* the budget instead of reserving anything.
    """
    ...
    return param_numel * n_states * elt_bytes


def local_param_numel(model_parts: list[torch.nn.Module] | None) -> int:
    """Per-rank parameter element count across every pipeline stage on this rank.

    Counts the local shard of a DTensor, so the result is already per rank.
    Sums over all ``model_parts``: with pipeline_parallel_degree > 1 a rank owns
    more than one stage, and counting only the first under-reserves optimizer
    state by roughly the stage count.
    """
    return sum(
