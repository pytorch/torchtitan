...
from torchtitan.experiments.graph_trainer.inductor_passes import BOXED_CODEGEN_META
...
def _ensure_boxed_graph_module(gm: fx.GraphModule) -> fx.GraphModule:
    """Use boxed FX codegen for GraphPP runtime-owned graph modules."""

    if gm.meta.get(BOXED_CODEGEN_META) is True:
        return gm
    # FX exposes boxed calling convention only through this private codegen.
    # GraphPP owns these generated GraphModules and always calls them with one
    # mutable args list so the generated code can clear it after unpacking.
    gm.graph.set_codegen(torch.fx.graph._BoxedCodeGen())
    gm.recompile()
    gm.meta[BOXED_CODEGEN_META] = True
    return gm


def _ensure_boxed_graph_modules(modules: _StageGraphModules) -> _StageGraphModules:
    """Box every optional graph module in one stage graph bundle."""

    def box_optional(gm: fx.GraphModule | None) -> fx.GraphModule | None:
        if gm is None:
            return None
        return _ensure_boxed_graph_module(gm)

    return _StageGraphModules(
        fw=_ensure_boxed_graph_module(modules.fw),
        full_bw=_ensure_boxed_graph_module(modules.full_bw),
        bw_di=box_optional(modules.bw_di),
        bw_dw=box_optional(modules.bw_dw),
        unshard=box_optional(modules.unshard),
        reduce_grad=box_optional(modules.reduce_grad),
    )
...
def _compile_stage_graphs(
    ...
    *,
    ...
    if graphs.compiled:
        return
    if not compile_config.enable or not compile_config.enable_passes:
        graphs.modules = _ensure_boxed_graph_modules(graphs.modules)
        return
