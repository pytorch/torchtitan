...
def make_graph_runtime(
    ...
    *,
    ...
    PyTorch pipeline schedules emit ``FULL_BACKWARD``. GraphRuntime replaces it
    with an explicit backward variant before graph construction;
    ...
    if compile_config.precompile_artifact_dir and not use_simple_spmd:
        raise ValueError(
            "PP=1 precompiled artifacts require the single "
            "FULL_FORWARD_BACKWARD schedule"
        )
    if use_simple_spmd:
