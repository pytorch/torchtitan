...
# Set from --compile.auto_sac.debug_solver. The solver logs around 80
...
def _fsdp_shard_degree(gm: torch.fx.GraphModule) -> int:
    """Number of ranks an FSDP parameter is sharded over, read off the graph.

    ``all_gather_into_tensor(input, group_size, group_name)`` carries the
    resolved group size, which is the factor by which a sharded parameter grows
    when unsharded. Returns the largest such size, or LOCAL_WORLD_SIZE when the
    graph has no FSDP all-gather to read.
    """
    sizes = [
        n.args[1]
    ...
    return max(1, int(os.environ.get("LOCAL_WORLD_SIZE", "1")))


def get_knapsack(nodes, limit, size_of, cost_of, sense, unit=1 << 20):
...
def _fmt_ranges(values) -> str:
    ...
    if not vals:
        # Callers pass layer lists that can legitimately come out empty (a
        # pattern covering no layers, a skipped-layer list with nothing in it).
        return ""
    start = prev = vals[0]
...
# MEM_MULTIPLIER and HOST_MEMORY_FRACTION live in the utils module so that
# memory_policy can read them without importing this one, which pulls in pulp.
...
def get_must_keep_list(
    ...
    for node in gm.graph.nodes:
        if node.op != "call_function" or _touches_backward(node):
            continue
        if _is_rng_op(node):
            must_keep.add(node)
            continue
        ...
        for user in node.users:
            if not _touches_backward(user) and _get_layer_id(user) > node_layer:
                must_keep.add(node)
                break
    return must_keep
...
def two_level_solver(
...
) -> tuple[torch.fx.GraphModule, dict | None]:
    """Run the two-level solver and tag the graph with per-node decisions.

    Returns ``(gm, metrics)``. It never returns None for a failed solve: a
    failure raises ValueError on every rank, so callers do not have to check.
    """
    global _debug_logging
    _debug_logging = debug_logging

    if prefetch_lookahead > 1 or defer_n_layers > 1:
        logger.warning(
            "auto_perf_maxing: the peak model assumes a prefetch/defer depth of "
            "1, but prefetch_lookahead=%d and defer_n_layers=%d are configured. "
            "Each extra layer of depth keeps another layer's transfers resident, "
            "so the measured peak can exceed --compile.auto_sac.memory_budget_gb.",
            prefetch_lookahead,
            defer_n_layers,
        )
        # TODO: delay the activation offload/reload by defer_n_layers /
        # prefetch_lookahead layers. The outer model does not support this
        # currently: holding activations across more layers raises the peak,
        # and the constraints above charge only one adjacent layer. Needed for
        # MoE, where the extra depth matters most.
    ...
    opt_bytes = optimizer_state_bytes(optimizer, local_param_numel(model_parts))
    if opt_bytes == UNKNOWN_OPTIMIZER_BYTES:
        # Refuse: opt_bytes is subtracted from the budget,
        # so the -1 sentinel would *add* a byte of headroom while reserving
        # nothing for optimizer state, and the plan would silently overshoot.
        raise ValueError(
            "auto_perf_maxing cannot size the optimizer state for this "
            "optimizer, so it cannot respect a memory budget. Add it to "
            "STATES_PER_PARAM in auto_sac_offload_solver_utils.py with its "
            "per-parameter state count."
        )
    ...
    # TODO: alternative runtime estimators. Benchmarking each node in
    # isolation prices at zero everything it cannot time that way -- HOPs such
    # as flex_attention, _c10d_functional collectives, and ops that reject
    # random inputs -- and costs a few seconds of compile time. A roofline cost
    # model (max of compute and transfer time from FLOPs and bytes) would be
    # free and device-independent but not highly trustable; timing the real graph
    # using ``run_traced`` would price and measuring each op in isolation is also
    # a good idea.
    ...
    _solve_error = None
    if _rank == 0:
        # Everything that can raise runs inside the try. Without it, a rank-0
        # exception skips the broadcast below and leaves every peer blocked in
        # broadcast_object_list until the process group times out, which hides
        # the real traceback behind an NCCL timeout on some other rank.
        try:
            ...
            if _fr is None:
                ...
                comm_flag = [True]
        except Exception as exc:  # noqa: BLE001 -- re-raised after the broadcast
            logger.exception("auto_perf_maxing: rank 0 failed to build a plan")
            _solve_error = f"{type(exc).__name__}: {exc}"
            comm_flag = [None]
    else:
        comm_flag = [None]

    # sync the comm_flag. Carry rank 0's error text so peers fail with the
    # cause rather than a bare "plan crashed".
    payload = [comm_flag[0], _solve_error]
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        torch.distributed.broadcast_object_list(payload, src=0)
    comm_flag[0], _solve_error = payload

    if not comm_flag[0]:
        raise ValueError(
            "AC solver plan crashed!"
            + (f" (rank 0: {_solve_error})" if _solve_error else "")
        )
...
def plan_outer(
    ...
    # `if all_layer_ids` rather than `len(...) > 1`: a single-layer graph is a
    # valid problem, and the old test folded it into the layerless case, so the
    # outer solver returned no plan and the caller raised "AC solver plan
    # crashed!" on a model that simply has one block.
    if not all_layer_ids:
        logger.warning(
            "outer: no layers found -- no activation bytes to free, nothing to do"
        )
        return None, None, None
    last_layer_id = max(all_layer_ids)
    ...
    block_param_bytes = defaultdict(int)  # layer index -> its own param bytes
    for n in persistent_state:
        _fqn = _param_owner_fqn(n)
        is_block = _fqn.startswith("layers.")
        _parts = _fqn.split(".")
        _layer = int(_parts[1]) if is_block and _parts[1].isdigit() else None
        for t in pytree.tree_leaves(n.meta.get("val")):
            ...
            if is_block:
                if _layer is not None:
                    block_param_bytes[_layer] += sz
            else:
                non_block_param_bytes += sz

    # Unshard factor for the FSDP peak terms below. Read it off the graph's own
    # all_gather group_size rather than LOCAL_WORLD_SIZE: the two agree only
    # when the shard group happens to be exactly one host, so on multi-node runs
    # LOCAL_WORLD_SIZE reconstructs too few shards and underestimates the
    # forward peak. Falls back to the env var for graphs with no FSDP collective.
    _shard_degree = _fsdp_shard_degree(gm)
    param_bytes_per_layer = param_bytes / len(layer_ids)

    # TODO: model per-layer parameter bytes instead of one mean. The FSDP peak
    # terms below multiply param_bytes_per_layer by the shard degree to size an
    # unshard, which is exact only for a homogeneous stack. A model mixing dense
    # and MoE blocks has layers well above the mean, and their unshard is
    # undercounted by (that layer's bytes - the mean) * (shard_degree - 1).
    # Warn with the real number rather than fail: the budget still holds for
    # every layer at or below the mean, and refusing MoE outright would be
    # worse than a slightly optimistic peak on the widest layers.
    if block_param_bytes:
        _mean = sum(block_param_bytes.values()) / len(block_param_bytes)
        _widest, _widest_bytes = max(block_param_bytes.items(), key=lambda e: e[1])
        if _mean > 0 and _widest_bytes > 1.05 * _mean:
            logger.warning(
                "auto_perf_maxing: layer parameter sizes are not uniform "
                "(widest is layer %d at %.2f GiB vs %.2f GiB mean). The peak "
                "model uses the mean, so it undercounts that layer's unshard by "
                "about %.2f GiB and the real peak may exceed the budget. This "
                "is expected on models that mix dense and MoE blocks.",
                _widest,
                _widest_bytes / M,
                _mean / M,
                (_widest_bytes - _mean) * max(0, _shard_degree - 1) / M,
            )
    ...
    for L in layer_ids:
        ...
        params_total_forward = (
            ...
            + (_shard_degree - 1) * param_bytes_per_layer
        ...
        # The SOLVED report must mirror the constraint exactly. Dropping terms
        # here makes the "predicted peak" line rank an incomplete bwd value
        # against a complete loss_constraint, so a binding bwd_peak_L is
        # reported as loss_peak with phantom slack equal to the omitted terms.
    ...
    if LpStatus[status] != "Optimal":
        # no optimal solution found by outer ILP-solver
        return None, None, None
...
def plan_and_tag_inner(
    ...
    solver_type: str = "greedy",
) -> tuple[torch.fx.GraphModule, float, float, dict]:
    """Pick keep/recompute/offload per tensor, then tag the graph with it.

    Returns ``(gm, build_seconds, solve_seconds, achieved_fractions)``.
    ...
    def build_inner_ilp(candidates, kf, of, remaining_off_g, act):
        ...
        # Offload may run up to 1% past its target, charged at transfer time.
        prob += off_short >= total_off - target_off_g, "offload_uplimit"
        prob += off_short <= target_off_g * 0.01, "offload_over_cap"
    ...
    if solver_type == "ilp":
        ...
        for _sig_i, (sig, (candidates, kf, of, sig_act)) in enumerate(
            sig_meta.items(), 1
        ):
            solver_debugs(
            ...
            # Charge the global pinned pool: build_inner_ilp reads
            # remaining_off_g, so without this the cap is re-offered in full to
            # every pattern and never binds. One signature is stamped onto many
            # layers, so charge for all of them.
            _off_bytes = sum(sz for n, sz in candidates if (o[n].value() or 0.0) > 0.5)
        ...
        # Per-layer solve: no signature grouping, so no layer is ever dropped.
        skipped = []
        block_build_s = 0.0  # summed per-layer build time
        block_solve_s = 0.0  # summed per-layer solve time
        ...
        for b, candidates in candidates_by_layer.items():
            ...
            if per_layer_fractions is not None:
                ...
                kf, of = keep_fraction, offload_fraction
            solver_debugs(
                "layer %s: keep %.4f recompute %.4f offload %.4f",
                b,
                kf,
                1 - kf - of,
                of,
            ...
            # act_g holds bytes, not GiB, despite the name: the budgets below
            # are byte quantities and only the log lines divide by M.
            act_g = act
            k, r, o = {}, {}, {}
            ...
            solver_debugs(
                f"requested keep: {keep_budget / M:.2f}, "
                f"offload: {offload_budget / M:.2f}, "
                f"recompute: {recompute_budget / M:.2f}"
            )

            # Rank by _recompute_ms, not by the node's own runtime. A
            # wait_tensor costs ~0 on its own, but recomputing one reissues the
            # all_gather (and any cast feeding only it), so keying off
            # runtime_per_node priced collective reissue as free and let greedy
            # pick it ahead of genuinely cheap tensors. This is the same cost
            # function the inner ILP objective uses, so the two solvers now
            # rank candidates identically.
            ranked_activations = sorted(
                (n for n in all_candidates if all_candidates_by_size[n] > 0),
                key=lambda n: (
                    -_recompute_ms(n) / (all_candidates_by_size[n] / M),
                    n.name,
                ),
            )

            def _fill(gap, ordered, decision):
                """Claim undecided nodes in `ordered` until `gap` bytes are
                placed, skipping any that would overshoot. Returns bytes placed.

                This only has to get close; steps 3a and 3b below close the
                remaining gap exactly with a knapsack.
                """
                placed = 0.0
                if gap <= 0:
                    return placed
            ...
            # Offload the head of the ranking -- highest runtime per byte --
            # since those are what the recompute pass below would charge most for.
            ...
            solver_debugs(
                f"achieved keep: "
                f"{(act_g - achieved_recompute - achieved_offload) / M:.2f}, "
                f"offload: {achieved_offload / M:.2f}, "
                f"recompute: {achieved_recompute / M:.2f}"
            )

            # Whatever the two fills could not place stays resident, so keep can
            # sit either side of the outer LP's target. Steps 3a/3b below close
            # the gap exactly: overshooting keep is what breaks the memory
            # budget, since bwd_peak_L prices the kept activations below it.
            ...
            if kept_act_budget > keep_budget:
                available_pool = [
                    n for n in recompute_order if decisions_per_act[n] == 0
                ]
                _freed = get_knapsack(
                    available_pool,
                ...
                for n in _freed or available_pool:
            ...
            for node, decision in decisions_per_act.items():
                k[node] = 1.0 if decision == 0 or decision == 1 else 0.0
            ...
            solver_debugs(
                "inner: layer %s timing: %d candidates build=%.3fs solve=%.3fs",
                b,
                len(candidates),
                _blk_build_s,
                _blk_solve_s,
            ...
            solver_debugs(
                f"layer: {b} requested budgets offload: {of * act_g / M:.2f} "
                f"keep: {kf * act_g / M:.2f} "
                f"recompute: {(1 - of - kf) * act_g / M:.2f} GiB"
            )
            ...
            for node, size in candidates:
                solver_debugs(
                    f"  {node.name:<24s} {size / M:7.3f} GiB "
                    f"{_recompute_ms(node):7.2f} ms  {_tag_of(k, r, o, node)}"
                )
            ...
            solver_debugs(
                f"layer: {b} achieved budgets offload: {achieved_offload / M:.2f} "
                f"keep: {achieved_keep / M:.2f} "
                f"recompute: {achieved_recompute / M:.2f} GiB"
            )
        ...
        solver_debugs(
            f"requested total keep: {total_keep / num_layers:.2f} "
            f"recompute: {total_recompute / num_layers:.2f} "
            f"offload: {total_offload / num_layers:.2f}"
        )

        solver_debugs(
            f"achieved total keep: "
            f"{100 * total_achieved_keep / total_activations:.2f}% "
            f"recompute: {100 * total_achieved_recompute / total_activations:.2f}% "
            f"offload: {100 * total_achieved_offload / total_activations:.2f}%"
        )
    ...
    for _n in gm.graph.nodes:
        if _n.op != "call_function" or _touches_backward(_n):
            continue
...
def get_fixed_bytes(
    ...
    for node in nodes:
        ...
        for t in pytree.tree_leaves(node.meta.get("val")):
            ...
            if prod.op in ("placeholder", "get_attr"):
                ...
                category = ACT if _touches_backward(last) else TEMP
    ...
    for t in range(num_indices):
        ...
        _by_dir = fixed_max_bwd if _touches_backward(node) else fixed_max_fwd
...
def find_last_fwd_use_index(node, sid, indices, _memo=None) -> int:
    ...
    for user in node.users:
        ...
        if not _has_sid(user_in) or _touches_backward(user):
