...
class TokenExpertTracker:
    ...
    def record_expert_assignment(self, layer_id, expert_indices):
        """
        Record which experts were assigned to each token in a specific layer

        Args:
            layer_id: The layer ID
            expert_indices: Tensor of shape [batch_size*seq_len, top_k] containing expert indices
        """
        ...
        for token_pos, experts in enumerate(experts_per_token):
            ...
            # Store all expert assignments for this token in this layer
            self.token_history[token_pos][layer_id] = experts
    ...
    def export_csv_report(self, filename="expert_assignments.csv"):
        """
        Save token-expert assignments as multiple CSV files where:
        - Each file represents one of the top-k expert assignments
        - Each row represents a token
        - Each column represents a layer
        - Each value is the expert assigned to that token in that layer at that top-k position
        """
        import csv
        ...
        # Get the top-k value from the first entry
        if tokens and layers:
            first_token = tokens[0]
            first_layer = layers[0]
            top_k = len(self.token_history[first_token][first_layer])
        else:
            top_k = 6  # Default if no data available

        # Create a file for each top-k position
        for k in range(top_k):
            k_filename = f"{filename.rsplit('.', 1)[0]}_{k}.csv"

            with open(k_filename, "w", newline="") as csvfile:
                ...
                for token_pos in tokens:
                    ...
                    for layer_id in layers:
                        if layer_id in self.token_history[token_pos]:
                            # Use the k-th expert assigned to this token in this layer
                            experts = self.token_history[token_pos][layer_id]
                            if k < len(experts):
                                row.append(experts[k])
                            else:
                                row.append(-1)  # No expert at this position
                        else:
                            row.append(-1)
                    writer.writerow(row)

            print(f"Expert trace for top-{k} saved to {k_filename}")
