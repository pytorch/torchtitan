...
def main(dataset: str,
    ...
    job_config.parse_args(["--training.dataset", dataset,
    ...
    total_non_padding_tokens = total_input_ids - int((batch["input_ids"] == 128004).sum())
    ...
    print(
        f'Padded images: {padded_images}, {padded_images/total_images*100:.2f}% (Each image with shape {list(batch["encoder_input"]["images"][0,0].shape)})'
    )
    print(80 * "#")
    # Tiles
    ...
    print(
        f'Padded tiles: {padded_tiles}, {padded_tiles/total_number_of_tiles*100:.2f}% (Each with shape {list(batch["encoder_input"]["images"][0,0,0].shape)})'
    )
    print(80 * "#")
