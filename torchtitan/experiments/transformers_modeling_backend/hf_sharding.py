...
def _hf_colwise_config() -> ShardingConfig:
    ...
    return ShardingConfig(
        ...
        out_shardings=_hf_activation_placement(tp=spmd.S(-1)),
    )
    ...
    *,
    ...
    if model.tok_embeddings is not None and not isinstance(
        ...
        model.tok_embeddings._sharding_config = ShardingConfig(
            ...
            in_shardings={"input": _hf_activation_placement(tp=spmd.R)},
            out_shardings=_sp_activation(enable_sp=enable_sp),
            local_spmd=True,
        )
...
def _set_layer_sharding_configs(layer: nn.Module, *, enable_sp: bool) -> None:
    ...
    attn._sharding_config = ShardingConfig(
        in_shardings={"hidden_states": _sp_activation(enable_sp=enable_sp)},
    )
    ...
    if not getattr(layer, "moe_enabled", False):
        ...
        mlp._sharding_config = ShardingConfig(
            in_shardings={mlp_arg: _sp_activation(enable_sp=enable_sp)},
            out_shardings=_sp_activation(enable_sp=enable_sp),
        )
...
def _hf_norm_config(*, enable_sp: bool) -> ShardingConfig:
    ...
    return ShardingConfig(
        ...
        in_shardings={"hidden_states": sp_layout},
...
def _rope_config(module: nn.Module, *, enable_sp: bool) -> ShardingConfig:
    ...
    Under SP the hidden-states arg arrives sharded
