...
def create_configs(model_name: str, out_dir: str, flavor: str, model_type: str = "transformers_modeling_backend", hf_assets_path: str = None, enable_profiling: bool = False, profile_freq: int = 5, enable_compile: bool = False, enable_full_ac: bool = False):
    """
    results/
        |_ meta-llama
            |_ Llama-3.2-1B
    ...
    # Configure full activation checkpointing
    if enable_full_ac:
...
class Scheduler:
    def __init__(self, inp_dir: str, qos: str) -> None:
        # Find all directories that contain job configuration files
        jobs_directory_paths = []
        for root, dirs, files in os.walk(inp_dir):
            # A job directory has a config.toml AND either a slurm script or status.txt
            has_config = "config.toml" in files
            has_slurm = any(f.endswith(".slurm") for f in files)
            
            if has_config and has_slurm:
...
def compare_throughput(torchtitan_dir: str, hf_dir: str, only: str = None, hide_status: bool = False):
    """
    ...
    for name, comparable, summary, stats in results:
        ...
        if hide_status:
            summary_table.add_row(name, tps_ratio, tt_tps, hf_tps, tt_mem, hf_mem, tt_mfu, hf_mfu)
        else:
...
if __name__ == "__main__":
    ...
    create_configs_parser.add_argument("--enable_compile", action="store_true",
                                       help="Enable torch.compile for model and loss")
    create_configs_parser.add_argument("--enable_full_ac", action="store_true",
                                       help="Enable full activation checkpointing")
    ...
    compare_throughput_parser.add_argument("--hide_status", action="store_true",
        help="Hide Status and Loss Diff columns in the summary table")

    args = parser.parse_args()

    if args.action == "create_configs":
        create_configs(args.model_name, args.out_dir, args.flavor, args.model_type, args.hf_assets_path, args.enable_profiling, args.profile_freq, args.enable_compile, args.enable_full_ac)
        ...
        compare_throughput(args.torchtitan_dir, args.hf_dir, args.only, args.hide_status)
