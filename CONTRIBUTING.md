...
- The command-line options are frozen: no new `--section.option` flags. A knob that belongs to a component goes in that component's config (dataclass), and one that changes the model goes in the model config (dataclass). See [configuration.md](docs/configuration.md).
