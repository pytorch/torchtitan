...
persisted and its step matches `checkpoint.async_eval_frequency`, `AsyncEval` launches the
...
`checkpoint.keep_async_eval_checkpoints=False` to let the normal checkpoint retention
policy remove them.
