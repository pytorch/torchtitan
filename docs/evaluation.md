...
`AsyncEval` moves evaluation off the training loop's critical path. It requests
a checkpoint every `async_eval.freq` steps and launches the evaluator
subprocess after that checkpoint is fully persisted. The trainer continues
training while the evaluator loads the checkpoint for evaluation.
...
Async-eval checkpoints are managed independently from
`checkpoint.keep_latest_k`. By default, all of them are preserved. Set
`async_eval.preserve_checkpoints=False` to retain the latest async-eval
checkpoint. Older checkpoints are deleted only after their eval jobs finish.
