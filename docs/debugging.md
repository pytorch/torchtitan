...
```bash
MODULE=llama3 CONFIG=llama3_debugmodel ./run_train.sh checkpointer:config --checkpointer.load_only
```

The `--checkpointer.load_only` flag prevents the training process from saving any checkpoints, allowing you to:
