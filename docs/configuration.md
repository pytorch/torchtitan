...
```bash
MODULE=torchtitan_configs.tests CONFIG=llama3_debugmodel_fsdp2_cp2 ./run_train.sh
```
...
The [torchtitan_configs](../torchtitan_configs/) package holds full configurations. It sits outside the `torchtitan` package on purpose: `torchtitan` ships the mechanisms -- `model_registry` for architectures, and the classes implementing each optimization -- while `torchtitan_configs` makes the choices. The core torchtitan provides only the building blocks, and users compose the runs suitable for their environments and settings.

### Writing your own

A different cluster usually means a different sharding layout, and therefore a different configuration. That needs no code change: add a function to `torchtitan_configs`, in a module named for the model, and name it on the command line. (`torchtitan_configs/tests.py` is separate -- it holds the configurations the integration tests run.)

```python
# torchtitan_configs/llama3.py
...
Frozen means the CLI, not the config dataclasses. A few options genuinely have no other home, such as `training.local_batch_size`, so [configs.py](../torchtitan/config/configs.py) is not closed -- discuss with the maintainers first. Annotate the field with `tyro.conf.Suppress` and a configuration can set it while the CLI stays as it is:
...
### What belongs in `torchtitan_configs`

What this repository ships, which is deliberately a small set:

- `tests.py` -- the configurations the integration tests run
- golden configurations verified on specific hardware, kept so a benchmark run is reproducible from its name alone
...
The per-model `config_registry.py` modules, selected with `--module <model> --config <function>`, are the earlier location for the same thing. They keep working and are still how the integration tests select configs, but they take no new entries.
