...
issued inside `RoutedExperts.forward`, together with the
...
One record per `RoutedExperts.forward` invocation:
...
class GroupedGemmRecord:
    ...
    # Shapes of the three grouped GEMMs in the routed-expert forward
...
A single shim inside `RoutedExperts.forward`, right after the token
dispatcher returns and before the `inner_experts` grouped GEMMs run:

```python
# torchtitan/models/common/moe.py
from torchtitan.components.moe_metrics import maybe_record_grouped_gemm

def forward(self, x_BLD, topk_scores_BLK, topk_expert_ids_BLK, ...):
    ...
    ...
    routed_output_RD = self.inner_experts(routed_input_RD, num_tokens_per_expert)
    ...
```
...
- **Activation checkpointing.** The existing comment in `MoE.forward` notes
  ...
  from `RoutedExperts.forward`, which is also re-run under AC. To avoid double
...
2. Wire `RoutedExperts.forward` and `TokenDispatcher.metadata` additions.
