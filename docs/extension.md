...
- `key_filter` returns a callable that identifies state dict keys owned by this converter (e.g. LoRA adapter keys). Used by `ModelWrapper.filter_base_keys()` to exclude converter keys from HF load containers. Return `None` if the converter doesn't introduce new keys.
- `state_dict_transform` returns a callable `(state_dict, last_step) -> state_dict` that transforms the model state dict during saves. The converter's config determines exact behavior (e.g. LoRA filters to adapter keys on interval saves, merges adapters on final save; QAT strips fake quantization on final save). Return `None` if no transform needed.
- `format_adapter` returns a `BaseStateDictAdapter` subclass that provides key mapping for the converter's external checkpoint format (e.g. PEFT safetensors for LoRA). Return `None` if no external format support needed.
