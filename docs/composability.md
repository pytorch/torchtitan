...
## Setting `CUDA_DEVICE_MAX_CONNECTIONS` for FSDP2 + native TP
For FSDP2 composed with PyTorch native DTensor tensor parallelism, set `CUDA_DEVICE_MAX_CONNECTIONS` to the number of CUDA streams (for example, 16 or 32) so compute and NCCL kernels can overlap.

This is the opposite of Megatron-style TP, which typically requires `CUDA_DEVICE_MAX_CONNECTIONS=1`. Do not copy that Megatron setting when using FSDP2 + native TP.
