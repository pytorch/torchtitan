...
from src.protocols.model import BaseModelArgs
from src.protocols.state_dict_adapter import StateDictAdapter
...
class MoEStateDictAdapter(StateDictAdapter):
    ...
    def _calculate_strided_shard_shard_indices(
        ...
        """
        Given a [StridedShard(dim=i), Shard(dim=i)] placement, calculate the
        start index and end index on dim-i for GPU rank
        (strided_shard_dim_degree, shard_dim_rank).
        ...
        Calculate the start_index from inner dimension (Shard(dim=i))
        to outer dimension (StridedShard(dim=i)).
        """
