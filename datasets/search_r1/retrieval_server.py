...
def load_corpus(corpus_path: str):
    # Read the jsonl directly into a list (list index == passage id, which is how
    # faiss hit ids map to passages). This avoids HF `datasets`' slow single-file
    # Arrow conversion (~30min for 21M lines) and its on-disk cache (which fails
    # the free-space check on the manifoldfs mount). corpus[int(idx)] -> {"id",
    # "contents"} works the same as a datasets Dataset.
    import json
    import time
    ...
    print(
        f"[retriever] loaded corpus: {len(corpus)} passages in "
        f"{time.time() - t0:.0f}s from {corpus_path}",
        flush=True,
    )
    return corpus
...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        _t0 = _t.time()
        if config.faiss_gpu:
            # GPU path (original): full read into CPU RAM, then clone to GPU(s).
            # (mmap doesn't help here -- index_cpu_to_all_gpus reads every vector.)
            print(f"[retriever] reading index {self.index_path} (for GPU) ...", flush=True)
            self.index = faiss.read_index(self.index_path)
        else:
            # CPU path: MMAP so a slow manifoldfs full-read is avoided (pages in on
            # demand). Returns in seconds vs tens of minutes over the mount.
            print(f"[retriever] mmap-reading index {self.index_path} ...", flush=True)
            self.index = faiss.read_index(self.index_path, faiss.IO_FLAG_MMAP)
        print(f"[retriever] index ready in {_t.time() - _t0:.0f}s", flush=True)
        if config.faiss_gpu:
