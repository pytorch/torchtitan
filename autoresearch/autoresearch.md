...
## This branch: Run 2 — Curated IDEAS (no references)

The goal is to see what an LLM agent can discover about optimizing a
Llama3 8B training graph **with human-curated seed ideas but no access
to prior optimization work or reference implementations**. Start from
the curated ideas in `IDEAS.md`, and expand with your own ideas from
graph inspection, profiling, and online research.
...
- ~~Web content, blog posts, papers, tutorials.~~ **Allowed for this run** — online research is permitted for finding optimization ideas (see IDEAS.md).
