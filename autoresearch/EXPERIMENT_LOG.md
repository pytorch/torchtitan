...
## cleanup_bundle (mega-cleanup) — discard (xxxxxxx)

- **Idea**: After 3 consecutive sub-noise discards (Exps 14/15/16, each +0.1-0.4%), bundle ALL accumulated small cleanup patterns into one pass and see if the sum compounds past +1%. Patterns: detach removal, view CSE, _conj CSE, clone CSE, identity `_unsafe_view`, identity/double `transpose.int`, identity `mul.Tensor`/`add.Tensor`.
- **Changes**: Added `cleanup_bundle` last in pass list. Generic `_cse(target, key_fn)` and `_remove_identity_op(target, predicate)` helpers. Applied CSEs first, then identity removals.
- **Result**: best-of-3 tps=5,157 (+0.64% vs 5,124), runs 5,139/5,157/5,154, mfu=30.20%, memory=47.05 GiB, wall_time=72-76s. Numerics bitwise-identical. Counts: detach 194, view CSE 96, _conj 32, clone 32, **0** for unsafe_view/transpose/mul/add. Total 354 ops/step removed. Below +1% threshold.
- **Analysis**: The opportunistic patterns (_unsafe_view, transpose, mul, add) all returned zero — confirming the metadata-cleanup well is genuinely empty. Even bundling everything we've found over 17 experiments, the cleanup-only ceiling is +0.64% — squarely sub-noise. The 256 `transpose.int` calls all use `(1, 2)` and never nest (SDPA always consumes them). All `_unsafe_view` calls are genuine rank changes after `mm`. The graph is structurally clean post Exp 1+7+11+13.
- **Lessons**: We've reached the local optimum for FX-level structural rewrites + bandwidth-bound collectives + metadata cleanups. The remaining big targets (RoPE fp32 round-trip, FSDP weight cast bytes, TP RS-ws-2 bandwidth) are blocked by bitwise-numerics constraints or out-of-passes.py-scope (FSDP API changes). Cumulative +23.1% over raw baseline (4,161 → 5,124) is the achievable ceiling without breaking the bitwise rule or expanding the file-modification scope.

---

## Exp 18 open-ended deep-dive — no_target (xxxxxxx)

- **Idea**: Open-ended subagent investigation. Survey the post-Exp-13 graph against 8 hint directions (matmul-fanout CSE, residual+norm fusion, backward chain reuse, SDPA-adjacent transpose CSE, fwd/bwd transpose pairing, in-place `_to_copy` opportunities, Inductor meta hints, post-coalesce peephole patterns).
- **Result**: `no_target` — all 8 directions ruled out at <300 ops/step. Specifically: every `t.default` has unique input (no fanout); residual `add` has 3+ users blocking rewrites; backward chain ops have multi-user constraints; all 256 `transpose.int` have unique inputs; fwd/bwd transposes operate on different tensors; `_to_copy` outputs feed wait_tensor or coalesced collectives (not in-place-capable); 33 coalesced-AG getitems are all 1:1 with downstream waits. Net countable patterns sum to 354 ops (same as Exp 17), confirmed sub-noise.
- **Analysis**: This confirms the +23.1% ceiling. The graph is now structurally clean modulo bitwise-constrained patterns (RoPE round-trip, fp32 reduce_dtype). No FX-level pass within `passes.py` scope can cross +1% from here.
- **Lessons**: Recorded the run summary in LEARNINGS.md. Further experiments are unlikely to find keep-worthy patterns; speculative attempts will likely all discard.
