...
class QKClipTensorParallelTest(DTensorTestBase):
    ...
    @with_comms
    def test_dp_tp_scales_storage_shards_that_split_heads(self) -> None:
        mesh = init_device_mesh(
            self.device_type,
            (2, 2),
        ...
        self.assertNotEqual(
            q_weight.to_local().shape[0] % attention.qk_head_dim,
            0,
        )
        ...
        maxima_by_coordinate = (
            ((50.0, 400.0, 25.0), (25.0, 800.0, 80.0)),
            ((200.0, 50.0, 500.0), (500.0, 40.0, 250.0)),
        )
        attention.inner_attention.max_attention_logits_H.append(
        ...
        scales_H = torch.tensor((0.5, 0.25, 0.2, 0.2, 0.125, 0.4), device=device)
        q_expected_HDI = torch.ones(
            num_heads,
            attention.qk_head_dim,
            in_features,
            device=device,
        )
        q_expected_HDI[:, :qk_nope_head_dim].mul_(scales_H.view(-1, 1, 1).pow(0.5))
        q_expected_HDI[:, qk_nope_head_dim:].mul_(scales_H.view(-1, 1, 1))
        torch.testing.assert_close(q_weight.full_tensor(), q_expected_HDI.flatten(0, 1))

        kv_expected_HDI = torch.ones(
            num_heads,
            qk_nope_head_dim + v_head_dim,
            in_features,
            device=device,
        )
        kv_expected_HDI[:, :qk_nope_head_dim].mul_(scales_H.view(-1, 1, 1).pow(0.5))
        kv_weight = attention.wkv_b.weight
        assert isinstance(kv_weight, DTensor)
        torch.testing.assert_close(
            kv_weight.full_tensor(),
            kv_expected_HDI.flatten(0, 1),
        )
        self.assertFalse(attention.inner_attention.max_attention_logits_H)
