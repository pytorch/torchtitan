...
# ``check_available=True`` is required: the default returns the accelerator the
# build was compiled for even when no device is usable at runtime, which would
# make the ``torch.cuda.get_device_capability()`` call below raise at collection
# time on a CUDA build with no GPU.
...
pytestmark = [
    pytest.mark.skipif(
        _accelerator is None, reason="an available accelerator is required"
    ),
    pytest.mark.skipif(
