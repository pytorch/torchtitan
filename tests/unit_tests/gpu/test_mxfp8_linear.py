...
def test_operands_fields_must_be_distinct_allocations():
    ...
    from torchtitan.components.quantization._fsdp_tensor import _managed_tensors
    ...
    with pytest.raises(ValueError, match="distinct allocations"):
        _managed_tensors(AliasingOperands(qdata, qdata.t()))
    ...
    assert len(_managed_tensors(DistinctOperands(qdata, scale))) == 2
