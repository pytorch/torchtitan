...
class TestAsyncTensorParallelSharding(DTensorTestBase):
    ...
    def test_parallelize_keeps_async_collectives_in_projection_leaves(self):
        ...
        local_qkv_out_features = (
            attn_cfg.qkv_linear.wqkv.linear.out_features // self.world_size
        )
        ...
        self.assertEqual(attn.qkv_linear.wqkv.linear.weight.shape[0], 1)
        self.assertEqual(
            attn.qkv_linear.wqkv.linear.weight.shape[1], local_qkv_out_features
        )
        self.assertEqual(attn.wo.linear.weight.shape[0], 1)
    ...
    def test_w13_tp_shards_the_matrix_row_dimension(self):
        ...
        self.assertEqual(
            feed_forward.w13.linear.weight.shape,
            (2, hidden_dim // self.world_size, DIM),
        )
        self.assertEqual(
            feed_forward.w2.linear.weight.shape,
    ...
    def test_matches_standard_feed_forward(self):
        ...
        with torch.no_grad():
            dist_gemm.w13.linear.weight = torch.nn.Parameter(
                standard.w13.linear.weight.chunk(R, 1)[self.rank].contiguous()
            )
            dist_gemm.w2.linear.weight = torch.nn.Parameter(
                standard.w2.linear.weight.chunk(R, 2)[self.rank].contiguous()
            )
    ...
    def test_matches_native_feed_forward(self):
        ...
        with torch.no_grad():
            fused.w13.linear.weight = torch.nn.Parameter(
                native.w13.linear.weight.chunk(R, 1)[self.rank].contiguous()
            )
            fused.w2.linear.weight = torch.nn.Parameter(
                native.w2.linear.weight.chunk(R, 2)[self.rank].contiguous()
            )
