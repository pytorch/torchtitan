...
class TestKDAContextParallelPartition(unittest.TestCase):
    def test_contiguous_fragments(self):
        partitioner = ContextParallelPartitioner(
            ...
            cp_mesh=cast(
                object,
                SimpleNamespace(size=lambda _axis=0: 2),
            ),
            load_balancer_config=None,
        )
        self.assertEqual(
            partitioner.token_partition(16),
            [[(0, 8)], [(8, 16)]],
        )
    ...
    def test_headtail_fragments(self):
        cp_mesh = cast(
            object,
            SimpleNamespace(size=lambda _axis=0: 2, device_type="cpu"),
        )
        with patch(
            "torchtitan.distributed.context_parallel._HeadTailLoadBalancer"
        ):
            partitioner = ContextParallelPartitioner(
                input_dict={"input": torch.arange(16)},
                input_shardings=None,
                cp_mesh=cp_mesh,
    ...
    def test_rejects_an_uneven_partition(self):
        ...
        with patch(
            ...
            partitioner = ContextParallelPartitioner(
                input_dict={"input": torch.arange(15)},
                input_shardings=None,
                cp_mesh=cp_mesh,
                load_balancer_config=HeadTailLoadBalancer.Config(),
            )
        with self.assertRaisesRegex(ValueError, "divisible by 4"):
            partitioner.token_partition(15)
        ...
        partitioner = cast(
            ...
            SimpleNamespace(
                cp_mesh=SimpleNamespace(
                    get_group=lambda: group,
                    size=lambda _axis=0: 2,
                ),
                token_partition=lambda _num_tokens: [
                    [(0, 2), (6, 8)],
                    [(2, 4), (4, 6)],
                ],
            ),
