...
class TestCheckpointManager(unittest.TestCase):
    ...
    @mock.patch.object(dist_checkpoint, "save")
    @mock.patch.object(dist_checkpoint, "load")
    def test_purge_keeps_async_eval_checkpoints(self, _mock_load, mock_save, _mock_rank):
        mock_save.side_effect = self.fake_save
        self.trainer_config.checkpoint.async_eval_frequency = 2
        manager = CheckpointManager(
            dataloader=self.data_loader,
            model_parts=self.model_parts,
            optimizers=self.optimizers,
            ...
            sd_adapter=None,
            base_folder=self.trainer_config.dump_folder,
            async_eval_checkpoint_callback=mock.Mock(),
        )
    ...
    @mock.patch.object(dist_checkpoint, "save")
    @mock.patch.object(dist_checkpoint, "load")
    def test_purge_can_remove_async_eval_checkpoints(self, _mock_load, mock_save, _mock_rank):
        mock_save.side_effect = self.fake_save
        self.trainer_config.checkpoint.async_eval_frequency = 2
        self.trainer_config.checkpoint.keep_async_eval_checkpoints = False
        manager = CheckpointManager(
            dataloader=self.data_loader,
            model_parts=self.model_parts,
            optimizers=self.optimizers,
    ...
    @mock.patch.object(dist_checkpoint, "save")
    @mock.patch.object(dist_checkpoint, "load")
    def test_save_schedule_includes_async_eval_checkpoints(
        self, mock_load, mock_save, mock_rank
    ):
        """
        Test regular, async-eval, and last-step checkpoint scheduling.
        """
        cfg = self.trainer_config.checkpoint
        cfg.interval = 3
        cfg.async_eval_frequency = 2
        cfg.keep_latest_k = 0
        mock_save.side_effect = self.fake_save
        callback = mock.Mock()
        manager = CheckpointManager(
            ...
            sd_adapter=None,
            base_folder=self.trainer_config.dump_folder,
            async_eval_checkpoint_callback=callback,
        )
    ...
    def test_async_eval_callback_rejects_load_only(self):
        config = CheckpointManager.Config(enable=True, load_only=True)
        with mock.patch.object(CheckpointManager, "__del__", return_value=None):
...
class TestConfigPostInit(unittest.TestCase):
    ...
    def test_sanity_and_range_checks(self):
        ...
        with self.assertRaisesRegex(ValueError, "frequency.*at least 1"):
            CheckpointManager.Config(async_eval_frequency=0)
