...
from torchtitan.models.common.linear import Linear, LinearStateInitializer
...
class TestLinear(unittest.TestCase):
    ...
    def test_init_states(self):
        """Linear.init_states re-initializes the weight tensor."""
        config = Linear.Config()
    ...
    def test_custom_init_std(self):
        ...
        config = Linear.Config(
            state_initializer=LinearStateInitializer.Config(
                init_mean=0.1, init_std=0.02
            )
        )
        linear = config.build(in_features=1000, out_features=500)
    ...
    def test_state_initializer_stored(self):
        """_state_initializer is stored on the instance."""
        config = Linear.Config(
            state_initializer=LinearStateInitializer.Config(
                init_mean=0.1, init_std=0.05
            )
        )
        linear = config.build(in_features=8, out_features=4)
        self.assertIsNotNone(linear._state_initializer)
        self.assertIsInstance(linear._state_initializer, LinearStateInitializer)
...
class TestModuleInjection(unittest.TestCase):
    ...
    def test_init_states_after_injection(self):
        """init_states works on injected module via Linear's MRO."""
        ...
        model = nn.Module()
        config = Linear.Config(
            state_initializer=LinearStateInitializer.Config(init_std=0.03)
        )
        model.fc = config.build(in_features=8, out_features=4)

        # Capture attrs, simulate conversion, inject
        saved_attrs = capture_module_attrs(model, ["_state_initializer"])
        model.fc = FakeQuantLinear(8, 4)
        inject_module_protocol(model, Linear, saved_attrs)

        # Should not raise — init_states comes from Linear via MRO
        with torch.no_grad():
    ...
    def test_capture_and_reattach_attrs(self):
        ...
        model = nn.Module()
        config = Linear.Config(
            state_initializer=LinearStateInitializer.Config(init_std=0.05),
            bias=True,
        )
        model.fc = config.build(in_features=8, out_features=4)

        # Capture attrs
        saved = capture_module_attrs(model, ["_state_initializer"])
        self.assertIn("fc", saved)
        self.assertIn("_state_initializer", saved["fc"])

        # Simulate Float8 conversion: replace with a new FakeQuantLinear
        model.fc = FakeQuantLinear(8, 4, bias=True)
        self.assertNotIsInstance(model.fc, Module)
        self.assertFalse(hasattr(model.fc, "_state_initializer"))

        # Inject and re-attach
        inject_module_protocol(model, Linear, saved)
        self.assertIsInstance(model.fc, Module)
        self.assertIsInstance(model.fc, Linear)
        self.assertIsNotNone(model.fc._state_initializer)
    ...
    def test_mx_style_class_swap_preserves_attrs(self):
        ...
        self.assertTrue(hasattr(model.fc, "_state_initializer"))  # attrs survive
