...
class TestLinear(unittest.TestCase):
    ...
    def test_init_weights_with_override(self):
        """Linear.init_weights respects init_std override."""
        config = Linear.Config(init_std=0.01)
        linear = config.build(in_features=1000, out_features=500)

        torch.manual_seed(42)
        with torch.no_grad():
            linear.init_weights(init_std=0.005)
        # std should be close to the override, not the config default
