...
from torchtitan.models.common.remat import (
    ...
    remat_save_region_selection,
    RematSaveRegionSelection,
    require_torch_remat,
)
...
class _SideEffectBlock(_RematBlock):
    ...
    def forward(self, x_BD: torch.Tensor) -> torch.Tensor:
        if not is_remat_recomputing(self):
...
class TestRematRegions(unittest.TestCase):
    def test_region_discovery_and_glob_selection(self):
        ...
        self.assertEqual(
            remat_save_region_selection(tree.left),
            RematSaveRegionSelection(
                qualified_name_by_local_name={"w1": "left.w1", "w2": "left.w2"}
            ),
        )
    ...
    def test_deepseek_remat_policy_matches_model(self):
        pytest.importorskip("torch_remat")
        require_torch_remat()
        ...
        deepseek_v3_remat_config().build().apply(model)
