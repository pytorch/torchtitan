...
class TestDeepSeekV4HashRoutingSharding(unittest.TestCase):
    def test_sp_ep_shards_hash_input_ids_with_activations(self):
        ...
        expected_dst = _per_axis_types(token_id_placement(enable_sp=True))
        for layer_id in (0, 1):
    ...
    def test_ep_without_sp_shards_hash_input_ids_on_tp(self):
        model_config = model_registry("debugmodel").model
        ...
        hash_dst = hash_moe.sharding_config.in_dst_shardings["input_ids_T"]
        self.assertEqual(
            _per_axis_types(hash_src), _per_axis_types(token_id_placement())
        )
        self.assertEqual(
            _per_axis_types(hash_dst),
