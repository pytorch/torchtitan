...
def test_trainer_accumulates_reused_cuda_graph_losses():
    ...
    metrics_processor.log.assert_called_once()
    log_call = metrics_processor.log.call_args
    assert log_call.args == (1, 6.0, 6.0, 4.0)
    assert log_call.kwargs["extra_metrics"]["n_tokens_seen"] == 3
