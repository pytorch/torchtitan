...
def test_pp_forward_backward_step_releases_consumed_loss_graphs() -> None:
    ...
    assert all(not loss.requires_grad for loss in emitted_losses)
    assert all(loss.grad_fn is None for loss in emitted_losses)
