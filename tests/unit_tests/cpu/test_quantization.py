...
from torchtitan.components.quantization.float8 import _get_float8_grouped_linear_cls
...
from torchtitan.models.gpt_oss.moe import GptOssDownGroupedLinear
...
@pytest.mark.parametrize("parent_cls", [GroupedLinear, GptOssDownGroupedLinear])
@pytest.mark.parametrize(
    "make_quantized_cls",
...
def test_float8_grouped_linear_checkpoint_state_uses_plain_tensors(parent_cls):
    pytest.importorskip("torchao")
    config = dict(num_groups=2, in_features=16, out_features=32)
...
def test_float8_grouped_linear_dcp_round_trip_needs_no_safe_globals(tmp_path):
    ...
    config = float8_cls.Config(num_groups=2, in_features=16, out_features=32)
    source = config.build()
    target = config.build()
