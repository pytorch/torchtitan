...
from torchtitan.models.common.linear import StructuredLinear
...
def _build_qkv_linear(with_bias: bool = False) -> QKVLinear:
    fused = QKVLinear.Config(
        ...
        wqkv=StructuredLinear.Config(
            in_features=_DIM,
            output_shape=(_R_DIM, _N_KV_HEADS * _HEAD_DIM),
            bias=with_bias,
        ),
    ).build()
    with torch.no_grad():
...
class TestQKVLinearCheckpointInterop(unittest.TestCase):
    def test_structured_qkv_preserves_logical_initialization(self):
