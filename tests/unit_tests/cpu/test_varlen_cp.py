...
from torchtitan.distributed.context_parallel import CPVarlenMetadata
...
class TestVarlenMetadataCudaGraphInputs(TestCase):
    """The metadata is rebuilt every step, so CUDA graphs must see its fields.

    An unregistered dataclass flattens to a single opaque leaf, which CUDA
    graph input handling classes as a non-tensor input and compares with
    ``!=``. ``eq=False`` makes that identity, so a rebuilt object would look
    like a changed input on step 2.
    """
    ...
    def _cp_metadata():
        return CPVarlenMetadata(
            ...
            max_q=4,
            max_k=4,
            k_global_gather_indices=torch.arange(4, dtype=torch.int64),
        )

    def test_metadata_exposes_its_tensors(self):
        leaves = CUDAGraphInputSpec({"m": self._metadata()}).flatten(
            {"m": self._metadata()}
        )
        tensors = [leaf for leaf in leaves if isinstance(leaf, torch.Tensor)]
        self.assertEqual(2, len(tensors), "cu_seq_q and cu_seq_k must be leaves")

    def _assert_non_tensor_leaves_match(self, build):
        """Mirror the check CUDAGraphWrapper runs on every step.

        It keeps the flattened non-tensor leaves from capture and compares
        them with ``!=`` against the next step's, so a rebuilt metadata object
        surviving as a leaf would look like a changed input.
        """
        spec = CUDAGraphInputSpec({"m": build()})
        captured = spec.flatten({"m": build()})
        rebuilt = spec.flatten({"m": build()})
        for first, second in zip(captured, rebuilt, strict=True):
            if isinstance(first, torch.Tensor):
                continue
            self.assertEqual(first, second)

    def test_rebuilt_metadata_matches_on_the_next_step(self):
        self._assert_non_tensor_leaves_match(self._metadata)

    def test_rebuilt_cp_metadata_matches_on_the_next_step(self):
        self._assert_non_tensor_leaves_match(self._cp_metadata)
