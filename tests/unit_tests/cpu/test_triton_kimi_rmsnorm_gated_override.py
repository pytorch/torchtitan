...
_OVERRIDE_TARGET = "torchtitan.overrides.kimi_rmsnorm_gated.triton_kimi_rmsnorm_gated"
_KIMI_RMSNORM_GATED_OVERRIDE = _REGISTRY[_OVERRIDE_TARGET]


class TestTritonKimiRMSNormGatedOverride(unittest.TestCase):
    def setUp(self):
        _REGISTRY.setdefault(_OVERRIDE_TARGET, _KIMI_RMSNORM_GATED_OVERRIDE)

    def test_override_replaces_all_kimi_rmsnorm_gated_modules(self):
        config = model_registry("debugmodel", attn_backend="flex")
        num_gated_norms = len(list(config.traverse(KimiRMSNormGated.Config)))
        ...
        self.assertEqual(
            len(list(config.traverse(TritonKimiRMSNormGated.Config))),
            num_gated_norms,
        )

    def test_config_is_replaced_without_changing_state_dict(self):
        stock_config = KimiRMSNormGated.Config(
            dim=128,
            eps=1e-5,
            param_init={"weight": torch.nn.init.ones_},
        )

        replacement = triton_kimi_rmsnorm_gated(stock_config)

        self.assertIsInstance(replacement, TritonKimiRMSNormGated.Config)
    ...
    def test_cpu_fallback_matches_stock_module(self):
        config = KimiRMSNormGated.Config(dim=128, eps=1e-5)
        stock = config.build()
        fused = triton_kimi_rmsnorm_gated(config).build()
        with torch.no_grad():
