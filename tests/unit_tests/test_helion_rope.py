...
class TestHelionRoPEOverride(unittest.TestCase):
    ...
    def test_forward_errors_without_helion(self):
        ...
        complex_module = HelionComplexRoPE.Config(
            dim=dim, max_seq_len=seqlen
        ).build()
        with self.assertRaisesRegex(ImportError, "helion.*not installed"):
            complex_module(xq, xk, positions)
...
class TestHelionRoPEKernel(unittest.TestCase):
    ...
    def test_eligible_rejects_unsupported_inputs(self):
        ...
        from torchtitan.overrides.helion_rope import _complex_eligible, _eligible
        ...
        self.assertTrue(
            _complex_eligible(q_transposed_layout, xk, cache_real, pos)
        )
        q_bad_last_dim = torch.empty(
            2, self.seqlen, 8, self.dim * 2, device=d, dtype=torch.bfloat16
        )[..., ::2]
        self.assertFalse(
            _complex_eligible(q_bad_last_dim, xk, cache_real, pos)
        )
