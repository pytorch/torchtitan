...
class TestYaRNScaling(unittest.TestCase):
    @staticmethod
    def _rope_config() -> ComplexRoPE.Config:
        max_seq_len = 4096
    ...
    def test_complex_rope_applies_at_original_sequence_length(self):
        yarn = self._rope_config().build()
        unscaled = ComplexRoPE.Config(dim=64, max_seq_len=4096).build()

        self.assertFalse(torch.equal(yarn.cache[1], unscaled.cache[1]))

    def test_deepseek_mscale_applies_at_original_sequence_length(self):
        ...
        attention_config.rope = dataclasses.replace(
            ...
            max_seq_len=attention_config.rope.original_seq_len,
        )
        attention = attention_config.build()

        expected_mscale = (
            0.1
            * attention_config.mscale
            * math.log(attention_config.rope.rope_factor)
            + 1.0
        )
