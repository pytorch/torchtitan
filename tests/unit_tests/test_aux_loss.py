...
class TestLoggedAuxLossAccumulation(unittest.TestCase):
    ...
    def test_unset_denominator_fails_loudly(self):
        """Unwired model specs must fail loudly during real forward; tracing
        (torch.compile capture) is allowed since the values are unused."""
        loss = self._make_loss()
        loss.per_step_denominator = -1
        carrier = torch.rand(4, 6, requires_grad=True)
        with self.assertRaises(ValueError):
            loss.inject(torch.tensor(5.0), carrier=carrier)
