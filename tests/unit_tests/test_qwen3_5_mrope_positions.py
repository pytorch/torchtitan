...
pipeline: for a dataloader batch, the position tensor that reaches every
transformer layer is ``mrope_positions`` when present, else the plain 2D
``positions`` -- while attention masks are always built from the 2D
``positions``.
...
class TestQwen35MRoPEPositions(unittest.TestCase):
    ...
    def test_text_batch_routes_2d_positions_to_layers(self):
        ...
        # No mrope: layers see the plain 2D positions.
        self.assertTrue(torch.equal(sink["positions"], positions))
        # Masks come from the 2D positions.
        self.assertEqual(
    ...
    def test_multimodal_batch_routes_mrope_to_layers(self):
        model, sink, parallel_dims, parallelism = self._build_stub_model()
        positions = torch.tensor([[0, 1, 2, 0, 1], [0, 1, 2, 3, 4]], dtype=torch.int32)
        # 3D T/H/W positions whose H/W channels differ from the 2D positions,
        # so routing the wrong tensor to the layers is detectable.
        ...
        # mrope present: layers see the 3D mrope positions, not the 2D positions.
        self.assertEqual(sink["positions"].ndim, 3)
        self.assertTrue(torch.equal(sink["positions"], mrope_positions))
        # Masks are still built from the 2D positions, not the 3D mrope.
        self.assertEqual(
            batch["attention_masks"]["deltanet"].cu_seq_q_host, (0, 3, 5, 10)
        )
