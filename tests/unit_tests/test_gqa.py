...
def test_qwen3_gqa_varlen_vs_flash_forward(batch_size, seq_len, dtype):
...
@pytest.mark.parametrize("seq_len", [16, 128])
@pytest.mark.parametrize("dtype", [torch.bfloat16, torch.float16])
def test_llama3_gqa_varlen_vs_flash_forward(batch_size, seq_len, dtype):

    sdpa_gqa_attention = llama3.model.model.Attention(
        sdpa_args := llama3.model.args.TransformerModelArgs(
            dim = 64,
            n_heads = 8,
            n_kv_heads = 4,
    ...
    varlen_gqa_attention = llama3.model.model.Attention(
        varlen_args := llama3.model.args.TransformerModelArgs(
            dim = 64,
            n_heads = 8,
            n_kv_heads = 4,
            max_seq_len = seq_len,
    ...
    rope_cache = llama3.model.model.precompute_freqs_cis(
        varlen_args.dim // varlen_args.n_heads,
        varlen_args.max_seq_len,
        varlen_args.rope_theta
    ).to(device="cuda:0", dtype=dtype)

    sdpa_out   =   sdpa_gqa_attention(x.detach().clone(), freqs_cis=rope_cache, attention_masks=None)
    varlen_out = varlen_gqa_attention(x.detach().clone(), freqs_cis=rope_cache, attention_masks=mask)
