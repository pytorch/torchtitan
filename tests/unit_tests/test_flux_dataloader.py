...
class TestFLUXDataLoader:
    def test_FLUX_dataloader(self):
        ...
        dl = self._build_dataloader(
            dataset_name, batch_size, seq_len, world_size, rank, seed
        )
    ...
    def _load_t5(self, device: str = "cuda", max_length: int = 512) -> HFEmbedder:
        # max length 64, 128, 256 and 512 should work (if your sequence is short enough)
        return HFEmbedder(
            "google/t5-v1_1-xxl", max_length=max_length, torch_dtype=torch.bfloat16
        ).to(device)

    def _load_clip(self, device: str = "cuda") -> HFEmbedder:
    ...
    def _build_dataloader(
        self, dataset_name, batch_size, seq_len, world_size, rank, seed
    ):
        config = JobConfig()
        config.parse_args(
            [
        ...
        return build_flux_dataloader(
            ...
            dp_rank=rank,
            embedder=[self._load_t5(seq_len), self._load_clip()],
            job_config=config,
            infinite=False,
        )
