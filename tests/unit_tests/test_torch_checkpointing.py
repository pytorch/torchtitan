...
from torch_checkpointing.checkpoint_writer import _temp_dir_path, CheckpointWriterConfig
...
class TorchCheckpointingManagerTest(unittest.TestCase):
    ...
    def test_multirank_save_requires_checkpoint_barrier(
        self,
        _is_initialized,
        _world_size,
        _rank,
        ...
        backend_config = BackendCheckpointManager.Config.with_sync_save()

        with self.assertRaisesRegex(ValueError, "requires a checkpoint barrier"):
            self._build_manager(config, backend_config=backend_config)
    ...
    def test_purge_runs_before_this_step_save_is_issued(self) -> None:
        ...
        with mock.patch.object(
            ...
            side_effect=lambda *, save_pending: calls.append(("purge", save_pending)),
        ):
            self.assertTrue(manager.save(curr_step=1))

        # save_pending: the purge runs before the save, so the checkpoint about
        # to be written is not on disk yet and must still be counted.
        self.assertEqual([("purge", True), "save"], calls)
        backend_manager.save_result.set_result(None)
        manager.close()
    ...
    def _purge_manager(self, keep_latest_k: int, entries: list[str]):
        ...
        manager._step_dir_pattern = manager_module._step_dir_pattern("tmp_")
    ...
    def _purge(self, manager) -> set[str]:
        # This manager purges before issuing its save, so it always reserves a
        # slot for the checkpoint about to be written.
        manager._purge_stale_checkpoints(save_pending=True)
        purged = set()
    ...
    def test_purge_reserves_a_slot_for_the_imminent_save(self, _rank) -> None:
        ...
        self.assertEqual({"/checkpoint/step-1"}, self._purge(manager))

    @mock.patch("torch.distributed.get_rank", return_value=0)
    def test_purge_treats_metadata_less_directories_as_abandoned(self, _rank) -> None:
        # This manager purges before issuing its save, so nothing is being
        # written and a step-N with no metadata is the residue of a save that
        # died. It is removed instead of occupying one of the k slots.
        manager = self._purge_manager(2, ["step-1", "step-2", "step-3"])
        manager._storage.isfile.return_value = False

        self.assertEqual(set(), self._purge(manager))

        self.assertEqual(
            [
                mock.call("/checkpoint/step-1"),
                mock.call("/checkpoint/step-2"),
                mock.call("/checkpoint/step-3"),
            ],
    ...
    def test_purge_deletes_abandoned_temporaries_without_spending_a_slot(
        ...
        # step-1 survives, and the temporaries go straight to storage rather
        # than onto the purge queue.
        self.assertEqual(set(), self._purge(manager))
        self.assertEqual(
            [
                mock.call("/checkpoint/tmp_step-2"),
                mock.call("/checkpoint/tmp_step-3"),
            ],
            manager._storage.remove.call_args_list,
        )
    ...
    def test_hf_final_save_converts_and_consolidates_before_commit(
        ...
        staged = _temp_dir_path(
            Path(sync_manager.save_calls[0][0]),
            sync_config.save.writer_config.temp_dir_prefix,
        )
