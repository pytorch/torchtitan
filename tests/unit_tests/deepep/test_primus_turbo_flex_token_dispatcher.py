...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_permute_basic(self):
        ...
        permuted_tokens, permuted_probs, sorted_indices = permute(tokens, routing_map, probs)
    ...
    def test_permute_without_probs(self):
        ...
        permuted_tokens, permuted_probs, sorted_indices = permute(tokens, routing_map, None)
    ...
    def test_unpermute_basic(self):
        ...
        restored_tokens = unpermute(permuted_tokens, sorted_indices, original_tokens.shape)
    ...
    def test_unpermute_different_dtypes(self):
        ...
        for dtype in [torch.float32, torch.float16, torch.bfloat16]:
            ...
            permuted_tokens = torch.randn(num_tokens, hidden_dim, device=self.device, dtype=dtype)
...
class TestUnit(unittest.TestCase):
    ...
    def test_dispatch_preprocess_shape_handling(self, mock_set_deepep):
        ...
        selected_indices_3d = torch.randint(0, self.num_moe_experts, (batch, seq, topk), device=self.device)

        dispatcher.dispatch_preprocess(top_scores_3d, selected_indices_3d)

        # Should be flattened to 2D
        expected_tokens = batch * seq
        self.assertEqual(dispatcher._comm_manager.token_probs.shape, (expected_tokens, topk))
        self.assertEqual(dispatcher._comm_manager.token_indices.shape, (expected_tokens, topk))
    ...
    def test_token_dispatch_sync_mode(self, mock_set_deepep):
        ...
        with patch.object(dispatcher._comm_manager, "dispatch") as mock_dispatch:
            mock_dispatched_hidden = torch.randn(num_tokens * 2, self.hidden_dim, device=self.device)
            mock_dispatch.return_value = mock_dispatched_hidden
            dispatcher._comm_manager.dispatched_probs = torch.rand(num_tokens * 2, device=self.device)

            mock_group = MagicMock()
            result_hidden, _ = dispatcher.token_dispatch(
                hidden_states, probs=None, group=mock_group, async_finish=False, allocate_on_comm_stream=False
            )
            mock_dispatch.assert_called_once_with(hidden_states, mock_group, False, False)
    ...
    @patch("torchtitan.distributed.deepep.utils.fused_indices_to_multihot")
    @patch("torchtitan.distributed.deepep.utils.permute")
    def test_dispatch_postprocess_verifies_metadata(self, mock_permute, mock_fused_indices_to_multihot, mock_set_deepep):
        ...
        dispatcher._comm_manager.dispatched_indices = torch.randint(0, num_dispatched_tokens, (num_dispatched_tokens,), device=self.device)
        dispatcher._comm_manager.dispatched_probs = torch.rand(num_dispatched_tokens, device=self.device, dtype=torch.float32)
        dispatcher._comm_manager.num_local_experts = self.num_moe_experts
        dispatcher._comm_manager.tokens_per_expert = torch.tensor([16] * self.num_moe_experts, device=self.device)
        ...
        self.assertEqual(dispatcher._comm_manager.hidden_shape_before_permute, hidden_states.shape)
    ...
    def test_token_combine_sync_mode(self, mock_fused_combine, mock_set_deepep):
        ...
        result = dispatcher.token_combine(hidden_states, group=mock_group, async_finish=False, allocate_on_comm_stream=False)

        mock_fused_combine.assert_called_once_with(hidden_states, mock_group, mock_handle, async_finish=False, allocate_on_comm_stream=False)
    ...
    def test_token_combine_handle_cleanup(self, mock_set_deepep):
        ...
        # Call token_combine
        with patch("torchtitan.distributed.deepep.utils.fused_combine") as mock_fused_combine:
            mock_fused_combine.return_value = (
                torch.randn(64, self.hidden_dim, device=self.device),
                MagicMock()
            )
            dispatcher.token_combine(torch.randn(64, self.hidden_dim, device=self.device))
    ...
    def test_end_to_end_dispatch_combine_flow(self, mock_multihot, mock_set_deepep):
        ...
        dispatched_hidden = torch.randn(num_dispatched, self.hidden_dim, device=self.device)
        dispatched_indices = torch.randint(0, num_dispatched, (num_dispatched,), device=self.device)
        dispatched_probs = torch.rand(num_dispatched, device=self.device, dtype=torch.float32)
        tokens_per_expert = torch.tensor([num_dispatched // self.num_moe_experts] * self.num_moe_experts, device=self.device)
        ...
        with patch.object(dispatcher._comm_manager, 'dispatch') as mock_dispatch:
            ...
            result_hidden, result_probs = dispatcher.token_dispatch(hidden_states, group=mock_group)

        # 3. Dispatch postprocess (simulates expert computation)
        routing_map = torch.randint(0, 2, (num_dispatched, self.num_moe_experts), device=self.device).bool()
        ...
        processed_hidden, _, processed_probs = dispatcher.dispatch_postprocess(expert_output, dispatched_probs)
    ...
    def test_end_to_end_preserves_dtype(self, mock_set_deepep):
        ...
        for dtype in [torch.float32, torch.float16]:
            ...
            hidden_states = torch.randn(num_tokens, self.hidden_dim, device=self.device, dtype=dtype)

            # Mock the entire flow
            with patch("torchtitan.distributed.deepep.utils.fused_dispatch"), \
                 patch("torchtitan.distributed.deepep.utils.fused_combine") as mock_combine, \
                 patch("torchtitan.distributed.deepep.utils.fused_indices_to_multihot"):

                combined_hidden = torch.randn(num_tokens, self.hidden_dim, device=self.device, dtype=dtype)
...
class TestDistributed(unittest.TestCase):
    ...
    def test_token_dispatch_distributed(self, mock_fused_dispatch):
        ...
        torch.manual_seed(42)
        top_scores = torch.rand(num_tokens, self.moe_router_topk, device=self.device, dtype=self.dtype)
        selected_indices = torch.randint(0, self.num_moe_experts, (num_tokens, self.moe_router_topk), device=self.device)

        dispatcher.dispatch_preprocess(top_scores, selected_indices)

        hidden_states = torch.randn(num_tokens, self.hidden_dim, device=self.device, dtype=self.dtype)
    ...
    def test_dispatch_postprocess_distributed(self):
        ...
        # Postprocess
        permuted_hidden, tokens_per_expert, permuted_probs = dispatcher.dispatch_postprocess(
    ...
    def test_dispatch_expert_assignment_distributed(self):
        ...
        top_scores = torch.ones(num_tokens, self.moe_router_topk, device=self.device, dtype=self.dtype) * 0.5
        # Distribute tokens across all experts
        selected_indices = torch.arange(num_tokens * self.moe_router_topk, device=self.device) % self.num_moe_experts
        ...
        dist.barrier()
        if self.rank == 0:
            print(f"✓ Expert assignment distributed: {total_count}/{expected_total} tokens")
    ...
    def test_combine_preprocess_distributed(self):
        ...
        permuted_hidden, _, _ = dispatcher.dispatch_postprocess(dispatched_hidden, dispatched_probs)
    ...
    def test_end_to_end_preserves_dtype_distributed(self):
        ...
        for dtype in [torch.bfloat16]:
            ...
            torch.manual_seed(42)
            top_scores = torch.rand(num_tokens, self.moe_router_topk, device=self.device, dtype=dtype)
            selected_indices = torch.randint(0, self.num_moe_experts, (num_tokens, self.moe_router_topk), device=self.device)
            hidden_states = torch.randn(num_tokens, self.hidden_dim, device=self.device, dtype=dtype)
            ...
            world_group = dist.distributed_c10d._get_default_group()
            dispatched_hidden, dispatched_probs = dispatcher.token_dispatch(
                hidden_states, group=world_group, async_finish=False, allocate_on_comm_stream=False
            )
            ...
            restored_hidden = dispatcher.combine_preprocess(expert_output)
            combined_output = dispatcher.token_combine(
                restored_hidden, group=world_group, async_finish=False, allocate_on_comm_stream=False
            )
            final_output = dispatcher.combine_postprocess(combined_output)
    ...
    def test_data_consistency_across_ranks(self):
        ...
        local_indices = dispatcher._comm_manager.token_indices.flatten()[:10]  # First 10 for comparison
