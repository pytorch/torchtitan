...
def build_model_tests_list() -> list[OverrideDefinitions]:
    ...
    model_tests = [
        ...
        OverrideDefinitions(
            [
                [
                    "--module kimi_k3 --config kimi_k3_debugmodel",
                    "--parallelism.data_parallel_shard_degree 2",
                ],
            ],
            "Kimi K3 multimodal FSDP",
            "kimi_k3_mm_fsdp",
            ngpu=2,
            supports_spmd_types=False,
        ),
    ]

    return [
        _configure_spmd_backend_and_typecheck(t) if t.supports_spmd_types else t
